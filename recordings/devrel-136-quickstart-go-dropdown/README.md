...
- 0–4s: form clean, only `Python` selected, dropdown closed.
- ~4–6s: dropdown opens, `Go` highlighted.
- ~6–7s: dropdown closes unexpectedly while mouse is still over `Go`. No
  chip added.
- ~7–9s: dropdown reopened, second click on `Go` succeeds — `Python + Go`
  chips visible.
