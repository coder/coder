# Frontend Guide (workflow and conventions)

The broader frontend conventions behind the FE1 to FE10 rules in `../SKILL.md`.
Load this on demand when you need commands, navigation, component/styling
conventions, performance guidance, or the full React rules.

FE1 to FE10 in `../SKILL.md` are the normative, referenceable contract. The
sections here are supporting detail: where a section elaborates a rule it is
tagged with that code (for example, `## Accessibility (supports FE6)`), so
guidance stays tied to a named rule rather than an anonymous bullet list.
Sections without a tag are general React correctness, not additional rules.

## TypeScript LSP Navigation (USE FIRST)

When investigating or editing TypeScript/React code, always use the TypeScript
language server tools for accurate navigation:
...
- `pnpm check` - Biome check (lint and format), errors on warnings
- `pnpm lint:types` - TypeScript typecheck (`tsc`)
- `pnpm lint` - Full lint suite (Biome, TypeScript types, circular deps, React Compiler, knip)
...
## Components (supports FE3)

Component reuse is a non-negotiable (FE3). See the skill body first.
...
- **Match existing patterns** in the same file before introducing new
  ...
  If sibling components initialize state lazily with `useState(initialFn)`,
  match that approach rather than introducing a different one in the same file
  without reason. (`useMemo` computes a derived value; it does not initialize
  state.)
...
## TypeScript type safety (supports FE2)
...
## React Query patterns (supports FE7)
...
## Testing patterns (supports FE10)
...
## Robustness (supports FE5)

- When rendering user-facing text from nullable/optional data, always
  provide a **visible fallback** (e.g., "Untitled", "N/A"). Never render
  a blank cell or element.
...
- `src/pages/AgentsPage/` (including `components/ChatElements/`) and
  `src/pages/AIBridgePage/` are opted into React Compiler via
  `babel-plugin-react-compiler` (see the include lists in
  `site/vite.config.mts` and `site/scripts/check-compiler.mjs`). The compiler
...
- Outside compiler-opted directories, still avoid premature memoization.
  `useCallback` and `useMemo` are antipatterns unless a memoized consumer
  needs a stable reference or a computation is measurably expensive.
...
1. `pnpm check` - Biome check (lint and format)
2. `pnpm lint` - Full lint suite (Biome, TypeScript types, circular deps, React Compiler, knip)
...
6. If the diff touches `site/src/`, run the deterministic FE1 to FE10
   self-review in `self-review.md` and fix every FAIL before creating the PR

## React rules

### 1. Purity and immutability

- **Components and custom Hooks must be pure and idempotent.** Same inputs produce the same output; move side-effects to event handlers or Effects.
...
- **Never use `key={String(booleanState)}`** to force remounts. When the
  ...
  renders. Any changing `key` remounts, so use one only when reset-by-remount
  is intentional and no exit transition must survive; otherwise drive the
  change through state without touching `key`.

### 7. Refs and DOM access
