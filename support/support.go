...
func DeploymentInfo(ctx context.Context, client *codersdk.Client, log slog.Logger) Deployment {
	...
	eg.Go(func() error {
		...
		if err != nil {
			return fmt.Errorf("fetch deployment config: %w", err)
		}
		d.Config = dc
		return nil
	})
	eg.Go(func() error {
		...
		if err != nil {
			return fmt.Errorf("fetch health report: %w", err)
		}
		d.HealthReport = &hr
		return nil
	})
	eg.Go(func() error {
		...
		if err != nil {
			return fmt.Errorf("fetch experiments: %w", err)
		}
		d.Experiments = exp
		return nil
	})
...
func NetworkInfo(ctx context.Context, client *codersdk.Client, log slog.Logger) Network {
	...
	eg.Go(func() error {
		...
		if err != nil {
			return fmt.Errorf("fetch coordinator debug page: %w", err)
		}
		defer coordResp.Body.Close()
		bs, err := io.ReadAll(coordResp.Body)
		if err != nil {
			return fmt.Errorf("read coordinator debug page: %w", err)
		}
	...
	eg.Go(func() error {
		...
		if err != nil {
			return fmt.Errorf("fetch tailnet debug page: %w", err)
		}
		defer tailResp.Body.Close()
		bs, err := io.ReadAll(tailResp.Body)
		if err != nil {
			return fmt.Errorf("read tailnet debug page: %w", err)
		}
	...
	eg.Go(func() error {
		...
		if err != nil {
			return fmt.Errorf("run interfaces report: %w", err)
		}
		n.Interfaces = rpt
		return nil
	})
...
func WorkspaceInfo(ctx context.Context, client *codersdk.Client, log slog.Logger, workspaceID uuid.UUID) Workspace {
	...
	eg.Go(func() error {
		...
		if err != nil {
			return fmt.Errorf("fetch provisioner job logs: %w", err)
		}
		defer closer.Close()
	...
	eg.Go(func() error {
		if w.Workspace.TemplateActiveVersionID == uuid.Nil {
			return fmt.Errorf("workspace has nil template active version id")
		}
		tv, err := client.TemplateVersion(ctx, w.Workspace.TemplateActiveVersionID)
		if err != nil {
			return fmt.Errorf("fetch template active version id")
		}
		w.TemplateVersion = tv
		if tv.Job.FileID == uuid.Nil {
			return fmt.Errorf("template file id is nil")
		}
		...
		if ctype != codersdk.ContentTypeZip {
			return fmt.Errorf("expected content-type %s, got %s", codersdk.ContentTypeZip, ctype)
		}
	...
	eg.Go(func() error {
		if w.Workspace.TemplateID == uuid.Nil {
			return fmt.Errorf("workspace has nil version id")
		}
		tpl, err := client.Template(ctx, w.Workspace.TemplateID)
		if err != nil {
			return fmt.Errorf("fetch template")
		}
		w.Template = tpl
		return nil
	})
	eg.Go(func() error {
		if ws.LatestBuild.ID == uuid.Nil {
			return fmt.Errorf("workspace has nil latest build id")
		}
		params, err := client.WorkspaceBuildParameters(ctx, ws.LatestBuild.ID)
		if err != nil {
			return fmt.Errorf("fetch workspace build parameters: %w", err)
		}
		w.Parameters = params
		return nil
	})
...
func AgentInfo(ctx context.Context, client *codersdk.Client, log slog.Logger, agentID uuid.UUID) Agent {
	...
	eg.Go(func() error {
		...
		if err != nil {
			return fmt.Errorf("fetch workspace agent: %w", err)
		}
		sanitizeEnv(agt.EnvironmentVariables)
		a.Agent = &agt
		return nil
	...
	eg.Go(func() error {
		...
		if err != nil {
			return fmt.Errorf("fetch agent startup logs: %w", err)
		}
		defer closer.Close()
...
func connectedAgentInfo(ctx context.Context, client *codersdk.Client, log slog.Logger, agentID uuid.UUID, eg *errgroup.Group, a *Agent) (closer func()) {
	...
	eg.Go(func() error {
		...
		if err != nil {
			return fmt.Errorf("fetch agent prometheus metrics: %w", err)
		}
	...
	eg.Go(func() error {
		...
		if err != nil {
			return fmt.Errorf("get agent magicsock page: %w", err)
		}
	...
	eg.Go(func() error {
		...
		if err := json.NewDecoder(bytes.NewReader(manifestRes)).Decode(&a.Manifest); err != nil {
			return fmt.Errorf("decode agent manifest: %w", err)
		}
	...
	eg.Go(func() error {
		...
		if err != nil {
			return fmt.Errorf("fetch coder agent logs: %w", err)
		}
		a.Logs = logBytes
		return nil
	})
	eg.Go(func() error {
		...
		if err != nil {
			return fmt.Errorf("get listening ports: %w", err)
		}
...
func Run(ctx context.Context, d *Deps) (*Bundle, error) {
	var b Bundle
	if d.Client == nil {
		return nil, fmt.Errorf("developer error: missing client!")
	}
	...
	if err != nil {
		return &b, fmt.Errorf("check authorization: %w", err)
	}
	for k, v := range authResp {
		if !v {
			return &b, fmt.Errorf("failed authorization check: cannot %s", k)
		}
	}
