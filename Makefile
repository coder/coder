...
# Install Vale styles
.vale/styles/.installed: .vale.ini
	vale sync
	touch "$@"

# Enhanced markdown linting with Vale
lint/markdown: node_modules/.installed .vale/styles/.installed
	echo "$(GREEN)==>$(RESET) $(BOLD)lint/markdown$(RESET)"
	# Run markdownlint for formatting/syntax
	pnpm exec markdownlint-cli2 --config .markdownlint.jsonc $(shell find docs -name '*.md') *.md
	# Run Vale for prose quality (exclude generated docs)
	vale --config .vale.ini docs --glob='!docs/reference/api/*' --glob='!docs/changelogs/*'
.PHONY: lint/markdown

# Incremental markdown linting (for changed files)
lint/markdown-incremental: node_modules/.installed .vale/styles/.installed
	echo "$(GREEN)==>$(RESET) $(BOLD)lint/markdown-incremental$(RESET)"
ifdef CHANGED_MD_FILES
	# Markdownlint on changed files
	echo "$(CHANGED_MD_FILES)" | tr ' ' '\n' | xargs -r pnpm exec markdownlint-cli2 --config .markdownlint.jsonc
	# Vale on changed files (exclude generated)
	echo "$(CHANGED_MD_FILES)" | tr ' ' '\n' | grep -v -E "(docs/reference/api/|docs/changelogs/)" | xargs -r vale --config .vale.ini
else
	echo "No changed files specified, use lint/markdown for full check"
endif
.PHONY: lint/markdown-incremental

# Smart lint target that chooses incremental vs full
lint/markdown-smart: node_modules/.installed .vale/styles/.installed
	echo "$(GREEN)==>$(RESET) $(BOLD)lint/markdown-smart$(RESET)"
ifdef FULL_CHECK_REASON
	echo "Full check required: $(FULL_CHECK_REASON)"
	$(MAKE) lint/markdown
else ifdef CHANGED_MD_FILES
	echo "Incremental check on changed files"
	$(MAKE) lint/markdown-incremental CHANGED_MD_FILES="$(CHANGED_MD_FILES)"
else
	echo "No changes detected, skipping checks"
endif
.PHONY: lint/markdown-smart

# Vale-only linting (for prose quality focus)
lint/vale: .vale/styles/.installed
	echo "$(GREEN)==>$(RESET) $(BOLD)lint/vale$(RESET)"
	vale --config .vale.ini docs --glob='!docs/reference/api/*' --glob='!docs/changelogs/*'
.PHONY: lint/vale
