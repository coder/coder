...
func main() {
	...
	if diff := cmp.Diff(string(dumpBytesAfter), string(stripGenPreamble(expectedSchemaAfter))); diff != "" {
		friendlyError(os.Stderr, fmt.Errorf("Schema differs from expected after migration: %s", diff), migrateFromVersion, migrateToVersion)
		os.Exit(1)
	}
...
func makeMigrateFS(version string) (fs.FS, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("git archive: %s\n", out)
	}
...
func gitShow(path, version string) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("git show: %s\n", out)
	}
	return out, nil
}
