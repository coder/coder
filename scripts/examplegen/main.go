...
func run(lint bool) error {
	...
	if len(errs) > 0 {
		return fmt.Errorf("parse failed: %w", errors.Join(errs...))
	}
...
func parseTemplateExample(projectFS, examplesFS fs.FS, name string) (te *codersdk.TemplateExample, err error) {
	...
	defer func() {
		...
		if len(errs) > 0 {
			err = fmt.Errorf("example %q has errors", name)
			for _, e := range errs {
	...
	if err != nil {
		return nil, errors.New("missing README.md")
	}
	...
	if exists {
		...
		if !valid {
			errs = append(errs, fmt.Errorf("tags isn't a slice: type %T", tagsRaw))
		} else {
			for _, tagI := range tagsI {
				tag, valid := tagI.(string)
				if !valid {
					errs = append(errs, fmt.Errorf("tag isn't a string: type %T", tagI))
					continue
				}
	...
	if err != nil {
		...
		if err != nil {
			errs = append(errs, fmt.Errorf("icon does not exist: %w", err))
		}
		if !strings.HasPrefix(cleanPath, filepath.Join("site", "static")) {
			errs = append(errs, fmt.Errorf("icon is not in site/static/: %q", icon))
		}
		icon, err = filepath.Rel(filepath.Join("site", "static"), cleanPath)
		if err != nil {
			errs = append(errs, fmt.Errorf("cannot make icon relative to site/static: %w", err))
		}
	}
	if len(errs) > 0 {
		return nil, errors.New("front matter validation failed")
	}
...
func getString(m map[string]any, key string) (string, error) {
	v, ok := m[key]
	if !ok {
		return "", fmt.Errorf("front matter does not contain %q", key)
	}
	vv, ok := v.(string)
	if !ok {
		return "", fmt.Errorf("%q isn't a string", key)
	}
	return vv, nil
}
