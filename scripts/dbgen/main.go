...
func run() error {
	...
	if err != nil {
		return fmt.Errorf("stub dbmem: %w", err)
	}
...
`, params.Returns, params.FuncName, params.Parameters, params.FuncName, params.Returns)
	...
	if err != nil {
		return fmt.Errorf("stub dbmetrics: %w", err)
	}
	err = orderAndStubDatabaseFunctions(filepath.Join(databasePath, "dbauthz", "dbauthz.go"), "q", "querier", func(params stubParams) string {
		return `panic("not implemented")`
	})
	if err != nil {
		return fmt.Errorf("stub dbauthz: %w", err)
	}
	err = generateUniqueConstraints()
	if err != nil {
		return fmt.Errorf("generate unique constraints: %w", err)
	}
	err = generateForeignKeyConstraints()
	if err != nil {
		return fmt.Errorf("generate foreign key constraints: %w", err)
	}
	return nil
}
...
`)
	for _, query := range uniqueConstraints {
		...
		default:
			return fmt.Errorf("unknown unique constraint format: %s", query)
		}
...
`)
	for _, query := range foreignKeyConstraints {
		...
		case strings.Contains(query, "ALTER TABLE") && strings.Contains(query, "ADD CONSTRAINT"):
			name = strings.Split(query, " ")[6]
		default:
			return fmt.Errorf("unknown foreign key constraint format: %s", query)
		}
...
func orderAndStubDatabaseFunctions(filePath, receiver, structName string, stub func(params stubParams) string) error {
	...
	if err != nil {
		return fmt.Errorf("read dbmem: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("parse dbmem: %w", err)
	}
	pointer := false
	...
	for _, fn := range funcs {
		...
		if !ok {
			...
			if err != nil {
				return fmt.Errorf("compile func decl: %w", err)
			}
			// Not implemented!
			decl = &dst.FuncDecl{
	...
	if err != nil {
		return fmt.Errorf("restore package: %w", err)
	}
	var buf bytes.Buffer
	err = format.Node(&buf, restorer.Fset, restored)
	if err != nil {
		return fmt.Errorf("format package: %w", err)
	}
...
func stub() {
	...
	if len(f.Decls) != 1 {
		return nil, fmt.Errorf("expected 1 decl, got %d", len(f.Decls))
	}
	decl, ok := f.Decls[0].(*dst.FuncDecl)
	if !ok {
		return nil, fmt.Errorf("expected func decl, got %T", f.Decls[0])
	}
	return decl, nil
}
...
func readQuerierFunctions() ([]querierFunction, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("parse querier.go: %w", err)
	}
	funcs, err := loadInterfaceFuncs(f, "sqlcQuerier")
	if err != nil {
		return nil, fmt.Errorf("load interface %s funcs: %w", "sqlcQuerier", err)
	}
	customFile, err := parseDBFile("modelqueries.go")
	if err != nil {
		return nil, fmt.Errorf("parse modelqueriers.go: %w", err)
	}
	// Custom funcs should be appended after the regular functions
	customFuncs, err := loadInterfaceFuncs(customFile, "customQuerier")
	if err != nil {
		return nil, fmt.Errorf("load interface %s funcs: %w", "customQuerier", err)
	}
...
func loadInterfaceFuncs(f *dst.File, interfaceName string) ([]querierFunction, error) {
	...
	for _, decl := range f.Decls {
		...
		for _, spec := range genDecl.Specs {
			...
			if !ok {
				return nil, fmt.Errorf("unexpected sqlcQuerier type: %T", typeSpec.Type)
			}
			break
		}
	}
	if querier == nil {
		return nil, fmt.Errorf("querier not found")
	}
	funcs := []querierFunction{}
