		...
		Children: []*serpent.Command{
			{
				...
				Handler: func(inv *serpent.Invocation) error {
					ctx := inv.Context()
					if len(inv.Args) == 0 {
						return errors.New("version argument missing")
					}
					if !r.dryRun && r.ghToken == "" {
						return errors.New("GitHub personal access token is required, use --gh-token or GH_TOKEN")
					}
...
func checkCoderRepo(path string) error {
	...
	if !strings.Contains(remote, "github.com") || !strings.Contains(remote, "coder/coder") {
		return fmt.Errorf("origin is not set to the coder/coder repository on github.com")
	}
	return nil
}
...
func (r *releaseCommand) promoteVersionToStable(ctx context.Context, inv *serpent.Invocation, version string) error {
	...
	if currentStable.GetTagName() == version {
		return fmt.Errorf("version %q is already the latest stable release", version)
	}
	...
	case newStable == nil:
		return fmt.Errorf("version %q is not found in the last %d releases", version, perPage)
	}
	...
	if !r.dryRun {
		...
		if err != nil {
			return fmt.Errorf("edit release failed: %w", err)
		}
...
func (r *releaseCommand) autoversion(ctx context.Context, channel, version string) error {
	...
	for _, file := range files {
		...
		if err != nil {
			return fmt.Errorf("autoversion file failed: %w", err)
		}
	}
	return nil
}
...
func (r *releaseCommand) autoversionFile(ctx context.Context, file, channel, version string) error {
		...
		if matchRe != nil {
			...
			if match := matchRe.FindStringSubmatchIndex(line); match != nil {
				...
				if vg == -1 {
					...
					return fmt.Errorf("bug: version group not found in match")
				}
				start := match[vg*2]
				end := match[vg*2+1]
	...
	if matchRe != nil {
		return fmt.Errorf("match not found in file")
	}
