...
// resourceRenames maps internal Go struct names to their user-facing
...
func main() {
	...
	flag.StringVar(&markerPath, "marker-path", "docs/admin/security/audit-logs.md", "Doc path to embed in generated marker comments")
	flag.Parse()

	generatorPrefix := fmt.Sprintf("<!-- Code generated by 'make %s'. DO NOT EDIT -->", markerPath)
	generatorSuffix := fmt.Sprintf("<!-- End generated by 'make %s'. -->", markerPath)
...
func readAuditableResources(allowlist map[string]struct{}) AuditableResourcesMap {
	...
	for resourceName, resourceFields := range audit.AuditableResources {
		...
		if renamed, ok := resourceRenames[friendlyResourceName]; ok {
...
func updateAuditDoc(doc []byte, auditableResourcesMap AuditableResourcesMap, generatorPrefix, generatorSuffix []byte) ([]byte, error) {
	...
	for _, resourceName := range sortedResourceNames {
		...
		_, _ = buffer.WriteString("\n|" + resourceName + "<br><i>" + auditActionsString + "</i>|<table><thead><tr><th>Field</th><th>Tracked</th></tr></thead><tbody>" + "|")
