...
Contains metrics extracted from the coder source code by the AST scanner (`scanner/scanner.go`), including AI Gateway metrics. The scanner resolves supported registerer prefixes from registration code and documents only the canonical prefix of the alias registerer. Unsupported cases remain in the static file.
