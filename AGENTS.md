...
- Audit: Read [Audit Approach](poc_audit/audit_approach.md) before working on audit journals, entity lifecycle, or entity identity. Its sections are marked by standing: Established holds decided positions, Derived holds reasoning that is offered for challenge, Findings holds verified facts about this codebase, and Open holds undecided questions. Do not treat Derived or Open content as decided. Its Terminology section defines "entry" and "record"; the three senses of "agent" are defined in the entity model below.
- Entity model: Read [Entity Model](poc_audit/entity_model.md) alongside the audit approach when working on entities, identity, or their relationships. It uses the same four sections of standing. Its Terminology section is authoritative for "actor", "sandbox", `workspace_agent`, "AI agent", and the principal and agent relation, including which sense takes the bare word "agent".
- Security findings: Read [Security Findings](poc_audit/security_findings.md) before working on credentials or authentication for `workspace_agent` entities. It records existing design problems, numbered P1 to P7, and the policy goals any fix must satisfy. Nothing in it is implemented. It describes what should change after the proof of concept, not how the system behaves today, so do not read it as a description of current behaviour.
