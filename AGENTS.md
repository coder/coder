...
Do not front-load everything; read what the task needs and as new
requirements appear.

- Dev server and development loop: [.claude/docs/WORKFLOWS.md](.claude/docs/WORKFLOWS.md)
- Git workflow and hooks: [.claude/docs/WORKFLOWS.md](.claude/docs/WORKFLOWS.md)
- API design guardrails: [.claude/docs/WORKFLOWS.md](.claude/docs/WORKFLOWS.md)
...
- Write comments only when they preserve concise, verifiable,
  non-obvious information that the code, names, types, or tests do not
  convey, especially why a surprising constraint or behavior exists.
  Never restate code, speculate, narrate future work, or describe change
  history (no `// Added per PR feedback`). Delete comments that add no
  information; preserve existing comments that explain non-obvious
  behavior.
