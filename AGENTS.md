...
- PoC design work: Read [poc_audit guidelines](poc_audit/AGENTS.md) before working on audit, entity lifecycle, entity identity, or credentials for `workspace_agent` entities. That directory holds the design documents for a proof of concept. Its guide indexes them and explains how their sections are marked by standing, so that undecided material is not mistaken for decided. It records positions rather than behaviour, so do not read it as a description of the running system, even where code for a position now exists.
