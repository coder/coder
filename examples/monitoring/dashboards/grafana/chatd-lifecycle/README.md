...
`coderd_chatd_stage_duration_seconds{stage,scope,chat_kind,organization_name,model}`
...
| `basic` (default) | `stage_duration_seconds` (12 buckets), `turn_time_seconds_total`, `turns_total`, `turn_outcomes_total`, `stage_anomalies_total`, `stage_metrics_level`          | the `[basic]` stages above         |
...
Series cost per `(chat_kind, organization_name, model)` tuple is
roughly 90 at `basic` (plus ~180 per replica per organization for the
model-less wait and connect stages) and roughly 1,400 at `full` in the
worst case, when every stage occurs in every tuple. Multiply by
replicas and by the number of models each organization uses.
Single-organization deployments pay nothing for the organization
label. Almost all of the `basic` cost is `stage_duration_seconds`; the
turn partition is one counter per category plus `turns_total` and
`turn_outcomes_total` per tuple.
  ...
  - name: chatd-latency
    rules:
      ...
      - alert: ChatdSlowTimeToFirstToken
        expr: |
          1 - (
            sum by (model) (rate(coderd_chatd_stage_duration_seconds_bucket{stage="time_to_first_token", scope="turn", le=~"10(\\.0)?"}[10m]))
            /
...
`mcp_connect`, and `capacity_wait`, plus `stage_anomalies_total`,
`turn_outcomes_total`, the level gauge, and every metric outside chatd.
`chat_turn` is kept because the dashboard's chat kind, organization, and
model variables are sourced from it, and `capacity_wait` is kept so an
alert on capacity wait time can be written against it. With the second
...
metric_relabel_configs:
  ...
  - source_labels: [__name__]
    regex: 'coderd_chatd_(turn_time_.*|turns_total|turn_stage_.*|stage_share_of_turn.*)'
    action: drop
```
...
| `organization_name` | name of the organization that owns the chat, empty for stages recorded without a known chat             | `$organization` (multi-select) |
...
of the hierarchy the way `$model` does. `organization_name` is likewise
a property of the turn and `$organization` filters the same panels; it
is applied wherever `$chat_kind` is. Background provider calls carry
...
observed once per turn with the turn's `chat_kind`, `organization_name`,
and `model` already known, so all three variables apply to them without
exception.
...
**Turns per minute** - closed turns per minute, split by model.
...
this row does not see. Dimensions: `$chat_kind`, `$organization`, and
`$model` apply; `model` is empty for turns that ended before a model was
resolved.
...
divided by all categories' total seconds. Dimensions: `$chat_kind`,
`$organization`, and `$model` apply; the grouping is fixed to `model`, because Grafana
...
arrives. Dimensions: this histogram is labeled by provider and model
only, so `$model` applies and `$chat_kind` and `$organization` do not
apply to this metric. The `time_to_first_token` stage in the profile
measures the same interval and does honor all three filters. How to
read: the primary user-perceived responsiveness metric for streaming.

**Background provider calls (`$stat`)** - rate and selected `$stat`
duration of background-scope `provider_attempt` samples: detached
title, summary, and status-label requests that are excluded from every
other panel. Dimensions: pinned to the background scope of the
`provider_attempt` stage; all three variables (`$chat_kind`,
`$organization`, `$model`) apply. Summary and status-label generation
