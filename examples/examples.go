...
func parseAndVerifyExamples() (examples []codersdk.TemplateExample, err error) {
	...
	if err != nil {
		return nil, fmt.Errorf("open %s: %w", examplesJSON, err)
	}
	defer f.Close()
	b := bufio.NewReader(f)
	...
	_, _, err = b.ReadLine()
	if err != nil {
		return nil, fmt.Errorf("read %s: %w", examplesJSON, err)
	}
	err = json.NewDecoder(b).Decode(&examples)
	if err != nil {
		return nil, fmt.Errorf("decode %s: %w", examplesJSON, err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("get templates fs: %w", err)
	}
	...
	if want != got {
		return nil, fmt.Errorf("mismatch between %s and embedded files: want %q, got %q", examplesJSON, want, got)
	}
	return examples, nil
}
