...
- Check for **size and length bounds on user-supplied values** that are stored and reprocessed. A multi-megabyte string stored in an encrypted column creates persistent cost on every read (decrypt) and every write (encrypt). Unbounded user input that reaches a storage or crypto path needs an upper-bound check before the write.
