...
func (c *Client) GitSSHKey(ctx context.Context, userID string) (GitSSHKey, error) {
	res, err := c.request(ctx, http.MethodGet, fmt.Sprintf("/api/v2/users/%s/gitsshkey", userID), nil)
	if err != nil {
...
func (c *Client) RegenerateGitSSHKey(ctx context.Context, userID string) (GitSSHKey, error) {
	res, err := c.request(ctx, http.MethodPut, fmt.Sprintf("/api/v2/users/%s/gitsshkey", userID), nil)
	if err != nil {
		return GitSSHKey{}, xerrors.Errorf("execute request: %w", err)
	}
	defer res.Body.Close()
