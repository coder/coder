 ...
 when required by your organization's security policy.`,
	...
	opts := serpent.OptionSet{
		...
		},
		{
			...
			Description: "Encrypt sensitive data in the database with AES-256-GCM. The value must be a comma-separated list of base64-encoded keys. Each key, when base64-decoded, must be exactly 32 bytes in length. The first key will be used to encrypt new values. Subsequent keys will be used as a fallback when decrypting. During normal operation it is recommended to only set one key unless you are in the process of rotating keys with the `coder server dbcrypt rotate` command.",
			Flag:        "db-encryption-keys",
			Env:         "CODER_DB_ENCRYPTION_KEYS",
			Annotations: serpent.Annotations{}.Mark(annotationEnterpriseKey, "true").Mark(annotationSecretKey, "true"),
			Value:       &c.ExternalTokenEncryptionKeys,
		},
		{
			Name:        "External Token Encryption Keys",
			Description: "Deprecated: Use --db-encryption-keys instead.",
