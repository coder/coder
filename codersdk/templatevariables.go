...
func parseVariableValuesFromVarsFiles(varsFiles []string) ([]VariableValue, error) {
	...
	for _, varsFile := range varsFiles {
		...
		case ".tfvars":
			...
			if err != nil {
				return nil, fmt.Errorf("unable to parse HCL content: %w", err)
			}
		case ".json":
			t, err = parseVariableValuesFromJSON(content)
			if err != nil {
				return nil, fmt.Errorf("unable to parse JSON content: %w", err)
			}
		default:
			return nil, fmt.Errorf("unexpected tfvars format: %s", ext)
		}
		...
		if ctyType.Equals(cty.String) {
			...
			_ = ctyValue.ForEachElement(func(key, val cty.Value) (stop bool) {
				if !val.Type().Equals(cty.String) {
					err = fmt.Errorf("unsupported tuple item type: %s ", val.GoString())
					return true
				}
			...
			return nil, fmt.Errorf("unsupported value type (name: %s): %s", attribute.Name, ctyType.GoString())
		}
	}
...
func createVariablesMapFromFile(variablesFile string) (map[string]string, error) {
	if variablesFile == "" {
		return nil, fmt.Errorf("variable file name is not specified")
	}
...
func parseVariableValuesFromCommandLine(variables []string) ([]VariableValue, error) {
	...
	for _, keyValue := range variables {
		...
		if len(split) < 2 {
			return nil, fmt.Errorf("format key=value expected, but got %s", keyValue)
		}
		values = append(values, VariableValue{
			Name:  split[0],
			Value: split[1],
