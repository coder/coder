...
// OrganizationAISpendFilter narrows the organization AI spend export and
...
// per-provider AI spend for the organization matching the filter. The caller
// is responsible for closing the returned ReadCloser.
func (c *Client) ExportOrganizationAISpend(ctx context.Context, organization uuid.UUID, filter OrganizationAISpendFilter) (io.ReadCloser, error) {
...
// OrganizationAISpend returns one page of per-user AI spend for the
// organization matching the filter. It requests the JSON representation of
// the export endpoint, which otherwise serves CSV. A zero pagination limit
// uses the server default.
func (c *Client) OrganizationAISpend(ctx context.Context, organization uuid.UUID, filter OrganizationAISpendFilter, pagination Pagination) (OrganizationAISpendReport, error) {
	res, err := c.Request(ctx, http.MethodGet,
		fmt.Sprintf("/api/v2/organizations/%s/ai/spend/export", organization.String()),
		nil,
		filter.asRequestOption(),
