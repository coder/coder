...
3. Runs a Claude agent (`anthropics/claude-code-action`) with the instructions
   ...
   - **deduplicates against Linear** (team `ENG`): comments on the existing
     ...
     priority) via [`linear.sh`](./linear.sh);
...
| `LINEAR_ACCESS_KEY` | Linear API key (already used by `linear-release.yaml`). |
...
You can validate the Linear helper locally:

```sh
LINEAR_ACCESS_KEY=*** .github/flake-bot/linear.sh search "TestFoo"
```
...
- **Linear access is a thin GraphQL wrapper** (`linear.sh`), not the official
  Linear MCP server, to stay dependency-free and reviewable in CI.
- The agent will not open a low-confidence fix PR; a clean triage with no PR is
  a valid outcome.

## Files

- `../workflows/flake-bot.yaml` — trigger, context collection, agent run.
- `system-prompt.md` — the agent's full instructions.
- `linear.sh` — Linear GraphQL helper (search/get/create/comment/state).
