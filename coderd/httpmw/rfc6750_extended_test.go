...
func TestOAuth2BearerTokenSecurityBoundaries(t *testing.T) {
	...
	key1ID, key1Secret := randomAPIKeyParts()
	hashedSecret1 := sha256.Sum256([]byte(key1Secret))
	...
	key2ID, key2Secret := randomAPIKeyParts()
	hashedSecret2 := sha256.Sum256([]byte(key2Secret))
	_, err = db.InsertAPIKey(ctx, database.InsertAPIKeyParams{
		ID:              key2ID,
		UserID:          user2.ID,
	...
	t.Run("TokenIsolation", func(t *testing.T) {
		...
		req1.Header.Set("Authorization", "Bearer "+key1ID+"-"+key1Secret)
		...
		req2.Header.Set("Authorization", "Bearer "+key2ID+"-"+key2Secret)
	...
	t.Run("CrossTokenAttempts", func(t *testing.T) {
		...
		// Try to use user1's key ID with user2's secret (should fail)
		invalidToken := key1ID + "-" + key2Secret
...
func TestOAuth2BearerTokenPrecedence(t *testing.T) {
	...
	t.Run("CookieTakesPrecedenceOverBearer", func(t *testing.T) {
		...
		require.Equal(t, keyID, rec.Header().Get("X-Key-ID"))
	})
...
// TestOAuth2BearerTokenConcurrency tests Bearer token authentication under concurrent load
func TestOAuth2BearerTokenConcurrency(t *testing.T) {
	t.Parallel()
	...
	handler := middleware(http.HandlerFunc(func(rw http.ResponseWriter, r *http.Request) {
		apiKey := httpmw.APIKey(r)
		require.Equal(t, keyID, apiKey.ID)
		rw.WriteHeader(http.StatusOK)
	}))
	...
	t.Run("ConcurrentValidRequests", func(t *testing.T) {
		...
		for i := 0; i < numGoroutines; i++ {
			...
			go func(index int) {
				...
				req := httptest.NewRequest("GET", fmt.Sprintf("/test-%d", index), nil)
				req.Header.Set("Authorization", "Bearer "+validToken)
				rec := httptest.NewRecorder()
				handler.ServeHTTP(rec, req)

				if rec.Code != http.StatusOK {
					errors <- xerrors.Errorf("goroutine %d: expected 200, got %d", index, rec.Code)
				}
			}(i)
		}
	...
	t.Run("ConcurrentInvalidRequests", func(t *testing.T) {
		t.Parallel()
		...
		for i := 0; i < numGoroutines; i++ {
			...
			go func(index int) {
				...
				req := httptest.NewRequest("GET", fmt.Sprintf("/test-%d", index), nil)
				req.Header.Set("Authorization", fmt.Sprintf("Bearer invalid-token-%d", index))
				rec := httptest.NewRecorder()
				handler.ServeHTTP(rec, req)

				if rec.Code != http.StatusUnauthorized {
					errors <- xerrors.Errorf("goroutine %d: expected 401, got %d", index, rec.Code)
				}

				wwwAuth := rec.Header().Get("WWW-Authenticate")
				if !strings.Contains(wwwAuth, "Bearer") {
					errors <- xerrors.Errorf("goroutine %d: missing WWW-Authenticate header", index)
				}
			}(i)
		}
