...
func reapJob(ctx context.Context, log slog.Logger, db database.Store, pub pubsub.Pubsub, jobToReap *jobToReap) error {
	...
	err := db.InTx(func(db database.Store) error {
		...
		if job.Type == database.ProvisionerJobTypeWorkspaceBuild {
			...
			// the current build.
			currentState, err := db.GetWorkspaceBuildProvisionerStateByID(ctx, build.ID)
			if err != nil {
			...
			if len(currentState) == 0 {
				...
				if err == nil {
					prevState, err := db.GetWorkspaceBuildProvisionerStateByID(ctx, prevBuild.ID)
					if err != nil {
