...
The `allow` rule therefore carries a third conjunct, `ai_workspace_designation_allow`, beside `permission_allow` and `scope_allow`. It always holds for human and system subjects. For an AI subject it permits `read`, ID-less workspace `create`, and agent lifecycle actions (`create_agent`, `update_agent`, `delete_agent`) on workspace-typed objects. Agent lifecycle actions update agent rows and daemon state; exact workspace scope and API parent checks constrain their targets. Workspace-runtime actions require `input.object.ai_agent_id = acting_ai_agent_id`. Defining the exemptions rather than the protected actions means a newly added workspace action is protected by default until deliberately classified.
