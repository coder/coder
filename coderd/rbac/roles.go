...
func ReloadBuiltinRoles(opts *RoleOptions) {
	...
	builtInRoles = map[string]func(orgID uuid.UUID) Role{
		...
		// agentsAccess grants org members the chat permissions needed for
		// the end-user chat workflow, including sharing. ActionDelete is
		// intentionally excluded: no dbauthz function checks it on
		// ResourceChat. Hard-deletion goes through ResourceSystem
		// (dbpurge).
