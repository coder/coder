...
func (api *API) postChatUserMemory(rw http.ResponseWriter, r *http.Request) {
	...
	count, err := api.Database.CountChatUserMemoriesByUserAndOrganization(ctx, database.CountChatUserMemoriesByUserAndOrganizationParams{
		UserID:         apiKey.UserID,
		OrganizationID: req.OrganizationID,
	})
	if err != nil {
		httpapi.Write(ctx, rw, http.StatusInternalServerError, codersdk.Response{Message: "Failed to count chat user memories.", Detail: err.Error()})
		return
	}
	...
	defer commit()
	memory, err := api.Database.InsertChatUserMemory(ctx, database.InsertChatUserMemoryParams{
		ID:             uuid.NullUUID{},
		OrganizationID: req.OrganizationID,
		UserID:         apiKey.UserID,
