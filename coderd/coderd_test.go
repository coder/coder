...
func TestCSRFExempt(t *testing.T) {
	...
	t.Run("PathBasedApp", func(t *testing.T) {
		...
		// A StatusGone means Coderd tried to proxy to the agent and failed because the agent
		// was not there. This means CSRF did not block the app request, which is what we want.
		require.Equal(t, http.StatusGone, resp.StatusCode, "status code 500 is CSRF failure")
