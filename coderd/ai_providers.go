...
func (api *API) aiProvidersUpdate(rw http.ResponseWriter, r *http.Request) {
	...
	err := api.Database.InTx(func(tx database.Store) error {
		...
		if req.Settings != nil {
			if err := validateAIProviderExternalIDUnchanged(existing, *req.Settings); err != nil {
				return err
			}
		...
		if existing.ClaudePlatformAWS != nil && old.Type != database.AIProviderTypeAnthropic {
			return errAIProviderClaudePlatformTypeMismatch
		}
		// Generate the server-owned external ID when the provider assumes a role
		// and lacks one.
		ensureAIProviderExternalID(&existing)
		...
		if cp := existing.ClaudePlatformAWS; cp != nil {
			keyCount := 0
			if req.APIKeys != nil {
