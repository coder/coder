...
// proxyNamesByHost returns a map from normalized hostname to
...
func (api *API) proxyCollisionWarning(provider database.AIProvider, namesByHost map[string][]string) string {
	if !provider.Enabled || provider.Deleted || !api.DeploymentValues.AI.BridgeProxyConfig.Enabled.Value() {
		return ""
	}
...
func (api *API) aiProviderStatusFromDB(ctx context.Context, provider database.AIProvider) *codersdk.AIProviderStatus {
	...
	return api.aiProviderStatus(provider, api.proxyNamesByHost(rows))
}
