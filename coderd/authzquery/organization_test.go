...
func (s *MethodTestSuite) TestOrganization() {
	s.Run("GetGroupsByOrganizationID", s.Subtest(func(db database.Store, check *MethodCase) {
	...
	s.Run("GetOrganizationByID", s.Subtest(func(db database.Store, check *MethodCase) {
		o := dbgen.Organization(s.T(), db, database.Organization{})
		check.Args(o.ID).Asserts(o, rbac.ActionRead).Returns(o)
	}))
	s.Run("GetOrganizationByName", s.Subtest(func(db database.Store, check *MethodCase) {
		o := dbgen.Organization(s.T(), db, database.Organization{})
		check.Args(o.Name).Asserts(o, rbac.ActionRead).Returns(o)
	}))
	s.Run("GetOrganizationIDsByMemberIDs", s.Subtest(func(db database.Store, check *MethodCase) {
	...
	s.Run("GetOrganizationMemberByUserID", s.Subtest(func(db database.Store, check *MethodCase) {
	...
	s.Run("GetOrganizationMembershipsByUserID", s.Subtest(func(db database.Store, check *MethodCase) {
	...
	s.Run("GetOrganizations", s.Subtest(func(db database.Store, check *MethodCase) {
	...
	s.Run("GetOrganizationsByUserID", s.Subtest(func(db database.Store, check *MethodCase) {
		u := dbgen.User(s.T(), db, database.User{})
		a := dbgen.Organization(s.T(), db, database.Organization{})
	...
	s.Run("InsertOrganization", s.Subtest(func(db database.Store, check *MethodCase) {
		check.Args(database.InsertOrganizationParams{
			ID:   uuid.New(),
			Name: "random",
		}).Asserts(rbac.ResourceOrganization, rbac.ActionCreate)
	}))
	s.Run("InsertOrganizationMember", s.Subtest(func(db database.Store, check *MethodCase) {
		o := dbgen.Organization(s.T(), db, database.Organization{})
		u := dbgen.User(s.T(), db, database.User{})
	...
	s.Run("UpdateMemberRoles", s.Subtest(func(db database.Store, check *MethodCase) {
		o := dbgen.Organization(s.T(), db, database.Organization{})
		u := dbgen.User(s.T(), db, database.User{})
		mem := dbgen.OrganizationMember(s.T(), db, database.OrganizationMember{
			OrganizationID: o.ID,
