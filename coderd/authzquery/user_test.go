...
func (s *MethodTestSuite) TestUser() {
	s.Run("DeleteAPIKeysByUserID", s.Subtest(func(db database.Store, check *MethodCase) {
		u := dbgen.User(s.T(), db, database.User{})
		check.Args(u.ID).Asserts(rbac.ResourceAPIKey.WithOwner(u.ID.String()), rbac.ActionDelete).Returns()
	}))
	s.Run("GetQuotaAllowanceForUser", s.Subtest(func(db database.Store, check *MethodCase) {
		u := dbgen.User(s.T(), db, database.User{})
		check.Args(u.ID).Asserts(u, rbac.ActionRead).Returns(int64(0))
	}))
	s.Run("GetQuotaConsumedForUser", s.Subtest(func(db database.Store, check *MethodCase) {
		u := dbgen.User(s.T(), db, database.User{})
		check.Args(u.ID).Asserts(u, rbac.ActionRead).Returns(int64(0))
	}))
	s.Run("GetUserByEmailOrUsername", s.Subtest(func(db database.Store, check *MethodCase) {
		u := dbgen.User(s.T(), db, database.User{})
	...
	s.Run("GetUserByID", s.Subtest(func(db database.Store, check *MethodCase) {
		u := dbgen.User(s.T(), db, database.User{})
		check.Args(u.ID).Asserts(u, rbac.ActionRead).Returns(u)
	}))
	s.Run("GetAuthorizedUserCount", s.Subtest(func(db database.Store, check *MethodCase) {
	...
	s.Run("GetFilteredUserCount", s.Subtest(func(db database.Store, check *MethodCase) {
		_ = dbgen.User(s.T(), db, database.User{})
		check.Args(database.GetFilteredUserCountParams{}).Asserts().Returns(int64(1))
	}))
	s.Run("GetUsers", s.Subtest(func(db database.Store, check *MethodCase) {
	...
	s.Run("GetUsersWithCount", s.Subtest(func(db database.Store, check *MethodCase) {
		a := dbgen.User(s.T(), db, database.User{CreatedAt: database.Now().Add(-time.Hour)})
		b := dbgen.User(s.T(), db, database.User{CreatedAt: database.Now()})
		check.Args(database.GetUsersParams{}).Asserts(a, rbac.ActionRead, b, rbac.ActionRead)
	}))
	s.Run("GetUsersByIDs", s.Subtest(func(db database.Store, check *MethodCase) {
		a := dbgen.User(s.T(), db, database.User{CreatedAt: database.Now().Add(-time.Hour)})
		b := dbgen.User(s.T(), db, database.User{CreatedAt: database.Now()})
	...
	s.Run("InsertUser", s.Subtest(func(db database.Store, check *MethodCase) {
	...
	s.Run("InsertUserLink", s.Subtest(func(db database.Store, check *MethodCase) {
		u := dbgen.User(s.T(), db, database.User{})
	...
	s.Run("SoftDeleteUserByID", s.Subtest(func(db database.Store, check *MethodCase) {
		u := dbgen.User(s.T(), db, database.User{})
		check.Args(u.ID).Asserts(u, rbac.ActionDelete).Returns()
	}))
	s.Run("UpdateUserDeletedByID", s.Subtest(func(db database.Store, check *MethodCase) {
	...
	s.Run("UpdateUserHashedPassword", s.Subtest(func(db database.Store, check *MethodCase) {
		u := dbgen.User(s.T(), db, database.User{})
		check.Args(database.UpdateUserHashedPasswordParams{
			ID: u.ID,
		}).Asserts(u.UserDataRBACObject(), rbac.ActionUpdate).Returns()
	}))
	s.Run("UpdateUserLastSeenAt", s.Subtest(func(db database.Store, check *MethodCase) {
		u := dbgen.User(s.T(), db, database.User{})
	...
	s.Run("UpdateUserProfile", s.Subtest(func(db database.Store, check *MethodCase) {
		u := dbgen.User(s.T(), db, database.User{})
	...
	s.Run("UpdateUserStatus", s.Subtest(func(db database.Store, check *MethodCase) {
		u := dbgen.User(s.T(), db, database.User{})
	...
	s.Run("DeleteGitSSHKey", s.Subtest(func(db database.Store, check *MethodCase) {
		key := dbgen.GitSSHKey(s.T(), db, database.GitSSHKey{})
		check.Args(key.UserID).Asserts(key, rbac.ActionDelete).Returns()
	}))
	s.Run("GetGitSSHKey", s.Subtest(func(db database.Store, check *MethodCase) {
		key := dbgen.GitSSHKey(s.T(), db, database.GitSSHKey{})
		check.Args(key.UserID).Asserts(key, rbac.ActionRead).Returns(key)
	}))
	s.Run("InsertGitSSHKey", s.Subtest(func(db database.Store, check *MethodCase) {
		u := dbgen.User(s.T(), db, database.User{})
	...
	s.Run("UpdateGitSSHKey", s.Subtest(func(db database.Store, check *MethodCase) {
		key := dbgen.GitSSHKey(s.T(), db, database.GitSSHKey{})
	...
	s.Run("GetGitAuthLink", s.Subtest(func(db database.Store, check *MethodCase) {
	...
	s.Run("InsertGitAuthLink", s.Subtest(func(db database.Store, check *MethodCase) {
		u := dbgen.User(s.T(), db, database.User{})
	...
	s.Run("UpdateGitAuthLink", s.Subtest(func(db database.Store, check *MethodCase) {
		link := dbgen.GitAuthLink(s.T(), db, database.GitAuthLink{})
	...
	s.Run("UpdateUserLink", s.Subtest(func(db database.Store, check *MethodCase) {
	...
	s.Run("UpdateUserRoles", s.Subtest(func(db database.Store, check *MethodCase) {
