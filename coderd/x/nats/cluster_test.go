...
func TestCluster_TokenAutoGenerated(t *testing.T) {
	t.Parallel()
	logger := slogtest.Make(t, &slogtest.Options{IgnoreErrors: true}).Leveled(slog.LevelDebug)
	ctx, cancel := context.WithTimeout(context.Background(), testutil.WaitShort)
	defer cancel()
	// No ClusterToken supplied: New must succeed and a non-empty
	// ephemeral token must be recorded on the Pubsub.
	p, err := New(ctx, logger, Options{
		PeerProvider: StaticPeerProvider([]Peer{{RouteURL: "nats://127.0.0.1:6222"}}),
	})
	require.NoError(t, err)
	t.Cleanup(func() { _ = p.Close() })
	require.NotEmpty(t, p.effectiveClusterToken)
	require.Len(t, p.effectiveClusterToken, 64) // 32 bytes hex
}
...
func TestCluster_RoutePoolSizePinned(t *testing.T) {
	...
	got, _, err := buildServerOptions(Options{ClusterToken: "tok"}, peers)
	require.NoError(t, err)
	require.Equal(t, DefaultRoutePoolSize, got.Cluster.PoolSize)
	require.Equal(t, natsserver.RANDOM_PORT, got.Cluster.Port)

	// Override.
	got, _, err = buildServerOptions(Options{ClusterToken: "tok", RoutePoolSize: 7, ClusterPort: 12345}, peers)
	require.NoError(t, err)
...
func TestCluster_BuildOptions_ClientListener(t *testing.T) {
	...
	got, gotToken, err := buildServerOptions(Options{}, nil)
	require.NoError(t, err)
	require.False(t, got.DontListen)
	require.Equal(t, "127.0.0.1", got.Host)
	require.Equal(t, natsserver.RANDOM_PORT, got.Port)
	require.NotEmpty(t, gotToken) // ephemeral token generated
}
