...
func contextResourcesToPrompt(
	...
	for _, r := range resources {
		...
		case database.WorkspaceAgentContextBodyKindInstructionFile:
			...
			// Only the agent knows which .coder directory is ~/.coder; any
			// other .coder directory is a nested one like any other.
	...
	// Global files first, then root files before the nested files that
	// refine them; depth alone would put a working directory outside the
	// home directory ahead of ~/.coder.
