...
type Server struct {
	...
	// organizationNames caches organization ID to name for stage labels.
...
func (p *Server) PromoteQueued(
	...
	if !promotedQueuedAt.IsZero() {
		var chatKind, organization string
		if refreshedOK {
...
func (p *Server) recordQueueWait(ctx context.Context, chatID uuid.UUID, chatKind, organization string, queuedAt, promotedAt time.Time) {
...
// use as a stage label. Names are cached for the life of the server,
// so a renamed organization keeps its old label until restart. A
