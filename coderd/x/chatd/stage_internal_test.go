...
func TestNewModelTracesEachProviderAttempt(t *testing.T) {
	...
	stageModel := chatloop.StageModel{Model: "gpt-4", Effort: "low"}
...
func TestRunnerTurnSpanEndOfUnfinishedTurnDropsAccounting(t *testing.T) {
	...
	require.Empty(t, turnStageCounts(t, registry))
...
func turnOutcomeCounts(t *testing.T, registry *prometheus.Registry) map[string]float64 {
	...
	for _, family := range families {
		...
		for _, metric := range family.GetMetric() {
			...
			out[outcome+"/"+model] = metric.GetCounter().GetValue()
		}
	}
	return out
}
...
func TestRunnerTurnSpanCountsOutcomes(t *testing.T) {
	...
	// A completed turn is counted once its partition is emitted, with
	// the model its stages stamped.
	turnCtx, completed := turn.Ensure(t.Context(), chat, time.Now().Add(-time.Minute))
	_, stream := tracer.Start(turnCtx, chatloop.StageStream)
	stream.SetModel(chatloop.StageModel{Model: "claude"})
	stream.End(nil)
	...
	// Its model is empty when no stage resolved one.
	...
	require.Equal(t, map[string]float64{
		chatloop.TurnOutcomeCompleted + "/claude": 1,
		chatloop.TurnOutcomeCompleted + "/":       1,
