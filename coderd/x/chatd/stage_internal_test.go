...
func emittedTurns(t *testing.T, registry *prometheus.Registry) uint64 {
	...
	var total float64
	for _, family := range families {
		if family.GetName() != "coderd_chatd_turns_total" {
			continue
		}
		for _, metric := range family.GetMetric() {
...
func TestRunnerTurnSpanParentsRecordedStages(t *testing.T) {
	...
	// The step has ended, so the queue wait is recorded on the turn
	// context rather than the step context.
	tracer.Record(turnCtx, chatloop.StageQueueWait, chatloop.StageModel{},
...
func TestRunnerTurnSpanInvalidateAfterPromotionKeepsFinishedTurn(t *testing.T) {
	...
	// transition has committed, so the turn stays completed and its
	// accounting is emitted; the failure must not leak into the turn
	// the promotion opens either.
...
func TestRunnerTurnSpanClampsStaleAnchor(t *testing.T) {
	...
	// So is a trigger equal to it: the same prompt reopening the turn
	// after an invalidation must not record a second acquisition.
	_, reopened := turn.Ensure(t.Context(), chat, firstTrigger)
	require.NotEqual(t, second, reopened)
	require.Equal(t, float64(2), staleAnchors())
	turn.Complete(reopened, time.Time{})
	turn.Settle(t.Context(), reopened)

	// A trigger after the previous anchor is kept.
	thirdTrigger := time.Now()
	_, third := turn.Ensure(t.Context(), chat, thirdTrigger)
	require.NotEqual(t, reopened, third)
	require.Equal(t, float64(2), staleAnchors())
	turn.End(nil)
	...
	require.Equal(t, firstTrigger.UTC(), turns[2].StartTime().UTC())
...
func TestMCPConnectOutcome(t *testing.T) {
	...
	tests := []struct {
		name      string
		outcomes  []mcpclient.ConnectOutcome
		wantErr   bool
		connected int64
		failed    int64
	}{
		{name: "AllConnected", outcomes: []mcpclient.ConnectOutcome{mcpclient.ConnectOutcomeConnected, mcpclient.ConnectOutcomeNoTools}, connected: 2},
		{name: "PartialFailure", outcomes: []mcpclient.ConnectOutcome{mcpclient.ConnectOutcomeConnected, mcpclient.ConnectOutcomeTimeout}, connected: 1, failed: 1},
		{name: "AllFailed", outcomes: []mcpclient.ConnectOutcome{mcpclient.ConnectOutcomeTimeout, mcpclient.ConnectOutcomeError}, wantErr: true, failed: 2},
		{name: "NoServers"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			tracer, recorder := newStageTestTracer(t)
			summaries := make([]mcpclient.ConnectSummary, 0, len(tt.outcomes))
			for _, outcome := range tt.outcomes {
				summaries = append(summaries, mcpclient.ConnectSummary{Outcome: outcome})
			}

			_, span := tracer.Start(t.Context(), chatloop.StageMCPConnect)
			err := mcpConnectOutcome(span, summaries)
			span.End(err)
			if tt.wantErr {
				require.Error(t, err)
			} else {
			...
			ended := recorder.Ended()
			require.Len(t, ended, 1)
			require.Contains(t, ended[0].Attributes(), attribute.Int64(chatloop.AttrMCPConnected, tt.connected))
			require.Contains(t, ended[0].Attributes(), attribute.Int64(chatloop.AttrMCPFailed, tt.failed))
			if tt.wantErr {
				require.Equal(t, codes.Error, ended[0].Status().Code)
			} else {
				require.Equal(t, codes.Unset, ended[0].Status().Code)
