...
func (p *Server) createChildSubagentChatWithOptions(
	...
	// Review the child prompt before persisting the child so prompt policy
	// applies to spawned chats.
	...
	if p.hookDispatcher != nil && p.hookDispatcher.Enabled() {
		...
		if overridden {
			prompt = override
			// Recompute derived titles to avoid leaking the original prompt.
			if titleDerivedFromPrompt {
				title = subagentFallbackChatTitle(prompt)
			}
		}
