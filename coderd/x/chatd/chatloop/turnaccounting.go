...
func (t *StageTracer) RecordTurnOutcome(outcome, chatKind, organization string, model StageModel) {
	if t == nil || t.metrics == nil {
		return
	}
	t.metrics.RecordTurnOutcome(outcome, chatKind, organization, model.Model)
}
...
func (t *StageTracer) emitTurnAccounting(acc *TurnAccumulator, chatKind, organization string, turnDuration time.Duration) {
	...
	t.metrics.RecordTurnOutcome(TurnOutcomeCompleted, chatKind, organization, model.Model)
	if turnDuration <= 0 {
	...
	for stage, count := range snapshot.stageCounts {
		...
		t.metrics.RecordTurnStage(stage, chatKind, organization, model.Model,
	...
	t.metrics.RecordTurn(chatKind, organization, model.Model)
	for _, category := range TurnTimeCategories {
		elapsed := snapshot.categories[category]
		t.metrics.RecordTurnCategory(category, chatKind, organization, model.Model, elapsed, elapsed.Seconds()/turnSeconds)
	}
}
