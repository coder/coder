...
func TestPrepareMessagesForRequest_AnthropicPDFPreflightAllowsLargeContextPages(t *testing.T) {
	t.Parallel()
	...
	_, prompt, err := prepareMessagesForRequest(
		context.Background(),
		RunOptions{
			Model:                model,
			ContextLimitFallback: 200_001,
			Logger:               slogtest.Make(t, &slogtest.Options{IgnoreErrors: true}),
		},
		messages,
		model.Provider(),
...
func pdfPreflightTestPDF(pages int) []byte {
	var b strings.Builder
	_, _ = b.WriteString("%PDF-1.7\n")
	for range pages {
		_, _ = b.WriteString("1 0 obj << /Type /Page >> endobj\n")
	}
