...
func TestTurnAccountingPartition(t *testing.T) {
	...
	stageShares := fixture.sums(t, "coderd_chatd_stage_share_of_turn", "stage")
	for stage, seconds := range stages {
		require.InDelta(t, seconds/turnDuration.Seconds(), stageShares[stage], 0.001, stage)
	}
...
func TestTurnAccountingConcurrentStageEnds(t *testing.T) {
	...
	counts := fixture.sums(t, "coderd_chatd_turn_stage_count", "stage")
	require.Equal(t, float64(workers), counts[StageToolCall])
	require.Equal(t, float64(workers), counts[StageCommit])
	stages := fixture.sums(t, "coderd_chatd_turn_stage_seconds", "stage")
...
func TestTurnMetricBuckets(t *testing.T) {
	...
	metrics.RecordTurnStage(StageStream, ChatKindRoot, "acme", "", time.Minute, 3.5, 1200)
...
func TestTurnMetricsLevels(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(string(tt.level), func(t *testing.T) {
			...
			metrics.RecordTurnStage(StageStream, ChatKindRoot, "acme", "m", time.Minute, 0.5, 2)
			metrics.RecordTurn(ChatKindRoot, "acme", "m")
			metrics.RecordTurnOutcome(TurnOutcomeCompleted, ChatKindRoot, "acme", "m")
			metrics.RecordTurnOutcome(TurnOutcomeError, ChatKindRoot, "acme", "")
			metrics.RecordTurnCategory(CategoryStreaming, ChatKindRoot, "acme", "m", time.Minute, 0.5)
			metrics.RecordTurnCategory(CategoryScheduling, ChatKindRoot, "acme", "m", 0, 0)
