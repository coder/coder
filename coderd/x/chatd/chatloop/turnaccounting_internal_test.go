...
func TestTurnAccountingPartition(t *testing.T) {
	...
	require.Equal(t, map[string]string{"stage": StageStream, "chat_kind": ChatKindRoot}, labels)
	labels = fixture.labelsOf(t, "coderd_chatd_turn_time_seconds", "category", CategoryStreaming)
	require.Equal(t, map[string]string{"category": CategoryStreaming, "chat_kind": ChatKindRoot}, labels)
}
...
func TestTurnAccountingZeroElapsedStageIsReported(t *testing.T) {
	...
	for _, family := range families {
		...
		for _, metric := range family.GetMetric() {
			if metricLabel(metric, "stage") == StageCommit {
				found = true
