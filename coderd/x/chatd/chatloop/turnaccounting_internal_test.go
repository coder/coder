...
func newTurnFixture(t *testing.T) turnFixture {
	...
	tracer := NewStageTracer(provider, NewMetricsWithOptions(registry, MetricsOptions{StageMetrics: true}))
	tracer.clock = clock
...
func (f turnFixture) anomalies(t *testing.T) map[StageAnomaly]float64 {
	t.Helper()
	families, err := f.registry.Gather()
	require.NoError(t, err)
	out := map[StageAnomaly]float64{}
	for _, family := range families {
		if family.GetName() != "coderd_chatd_stage_anomalies_total" {
			continue
		}
		for _, metric := range family.GetMetric() {
			out[StageAnomaly(metricLabel(metric, "reason"))] = metric.GetCounter().GetValue()
		}
	}
	return out
}
