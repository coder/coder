...
func (p *Server) dispatchPreToolUse(
	...
	return p.hookDispatcher.DispatchWithID(ctx, event)
}
...
// dispatchRowResponse rebuilds persisted non-permission effects for replay.
...
// Keep the marker with the effects so a rollback cannot suppress replay.
...
// endChatAfterPromptDenial preserves accepted messages while archiving the chat.
// An already archived or deleted chat satisfies the instruction.
...
// endChatFromEditSessionStart archives before the edited prompt reaches history.
...
// endChatAfterToolHookFailure preserves an earlier accepted end_chat after a later failure.
