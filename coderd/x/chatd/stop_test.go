...
func TestStopHookNoOpFinishesTurn(t *testing.T) {
	...
	require.Equal(t, "ok", lifecycleDispatch(t, db, chat.ID, agenthooks.EventStop).Result)
}
...
func TestStopHookDispatchFailureErrorsChat(t *testing.T) {
	...
	dispatch := lifecycleDispatch(t, db, chat.ID, agenthooks.EventStop)
...
func TestStopHookEndChatArchives(t *testing.T) {
	t.Parallel()
	...
	user, org, model := seedChatDependenciesWithProvider(t, db, "openai-compat", openAIURL)
	consumer := stopConsumer(t, func() (int, string) {
		return http.StatusOK, `{"user_message":"chat ended","end_chat":true}`
	})
	server := newActiveTestServer(t, db, ps, func(cfg *chatd.Config) {
		cfg.AIBridgeTransportFactory = chatAIGatewayTransportFactoryPointer(chattest.NewMockAIBridgeTransport(t, openAIURL))
		cfg.HookDispatcher = newHookDispatcher(t, db, consumer)
	...
	chat, err := server.CreateChat(ctx, chatd.CreateOptions{
		...
		ModelConfigID:  model.ID,
		InitialUserContent: []codersdk.ChatMessagePart{
			codersdk.ChatMessageText("archive on stop"),
		},
	})
	require.NoError(t, err)
