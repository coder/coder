...
// TestLazyInstructionDiscoveryDiscardsProbeAfterRebind checks that a
// probe answered by the agent the tools reached is dropped when the chat is
// rebound to another agent before the answer is applied: the rebind cleared
// the rows for the new workspace, and the old agent's files must not land
// on it.
func TestLazyInstructionDiscoveryDiscardsProbeAfterRebind(t *testing.T) {
	t.Parallel()
	...
	user, org, model := seedChatDependenciesWithProvider(t, db, "openai-compat", openAIURL)
	ws, dbAgent := seedWorkspaceWithAgent(t, db, user.ID)
	//nolint:gocritic // Seeding agent context and rebinding as the chatd subject.
	chatdCtx := dbauthz.AsChatd(ctx)
	seedAgentInstructionContext(chatdCtx, t, db, dbAgent.ID, discoveryRootSource, "root rules")
	...
	mockConn.EXPECT().ReadFileLines(gomock.Any(), discoveryTouchedFile, int64(1), int64(0), gomock.Any()).
		Return(workspacesdk.ReadFileLinesResponse{Success: true, FileSize: 4, TotalLines: 1, LinesRead: 1, Content: "data"}, nil)
	var (
		chatID   atomic.Pointer[uuid.UUID]
		newAgent atomic.Pointer[database.WorkspaceAgent]
	)
	mockConn.EXPECT().ResolveContextInstructions(gomock.Any(), workspacesdk.ResolveContextInstructionsRequest{
		Directories: []string{discoveryNestedDir, discoveryNestedDir + "/src"},
	}).DoAndReturn(func(context.Context, workspacesdk.ResolveContextInstructionsRequest) (workspacesdk.ResolveContextInstructionsResponse, error) {
		// The chat is rebound to a rebuilt workspace's agent while the
		// probe is in flight; the rebind clears the pinned rows.
		current, err := db.GetChatByID(chatdCtx, *chatID.Load())
		require.NoError(t, err)
		agent := dbgen.WorkspaceAgent(t, db, database.WorkspaceAgent{ResourceID: dbAgent.ResourceID, Directory: dbAgent.Directory, OperatingSystem: "linux"})
		...
		require.NoError(t, err)
		require.NoError(t, db.DeleteChatContextResourcesByChatID(chatdCtx, current.ID))
		return instructionFileResponse(discoveryNestedDir, discoveryNestedSource, "site rules"), nil
	})
	...
	server := newActiveTestServer(t, db, ps, func(cfg *chatd.Config) {
		cfg.AIBridgeTransportFactory = chatAIGatewayTransportFactoryPointer(chattest.NewMockAIBridgeTransport(t, openAIURL))
		cfg.AgentConn = func(_ context.Context, agentID uuid.UUID) (workspacesdk.AgentConn, func(), error) {
			if current := newAgent.Load(); current == nil || agentID != current.ID {
				require.Equal(t, dbAgent.ID, agentID)
			}
			return mockConn, func() {}, nil
		}
	...
	chat, err := server.CreateChat(ctx, chatd.CreateOptions{
		...
		WorkspaceID:    uuid.NullUUID{UUID: ws.ID, Valid: true},
		AgentID:        uuid.NullUUID{UUID: dbAgent.ID, Valid: true},
		Title:          "lazy-instruction-discovery-rebound",
		ModelConfigID:  model.ID,
		InitialUserContent: []codersdk.ChatMessagePart{
			codersdk.ChatMessageText("read the app"),
		},
	...
	require.NotContains(t, pinnedBySource(ctx, t, db, chat.ID), discoveryNestedSource, "the previous agent's file is not pinned onto the rebound chat")
}
