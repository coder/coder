...
func TestPreToolUseHookEndChatShortCircuitsGeneratedStep(t *testing.T) {
	t.Parallel()
	...
	openAIURL := chattest.NewOpenAI(t, func(req *chattest.OpenAIRequest) chattest.OpenAIResponse {
		...
		first := chattest.OpenAIToolCallChunk("my_dynamic_tool", `{"arg":"a"}`)
		first.Choices[0].ToolCalls[0].ID = "call_end_first"
		second := chattest.OpenAIToolCallChunk("my_dynamic_tool", `{"arg":"b"}`).Choices[0].ToolCalls[0]
		second.ID = "call_fail_second"
		second.Index = 1
	...
	consumer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if data.ToolUseID != "call_end_first" {
	...
	chat, err := server.CreateChat(ctx, chatd.CreateOptions{
		OrganizationID: org.ID,
		OwnerID:        user.ID,
		Title:          "pre-tool-use-end-chat-generated",
		ModelConfigID:  model.ID,
		InitialUserContent: []codersdk.ChatMessagePart{
			codersdk.ChatMessageText("stop before the tools"),
		},
		DynamicTools: dynamicToolJSON(t, "my_dynamic_tool"),
	})
	require.NoError(t, err)
	...
	require.Equal(t, int32(1), hookCalls.Load(), "the second call must not dispatch after end_chat")

	rows, err := db.ListChatHookDispatchesByChatID(ctx, chat.ID)
	require.NoError(t, err)
	for _, row := range rows {
		if row.Event == string(agenthooks.EventPreToolUse) {
			require.Equal(t, "call_end_first", row.ToolUseID.String)
		}
	}
}
