...
type ExecuteArgs struct {
	...
	ModelIntent     *string `json:"model_intent,omitempty" description:"A short, natural-language, present-participle phrase describing why you are running this command. This is shown to the user as a status label while the command runs. Use plain English with no underscores or technical jargon. Do not include the command, duration, or words like \"using\" or \"for\". Keep it under 100 characters. Good examples: \"Running the unit tests\", \"Checking repository state\", \"Inspecting build output\"."`
