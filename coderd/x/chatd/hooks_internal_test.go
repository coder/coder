...
func TestNarrowHookAllowedToolsResponses(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name:    "Intersection",
			current: pqtype.NullRawMessage{RawMessage: []byte(`["read_file","spawn_agent","execute"]`), Valid: true},
			responses: []agenthooks.Response{
				{AllowedTools: &[]string{"read_file", "spawn_agent"}},
				{},
				{AllowedTools: &[]string{"read_file", "execute"}},
			},
			want:      `["read_file"]`,
			wantValid: true,
			wantUsed:  true,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, used, err := narrowHookAllowedToolsResponses(tt.current, tt.responses)
			require.NoError(t, err)
			require.Equal(t, tt.wantUsed, used)
			require.Equal(t, tt.wantValid, got.Valid)
			if tt.wantValid {
...
func TestPendingHookAllowedToolsContextChatScope(t *testing.T) {
	t.Parallel()

	chatID := uuid.New()
	policy := pqtype.NullRawMessage{RawMessage: []byte(`[]`), Valid: true}
	ctx := withPendingHookAllowedTools(t.Context(), chatID, policy)

	got, ok := pendingHookAllowedToolsFromContext(ctx, chatID)
	require.True(t, ok)
	require.Equal(t, policy, got)
	_, ok = pendingHookAllowedToolsFromContext(ctx, uuid.New())
	require.False(t, ok)
}
