...
func TestPostToolUseHookEndChatOrderingWithFailure(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name:       "failure before end_chat fails closed",
			failID:     "call_first",
			endChatID:  "call_second",
			wantErrors: true,
		},
		{
			name:      "failure after end_chat archives",
			endChatID: "call_first",
			failID:    "call_second",
			wantEnded: true,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErrors {
				failed := waitForChatStatus(ctx, t, db, chat.ID, database.ChatStatusError)
				require.False(t, failed.Archived)
				require.Contains(t, chatLastErrorMessage(failed.LastError), "hook dispatch failed: post_tool_use: http_error")
			}
			if tt.wantEnded {
				...
				testutil.Eventually(ctx, t, func(ctx context.Context) bool {
					ended, err = db.GetChatByID(ctx, chat.ID)
					return err == nil && ended.Archived && ended.Status == database.ChatStatusWaiting
				}, testutil.IntervalFast)
				require.False(t, ended.LastError.Valid)
			}
			...
			require.Equal(t, map[string]string{
				tt.failID:    "http_error",
				tt.endChatID: "ok",
			}, results, "both executed results must have dispatch rows")
		})
	}
}
