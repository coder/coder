...
// received. The manifest carries agent_plugins_enabled from the
// agent-plugins experiment, so with it off the agent never discovers the
// plugin and only the plain skill reaches the chat.
func TestChatContextPluginSkillsFromAgentPush(t *testing.T) {
	...
	runPluginTurn := func(t *testing.T, enabled bool) turnResult {
		t.Helper()
		...
		// With plugins enabled the agent's first resolve already includes
		// the plugin, so waiting for every expected source is sufficient.
		wantPushed := []string{plainSkillSource}
		if enabled {
			wantPushed = append(wantPushed, pluginSource, pluginSkillSource)
		}
	...
	t.Run("ExperimentOn", func(t *testing.T) {
		t.Parallel()

		res := runPluginTurn(t, true)
	...
	t.Run("ExperimentOff", func(t *testing.T) {
		...
		res := runPluginTurn(t, false)

		require.Len(t, res.pinned, 1, "only the plain workspace skill is pinned with the experiment off")
		require.Equal(t, database.WorkspaceAgentContextBodyKindSkill, res.pinned[res.plainSkillSource].BodyKind)
		require.NotContains(t, res.pinned, res.pluginSource)
		require.NotContains(t, res.pinned, res.pluginSkillSource)

		require.Equal(t, plainSkillName, res.listed[res.plainSkillSource].SkillName)
		require.NotContains(t, res.listed, res.pluginSource)
		require.NotContains(t, res.listed, res.pluginSkillSource)

		require.Contains(t, res.systemPrompt, "<available-skills>")
		require.Contains(t, res.systemPrompt,
			"- "+plainSkillName+": "+plainSkillDescription+"\n",
			"plain workspace skill should still be indexed")
		require.NotContains(t, res.systemPrompt, pluginSkillDescription,
			"plugin skill should not reach the prompt with the experiment off")
		require.NotContains(t, res.systemPrompt, "(plugin: "+pluginName+")")
	})
}
