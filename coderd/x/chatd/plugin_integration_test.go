...
// received. The manifest carries plugins_supported only when the
// agent-plugins experiment is enabled, so with it off the agent never
// discovers the plugin and only the plain skill reaches the chat.
