...
func buildSpawnAgentDescription(
	...
	description := "Assign a clearly scoped, independent task to a child agent. " +
		...
		" The child cannot spawn its own subagents. Its tools depend on its " +
		"mode and current access; do not assume they match yours. " +
		...
		"Track the assignments you start and account for unfinished work with " +
		"the lifecycle tools available to you; report a missing capability " +
		"rather than claiming work stopped or finished. An error status can be " +
		"recoverable; address its cause before retrying."
	if currentChat.PlanMode.Valid && currentChat.PlanMode.ChatPlanMode == database.ChatPlanModePlan {
