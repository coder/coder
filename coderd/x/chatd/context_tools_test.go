...
// withoutExperiment returns the known experiments minus experiment.
func withoutExperiment(experiment codersdk.Experiment) codersdk.Experiments {
	return slices.DeleteFunc(slices.Clone(codersdk.ExperimentsKnown), func(e codersdk.Experiment) bool {
		return e == experiment
	})
}

// offeredRootChatToolNames runs one turn of a root chat under the
// given experiments and returns the tool names in the first model
// request.
func offeredRootChatToolNames(t *testing.T, experiments codersdk.Experiments) []string {
	t.Helper()
	...
	require.NotEmpty(t, toolNames, "the model request carries the root chat tools")
	return toolNames
}
...
func TestActiveServer_ClearContextTool(t *testing.T) {
	...
	t.Run("offered only with the chat-context-tools experiment", func(t *testing.T) {
		t.Parallel()

		t.Run("enabled", func(t *testing.T) {
			t.Parallel()
			require.Contains(t, offeredRootChatToolNames(t, codersdk.ExperimentsKnown), "clear_context")
		})
		t.Run("disabled", func(t *testing.T) {
			t.Parallel()
			require.NotContains(t, offeredRootChatToolNames(t, withoutExperiment(codersdk.ExperimentChatContextTools)), "clear_context")
		})
	})
...
func TestActiveServer_CompactContextTool(t *testing.T) {
	...
	t.Run("offered only with the chat-context-tools experiment", func(t *testing.T) {
		...
		t.Run("enabled", func(t *testing.T) {
			t.Parallel()
			require.Contains(t, offeredRootChatToolNames(t, codersdk.ExperimentsKnown), "compact_context")
		})
		t.Run("disabled", func(t *testing.T) {
			t.Parallel()
			require.NotContains(t, offeredRootChatToolNames(t, withoutExperiment(codersdk.ExperimentChatContextTools)), "compact_context")
		})
	})
