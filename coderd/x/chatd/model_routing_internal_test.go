...
func TestAIBridgeProviderFormatMapping(t *testing.T) {
	...
	// provider types ignore the model parameter entirely. AIGOV-532.
	tests := []struct {
		name         string
...
func TestAIGatewayRequestFormatForProviderType(t *testing.T) {
	...
	// ignore the model parameter. AIGOV-532.
	tests := []struct {
		name         string
		providerType database.AIProviderType
		model        string
...
// models, so Chat Completions would fail at the bridge. AIGOV-532.
