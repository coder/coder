...
func TestLockLocksChatRow(t *testing.T) {
	...
	updateWG.Go(func() {
		_, updateErr = updateMachine.Update(ctx, func(_ *chatstate.Tx, _ database.Store, _ database.Chat) error {
			close(updateEntered)
			return nil
		})
	})
...
func TestConcurrentUpdatesSerializeOnChatRow(t *testing.T) {
	...
	for i := 0; i < updates; i++ {
		...
		go func() {
			defer wg.Done()
			m := chatstate.NewChatMachine(f.DB, f.Pub, created.Chat.ID)
			_, errs[i] = m.Update(ctx, func(_ *chatstate.Tx, _ database.Store, _ database.Chat) error { return nil })
		}()
	}
	wg.Wait()
