...
type Tx struct {
	...
	store  database.Store
	chatID uuid.UUID
	// publisher is the surrounding Update's publish buffer. Transitions
	// that cascade to other chats (EndChat archiving children) pass it
	// to nested machines so child publications flush with the outer
	// transaction.
	publisher Publisher
}
