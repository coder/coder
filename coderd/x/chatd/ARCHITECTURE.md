...
Deployments without remaining licensed agent runtime hours cap how many chats generate at once. The cap is enforced at worker ownership acquisition through the `AgentCapacityLimiter` seam (`agentadmission.go`): before a worker writes ownership, it asks the limiter whether the chat may run, and the limiter also reports the caps it enforces. A noop limiter (AGPL builds) admits everything and reports no caps; the enterprise implementation (`enterprise/coderd/x/chatd`) enforces two deployment-wide pools, 5 root chats and 10 subagent chats. Every chat that generates through chatd counts, including chats attached to a workspace, because their model generation runs server-side all the same. Workspace agents in the workspace sense (the agent process providing SSH and apps) are not chats and are outside this limit.

A chat holds a capacity slot while it is unarchived, in `running` or `interrupting` status, owned, and its owning runner has a fresh `chat_heartbeats` row. Counting live runners from the database means a crashed replica's slots free once its heartbeats go stale (the production staleness window, minutes), and slots free at the status transition out of `running`/`interrupting`, before the asynchronous abandon clears ownership. `requires_action` chats keep their runner but hold no slot; when the action completes they resume on the runner they already own, without re-admission, which can transiently exceed the cap. The overage is bounded: the action deadline is five minutes, and a resumed chat immediately counts as active, so new admissions refuse until the pool drains back below the cap. The same bounded softness applies to license expiry: in-flight chats finish their turns. Resumes from idle states (`waiting`, `error`) get no such bypass: those transitions clear ownership even when a not-yet-abandoned owner is still attached, so a message arriving in the abandon gap routes through acquisition and re-admits under the cap instead of restarting generation on the retained runner.

Refused chats stay `running` but unowned: no runner, no parked goroutine, and no database marker. Queued state is derived, not stored: a chat is queued for capacity when it is `running`, unarchived, has no live owner heartbeat, and its pool is at capacity. Pool fullness is what distinguishes a capacity wait from the ordinary sub-second wait for a worker pickup. The single-chat GET derives the flag through `Server.ChatQueuedForCapacity`, which asks the limiter for the current limits and runs `GetChatQueuedForCapacity`; list responses skip the derivation and always report false. Because a refusal must not republish an ownership hint (every worker would wake into an immediate re-refusal), the refused acquisition transaction rolls back.

Admission decisions are made per pool: root chats (`parent_chat_id IS NULL`) and subagent chats draw from separate pools, so a parent blocked in `wait_agent` keeps its root slot without starving its children, and there is no yield machinery. Subagents cannot nest, so the subagent pool cannot deadlock on itself. Interrupting chats always admit, otherwise an over-cap user could not stop their own chat. Counting and claim serialize across replicas with the `LockIDChatCapacityAdmission` advisory lock held to the end of the acquisition transaction; the count excludes the candidate itself so stale-ownership takeovers are capacity-neutral. The lock is only taken on capped deployments: the entitlement check short-circuits before it.

Acquisition candidates order by `updated_at`, so admission is approximately FIFO per pool, and each acquisition pass excludes already-attempted chats so refused candidates cannot hide the rest of the queue behind a full batch. The approximation has a known edge: sending a message to a capacity-queued chat bumps `updated_at` and re-queues it at the back of the line. Each worker also keeps an in-memory map of chats it refused (first refusal time included). The map drives `capacity_change` watch events, published once on queue entry and once on admission so open chat pages toggle the banner without a reload, and the `agent_capacity_wait_seconds` histogram observed at admission. Queue entry validates and publishes from one fresh snapshot, because a candidate row can go stale when another replica acquires the chat after the batch query; a newly owned chat never publishes, and a concurrent acquisition's queued event carries the pre-acquisition `updated_at`, which is older than the owner's clear. Pool fullness is not re-counted at queue entry; the refusal moments earlier proved it, and a capacity release inside that window is corrected by the next pass's admission and clear. Because the replica that admits a chat may not be the replica that refused it, and the cap can change between refusal and admission (license updates), every acquisition of a running or interrupting chat publishes the clear whenever a limiter is configured, without requiring local refusal history. It is a per-replica approximation, not shared state; entries are pruned when a complete candidate scan no longer returns the chat, and an all-skipped pass reconciles against one listing of capacity-wait candidates, pruning departed entries and queueing arrivals in proven-full pools, so a sustained full-pool backlog neither starves the prune nor hides the queued event of a chat sorted behind the backlog. A lost `capacity_change` event leaves a stale banner until the next single-chat refetch. Workers also periodically report database-derived gauges per pool, `coderd_chatd_agents_active` and `coderd_chatd_agents_queued_for_capacity`; every replica reports the same deployment-wide values, so aggregate them with max, not sum.

When a runner observes its chat leaving the counted statuses, it publishes a bare ownership hint so all workers immediately re-run acquisition and admit queued chats; the acquisition ticker (default 1s) is the fallback for missed nudges, entitlement changes, and heartbeat-staleness releases.
