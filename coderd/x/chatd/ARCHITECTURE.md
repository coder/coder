...
<!-- TODO(human): this section predates the search_tsv backfill changes. The deployed set_chat_message_revision_before() and update_chat_history_after_message_update() functions exclude search_tsv and (as of migration 000574) search_tsv_config from their change comparisons so the dbpurge search backfill sweep does not advance revisions or history_version. Update the SQL listings and prose to match. -->
