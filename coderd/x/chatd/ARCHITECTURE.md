...
TODO: document that `resolveModelCall` coerces `provider_options.openai.reasoning_summary` to `auto` for Bedrock models on the OpenAI wire format (Mantle rejects `concise` and `detailed` with HTTP 400 and returns no summaries for `auto`), that only the derived provider options see the coerced value while the resolved call config keeps the configured one for provider substitution, and that Anthropic models on Bedrock and non-Bedrock providers are unaffected.
