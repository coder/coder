...
Deployments without remaining licensed agent runtime hours limit concurrent generation to 5 root chats and 10 subagent chats. Before ownership acquisition, the enterprise limiter serializes the heartbeat-backed deployment-wide slot count and ownership claim in one database transaction; interrupt requests bypass admission, and missing runtime-hour usage values fail open. A slot remains occupied while complete ownership has a fresh matching heartbeat, regardless of status or archive state, and the two pools remain independent. Refused chats stay running and unowned; the single-chat API derives `queued_for_capacity` from pool fullness, and the open-chat query polls it every five seconds. Workers consider a bounded, pool-interleaved candidate pass in best-effort FIFO order by `updated_at`, with periodic acquisition covering heartbeat expiry and entitlement changes.
