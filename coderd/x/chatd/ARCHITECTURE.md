...
Trailing user messages the assistant has not answered yet (the *pending-user segment*) are not summarized: they are excluded from the summarizer's input and re-committed after the triplet as uncompressed model-only user rows, so the pruned prompt keeps them verbatim instead of relying on summary fidelity. The segment is derived from persisted rows before provider-switch sanitization. Replay rows carry no usage and cannot re-trigger compaction.
