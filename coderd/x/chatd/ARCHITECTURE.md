...
The immediate pass is best-effort, so a periodic per-replica sweep guarantees convergence: every minute it claims `cancel_requested` rows with process identity whose `updated_at` is older than the retry age (the claim bumps `updated_at` inside `FOR UPDATE SKIP LOCKED`, acting as a cross-replica lease) and re-runs the kill flow until a terminal outcome is recorded. An interrupted process is therefore eventually killed even when its agent was unreachable at interrupt time or the server died mid-reconciliation.
