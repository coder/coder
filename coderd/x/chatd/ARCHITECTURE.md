...
- TODO(CODAGT-928): document the tool-batch billing methods `StartToolBatch`, `ToolBatchStartedAt`, `RecordToolCompletion`, and `ToolCompletions`, the local-tool counterparts of `StartModelInvocation`/`ModelInvokedAt`, and `CloseEpisodeForBilling`, which the interrupt task uses to close the episode and snapshot its billing stamps in one atomic step so it can bill an interrupted attempt's partial window.
