...
The consumer can observe activity, add model-only or user-visible context, replace supported prompt or tool input, and deny prompts or tool calls. Prompt submission is evaluated once when the submission is accepted, including queued messages and subagent prompts. The prompt-submission event carries every caller-controlled instruction channel admitted with the turn (the per-chat system prompt at create and subagent spawn, dynamic tool definitions at create, and the owner's stored custom prompt on every admission), so a policy is never asked to admit a turn it cannot fully see. Subagent types that embed the submitted prompt into the child's system prompt (computer-use) expose the built value on the event, and a prompt override rebuilds the child system prompt from the approved prompt so replaced text cannot survive at system priority.

The admitted custom prompt is snapshotted per turn: a create or edit stamps the chat row (`chats.admitted_custom_prompt`), a direct send stamps it in the same transaction that inserts the message, and a queued send stores the snapshot on the queued row (`chat_queued_messages.admitted_custom_prompt`), which every promotion path copies to the chat row in the transaction that moves that message into history. Generation injects the chat-row snapshot instead of re-reading the live per-user config while hooks are enabled, so each turn generates with the value its own admission event was shown: the owner rewriting the stored prompt between admission and generation (queued sends stretch that window arbitrarily, and another replica may generate the turn) cannot put instructions in front of the model that no policy admitted, and a later admission cannot retroactively change an earlier queued turn. A turn without a recorded admission injects no custom prompt while hooks are enabled, and a turn admitted while hooks are disabled stamps a NULL snapshot rather than inheriting an earlier admitted turn's value, so toggling hooks back on cannot replay stale admitted instructions. Returned context becomes part of the conversation for its intended audience, except that context returned before a compaction guides the compaction summary instead.

Both protocol directions authenticate with the shared secret. Requests carry a JWT binding the body, event type, chat, and dispatch ID; responses steer chats, so every 2xx response must echo those claims in a `Coder-Hook-Signature` token bound to the exact response bytes, and the dispatcher rejects unsigned or mismatched responses as protocol errors.

Lifecycle hooks fail closed. If the consumer cannot be reached or returns an invalid response, Coder stops the triggering operation rather than continuing without the consumer's decision. Affected chats can enter an error state until the consumer recovers or hooks are disabled. Tool paths that dispatch hooks inside tool execution (subagent spawn and messaging) fail their turn closed as well, with dispatch failures redacted to event, class, and dispatch ID before they can persist into client-visible transcripts.
