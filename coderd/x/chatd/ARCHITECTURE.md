...
TODO: document that `FinishTurn` from `R1` and `FinishInterruption` from `I1` check the queue head: a held head is not promoted and the chat lands in `P` (`paused`, queue intact) instead of `R0`/`R1`.
