...
By default the summary is generated with the chat model. Admins can override the compaction model per organization via the `compaction` context of the chat model override API (`/api/experimental/organizations/{organization}/chats/config/model-override/{context}`, stored in the org-suffixed `agents_chat_compaction_model_override:<org-uuid>` site config). The same per-org scoping applies to the other override contexts (`general`, `explore`, `title_generation`, and `advisor`, the last stored under `agents_advisor_model_override:<org-uuid>`), and personal model overrides are keyed per user and organization in `user_configs` (`chat_personal_model_override:<org-uuid>:{context}`). Override resolution is keyed by the chat's organization, and a stored reference naming another organization's config resolves exactly like one naming a missing row. The override affects only the summary call; thresholds, compressed-message storage, and the post-compaction assistant generation keep using the chat model.
