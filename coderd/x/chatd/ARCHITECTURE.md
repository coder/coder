...
- Promotion: a promoting transition (`FinishTurn`, `FinishInterruption`, a message sent to an errored chat, or `PromoteQueued`) returns the promoted message's queued `created_at`, and its caller records a standalone `queue_wait` stage from that instant to the promotion. The promoted message's `created_at` is the promotion time, so the next `Ensure` opens the promoted turn there and records its `acquisition` from it.
...
The `provider` attribute on `prepare`, `generation_step`, `thinking`, and `tool_call` is the model's wire protocol (`Model.Provider()`).
