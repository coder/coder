...
- `propose_mcp_server` proposes creating a new **personal** MCP server. It posts a confirmation card containing the description and review actions to the bound Slack thread, and persists the description and setup instructions with the proposed configuration in an `mcp_server_proposals` row. Setup instructions are shown only on the dashboard review page; nothing is created until the requesting user accepts the proposal there.
