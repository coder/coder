...
File links are metadata, but they carry one transition-time invariant: a transition that persists message content referencing uploaded files (chat create, message send, queued send, message edit) records the file links in the same transaction, and exceeding the per-chat attachment cap rejects the whole transition. File retention treats links as the file's lifecycle owner, so a persisted message must never reference a file without a link.
