...
- TODO(CODAGT-928): document the tool-batch billing methods `StartToolBatch`, `ToolBatchStartedAt`, `RecordToolCompletion`, and `ToolCompletionsAt`, the local-tool counterparts of `StartModelInvocation`/`ModelInvokedAt` that the interrupt task reads before `CloseEpisode` to bill an interrupted tool batch's partial window.
