...
- TODO(CODAGT-928): document the tool-batch billing methods `StartToolBatch`, `ToolBatchStartedAt`, `RecordToolCompletion`, and `ToolCompletions`, the local-tool counterparts of `StartModelInvocation`/`ModelInvokedAt`, and `CloseEpisodeForBilling`, which the interrupt task uses to close the episode and snapshot its billing stamps (including the stable first-close instant used as the interrupt instant) in one atomic step; repeat calls return the identical snapshot and push the eviction deadline out, so retried interrupt tasks bill the same window and keep their snapshot through a database outage.
