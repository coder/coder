...
TODO: document that the `1`/`0` queue dimension is "the queue head exists and is not held" (`chatstate.QueueState.HasPromotableHead`, read by `chatstate.LoadQueueState`), not raw row count. At most one queued message per chat carries `held_at` (while its owner edits it; holding another moves the hold); the held row and every row behind it are invisible to the classifier, so a chat whose head is held classifies as the `0` state even though `chat_queued_messages` has rows. Rows ahead of the held row still drain in order. Describe the effect on `W`: `waiting` with a held head is valid and idle; `waiting` with a promotable head remains invalid.
...
TODO: document `EditQueuedMessage(qid, content?, held?)`: rewrites a queued row's content and/or sets or clears its hold in one transition; setting the hold on a row releases the chat's previous hold in the same transition. Document that `DeleteQueuedMessage`, `PromoteQueuedMessage`, and `EditQueuedMessage` are admitted from `W`, `E0`, `R0`, `I0`, and `A0` because a held head can hide queued rows there, and that `SendMessage` from `R0`/`I0`/`A0` with a held head queues behind it and stays in the `0` variant (and from `E1` with a held row behind the head lands in `R0`). Document that `ChatMachine.Update` rejects any callback whose result classifies as Invalid (`ErrInvalidResultState`), which is what stops a release or delete from leaving `W` with a promotable head; the server composes `PromoteQueuedMessage` for those cases (release of the idle head is content write then promote; delete of the idle held head is promote successor then delete). From `E0` a release lands in `E1` without clearing the error. `PromoteQueuedMessage` clears the target's hold before reordering, so "send now" on a held row releases it and on a row behind a held row goes around the hold. Sends on an idle chat (`W`, `E0`) still insert into history directly; `busy_behavior=interrupt` still appends at the tail and waits behind a held head.
...
TODO: document that `FinishInterruption` and `FinishTurn` promote only a promotable (unheld) head; with a held head they land in `waiting` and leave the queue untouched, which is what pauses the queue at an edited row without the runner spinning.
...
TODO: update the `DELETE` and `promote` sections above for holds: both endpoints also accept `W`, `E0`, `R0`, `I0`, and `A0` (the `0` states can physically hold queued rows when the head is held), an empty queue is a `404` rather than a `409`, `DELETE` is now owner-only like `promote` because deleting the held head of an idle chat starts the row behind it (the server promotes the successor, then deletes, in one transaction). Add a section for `PATCH /api/experimental/chats/{chat}/queue/{queuedMessage}` (`EditQueuedMessage`): owner-only; request `content?`, `model_config_id?`, `reasoning_effort?`, `held?` (at least one required, `content: []` rejected); content passes the same input validation, `user_prompt_submit` hook, model and effort checks, and file linking as a send; `204` response, outcome arrives through the stream; releasing the held head of an idle chat is served as content write then `PromoteQueuedMessage`; moving the hold off the held head of an idle chat is a `409`; state mappings per the `EditQueuedMessage` row of the matrix in `transition.go`. Note the client flow: `held:true` before editing (moves any existing hold), `{content, held:false}` to save, `{held:false}` to cancel, `404` on a row that was already promoted.
...
TODO: note that `held_at` changes are client-visible queue updates: the update trigger lists `held_at`, and the `queue_update` snapshot carries `held_at` so every client can render the held row and the paused rows behind it.
