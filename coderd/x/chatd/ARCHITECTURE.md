...
Model configs may also set `openai_config.reasoning_model` to override whether the OpenAI client treats the model as a reasoning model: true sends reasoning effort and summary while dropping temperature and top_p, and false forces plain sampling. Unset keeps fantasy's model-name heuristics. The override is applied once at client construction via fantasy's `WithReasoningModelFunc` provider option. The setting is visible for OpenAI and Bedrock providers because Bedrock also serves OpenAI-format models; it is ignored for Anthropic models on Bedrock.

TODO: document that `chatopenai.UsesResponsesAPI` owns the unset-override decision for both the client (`WithResponsesAPIFunc`) and `TransportFor`.
