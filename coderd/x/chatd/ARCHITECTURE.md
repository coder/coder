...
When a turn finishes normally, its stages are rolled up and emitted once, labelled with the turn's `chat_kind`, `organization_name`, and `model`. The model is the first one resolved in the turn. Turns that end with an error or an interruption are invalidated and emit nothing.
