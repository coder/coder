...
After the interrupt commit, the worker best-effort reconciles the `cancel_requested` rows: it dials the recorded agent with a short timeout and sends a kill signal. A confirmed outcome (the agent definitively has no such process, or a post-kill snapshot shows exit) resolves the row to `canceled`. A delivered but unconfirmed kill records `cancel_signal_sent_at` and leaves the row `cancel_requested` with its full process identity. Rows without a recorded process identity are polled through the record grace window (an interrupted in-flight `StartProcess` can still land its handle via the uncanceled record write); a handle that arrives gets the kill flow, and only after the window closes does the row resolve to `unknown`. The interrupt commit maps background rows to `detached` only when their handle is recorded; a background row whose start was still in flight becomes `cancel_requested`, and the reconciler resolves it to `detached` without signaling once the handle lands, because the interrupt spares background processes. This runs after the commit, so a slow or unreachable agent never delays the interrupt.

The immediate pass is best-effort, so a periodic per-replica sweep guarantees convergence: every minute it claims `cancel_requested` rows whose `updated_at` is older than the retry age (the claim bumps `updated_at` inside `FOR UPDATE SKIP LOCKED`, acting as a cross-replica lease). Rows with process identity re-run the kill flow until a terminal outcome is recorded; rows without it (a crash between the interrupt commit and reconciliation) get the same late-handle wait as the immediate pass and then resolve to `unknown`, so no row is stranded `cancel_requested` forever. An interrupted process is therefore eventually killed even when its agent was unreachable at interrupt time or the server died mid-reconciliation.
