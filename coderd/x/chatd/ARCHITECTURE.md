...
The histogram observes `chat_turn`, `queue_wait`, `capacity_wait`, `acquisition`, `mcp_connect`, `stream`, `time_to_first_token`, `provider_attempt`, `tool_call`, `commit`, and `retry_backoff`. `generation_step`, `prepare`, `thinking`, and `compaction` are span-only unless the `chat-stage-metrics-full` experiment is enabled, which observes every stage on a 16-edge ladder, adds `thinking` and `compaction` to the model histogram, and registers the per-turn distribution families described under [Turn accounting](#turn-accounting).
...
The turn's wall time is partitioned into disjoint categories that sum to the turn duration. `coderd_chatd_turn_time_seconds_total` accumulates seconds per category and `coderd_chatd_turns_total` counts the accounted turns, so their ratio is mean seconds per turn by category. Every category is emitted for every accounted turn, including the ones with no time, so every category has a series. With the `chat-stage-metrics-full` experiment, `coderd_chatd_turn_time_seconds` and `coderd_chatd_turn_time_share` additionally record the per-turn distribution of each category's seconds and share, and `coderd_chatd_turn_stage_seconds` records the total seconds the turn spent in each stage; stages overlap, so those totals do not partition the turn. Per-turn occurrence counts are a trace question: the `chat_turn` span's children carry them. The categories and the stages that feed them:
