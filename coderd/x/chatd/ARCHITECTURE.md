...
TODO: document the paused execution state `P`: `chats.status = paused`, not archived, non-empty queue for which a pause condition holds (`chatstate.StateP`, classified from `chatstate.QueueState{HasRows, Paused}` read by `chatstate.LoadQueueState`; `Paused` is `chatstate.queuePaused(head)`, the single place pause mechanisms register their condition; today: the head has `held_at`). `paused` is a first-class status (re-added to the `chat_status` enum by 000592; 000543 had removed the unused legacy value) so every reader of `status` distinguishes it from idle without the queue. At most one queued message per chat carries `held_at` (set while its owner edits it; holding another moves the hold). A hold changes no other state: busy states classify by row count as before. `waiting` with rows remains invalid, as does `paused` without a pause condition. `P` is not runnable; a send to `P` queues at the tail. Status consumers that treat `paused` like `waiting`: the runner (abandon task), `interruptSubagent` (no turn to interrupt), and the end-of-turn label, summary, and push (`turnFinished`). SQL: `paused` is in the active-status lists of `MarkChatsContextDirtyByAgent` and `GetActiveChatsByAgentID` and excluded from both auto-archive queries; `GetStaleChats` never returns it.
...
TODO: document `EditQueuedMessage(qid, content?, held?)`: rewrites a queued row's content and/or sets or clears its hold in one transition; setting the hold on a row releases the chat's previous hold in the same transition. Admitted from `E1`, `R1`, `I1`, `A1` (same state) and from `P`. From `P`: releasing the head promotes it and the chat runs (`R0`/`R1`); a hold-move off the head is refused (`ErrPausedHeadMustResume`). `DeleteQueuedMessage` from `P`: deleting the held head promotes the row behind it (`R0`/`R1`) or idles the chat (`W`). `PromoteQueuedMessage` clears the target's hold before reordering and from `P` pops the target into history like `E1`/`A1`; "send now" on a row behind a held row goes around the hold. `SendMessage` from `P` queues (`P`); from `E1` with a held head it queues without promoting (`E1`). Not admitted from `P`: `SetArchived`, `ClearContext`, `RequestCompaction`, `Interrupt`, `FinishTurn`, `FinishError`.
...
TODO: document that `FinishTurn` from `R1` and `FinishInterruption` from `I1` check the queue head with `queuePaused`: a paused head is not promoted and the chat lands in `P` (`paused`, queue intact) instead of `R0`/`R1`. `P` is left through `resumeIfUnpaused`, which promotes the head once no pause condition remains, or lands in `W` when no rows remain.
...
TODO: update the `DELETE` and `promote` sections above for holds: both endpoints also accept `P`; `DELETE` is now owner-only like `promote` because deleting the held head of a paused chat starts the row behind it. Add a section for `PATCH /api/experimental/chats/{chat}/queue/{queuedMessage}` (`EditQueuedMessage`): owner-only; request `content?`, `model_config_id?`, `reasoning_effort?`, `held?` (at least one required, `content: []` rejected); content passes the same input validation, `user_prompt_submit` hook, model and effort checks, and file linking as a send; `204` response, outcome arrives through the stream; releasing the head of a paused chat sends it; holding another row while paused is a `409`; state mappings per the `EditQueuedMessage` cells in `transition.go`. Note the client flow: `held:true` before editing (moves any existing hold), `{content, held:false}` to save, `{held:false}` to cancel, `404` on a row that was already promoted. Clients see `status: "paused"` on the chat and `held_at` on the head row; there is no derived paused state.
