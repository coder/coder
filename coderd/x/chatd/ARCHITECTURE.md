...
A turn that resumes replays the tool calls history still shows as unresolved (`unresolvedToolCallsFromHistory`). Interrupts and edits do not produce those: both commit synthetic cancellation results for outstanding calls, which marks them handled and suppresses replay. What remains is the crash window: a worker that dies or an attempt that is retried between running a tool and committing its result, including a lost response. For the `execute` tool a naive replay runs the shell command again, which is unsafe for non-idempotent commands.

To deduplicate replays, chatd derives an idempotency key per execute dispatch: the assistant message ID that issued the call, then the provider tool call ID, joined by `-`. Both inputs are durable, so a replay of the same persisted call derives the same key. Provider tool call IDs alone can repeat across regenerations, but history edits soft-delete and re-insert messages, so a regenerated assistant message gets a new row ID and therefore new keys. The chat is not part of the key because the agent already knows which chat sent the request and scopes reservations to it. The local tool dispatcher attaches this identity to every call it runs, so any tool that must run at most once per persisted call can derive a key from it; `execute` is the only one that does today, and it sends the key as `idempotency_key` on the agent's `StartProcess` request.

The workspace agent holds these keys in a general run-once reservation registry, which is deliberately not specific to processes: it owns the reservation rule (reserve before working, one ticket per key, later callers with the same input attach to the published value, retention after completion) while each operation keeps its own notion of what a published value is. Process starts are its only adapter today, and the process manager still owns output, exit state, signals, and git-watcher registration. Reservations are keyed by chat and idempotency key together, so two chats that send the same key value get independent reservations rather than colliding.

A start whose key already named a process attaches to it instead of spawning a duplicate. Retention starts when the process exits, and runs for 60 minutes, so a replay arriving after the command finished still receives the recorded result; a command still running after that window is never reaped. Reusing a key within one chat with different parameters (command, requested workdir, env, or background flag) is refused with the `input_mismatch` conflict code, and the tool returns an error result without starting anything. When the start holding a key has not published its process yet, the conflict is `start_pending` and therefore unresolved rather than permanent: the tool retries while its budget lasts, and only reports an unresolved outcome if the budget runs out first. Both codes travel in the response body, so callers never classify conflicts by matching error text.

Attached foreground starts also return the process's original start time. The replay waits only the remaining execution budget (`started_at + timeout - now`) instead of a fresh full timeout, to the extent the agent and coderd clocks agree; an agent clock running ahead can grant a replay more of the budget than has actually elapsed. An attached process that already exited reports its real result even past the deadline; one still running past the deadline returns the standard timed-out result carrying the `background_process_id` handle.
