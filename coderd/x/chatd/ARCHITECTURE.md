...
For Bedrock models using the OpenAI wire format, `resolveModelCall` coerces any set `provider_options.openai.reasoning_summary` value to `auto` before building per-call provider options. Mantle rejects `concise` and `detailed` with HTTP 400 and only accepts `auto`, currently without returning summaries. Coercion keeps the stored config portable between Bedrock and direct OpenAI and allows summaries to flow if AWS adds support later. Anthropic models on Bedrock and non-Bedrock providers are unchanged.
