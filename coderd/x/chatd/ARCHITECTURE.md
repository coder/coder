...
# Gateway interception links

Chatd sends every model call through the in-process AI Gateway, which records one `aibridge_interceptions` row per request. Chatd stores the ID of the interception that produced an assistant message in `chat_messages.aibridge_interception_id`, so the message joins to its interception and, through it, to the `provider_response_id` on `aibridge_token_usages`, `aibridge_user_prompts`, and `aibridge_tool_usages`.

The ID reaches chatd through a trusted in-process context callback, never through HTTP headers. Before an assistant step or a compaction summary call, the [generation goroutine](#generation-goroutine) attaches the callback with `aibridge.WithInterceptionRecordedHook`. The gateway calls it synchronously after it records the interception and before it contacts the upstream provider, so an attempt interrupted mid-stream already knows its interception. The callback stores the ID on the attempt's episode in the [Message part buffer](#message-part-buffer) with `RecordInterception`. If one attempt makes several gateway requests, such as a compaction summary regenerated without tool definitions, the last ID wins because that request produced the persisted output.

Chatd stamps the episode's ID on the assistant message of a committed step, on the visible `chat_summarized` tool-call message of a compaction step, and on every assistant message of an interrupted partial suffix. User, tool, and system messages keep the column NULL, as do messages written before it existed. Title generation and turn summaries run outside generation attempts, so their interceptions are not linked to any message. The column has no foreign key because the gateway retains interceptions independently of chat history, and the chat API does not expose it.
...
- `RecordInterception(chat_id, history_version, generation_attempt, interception_id)`: stores the AI Gateway interception ID for the episode, replacing any earlier one. Returns a predefined error if the episode is not found or already closed. See [Gateway interception links](#gateway-interception-links).
- `InterceptionID(chat_id, history_version, generation_attempt)`: returns the ID stored by `RecordInterception`, or none when the episode is unknown or never recorded one. Like `ModelInvokedAt`, it must be read before `CloseEpisode`. The interrupt goroutine reads it just before closing the episode and stamps it on the assistant messages of the interrupted suffix.
