...
- `EpisodeModelInvocation(chat_id, history_version, generation_attempt)`: `StartModelInvocation` stamps the instant the episode opens its provider stream, and `ModelInvocationDuration` returns the span between that stamp and `CloseEpisode`. The duration is zero when the episode is unknown, never opened a provider stream (such as a local tool execution batch), or is still open. The interrupt goroutine uses it as the interrupted attempt's billable runtime.
