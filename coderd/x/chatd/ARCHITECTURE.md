...
- `CommitStep(step)` inserts one durable message suffix while remaining `running`. A committed model, local-tool, or compaction attempt atomically inserts one `chat_execution_steps` row and associates every generated and hook-appended message in the suffix with it.
...
- `FinishInterruption(optionalPartialStep)` inserts one final interrupted assistant/tool suffix if present, or finalizes interruption without a suffix if none is available, clears the interrupting state, and lands in `waiting` if no queued message is promoted. A durable interrupted suffix and its execution-step runtime are committed atomically. If interrupt finalization also promotes the queue head, it inserts the promoted queued message into history without associating it with the interrupted step and lands in `running`.
...
- `SetOperation(chat_id, history_version, generation_attempt, operation)` and `Operation(...)`: record and read whether the episode is a model, local-tool-batch, or compaction attempt. The interrupt goroutine reads this metadata instead of inferring the operation from buffered content.
...
- `ModelInvokedAt(chat_id, history_version, generation_attempt)`: returns the instant stamped by `StartModelInvocation`, or the zero time when the episode is unknown or never opened a provider stream. It must be read before `CloseEpisode`, because closed episodes are garbage collected and reading afterwards races the cleanup loop. The interrupt goroutine reads it just before closing the episode and uses the span between that instant and the interrupt when model-generated assistant content becomes durable.
- `RecordToolStart(chat_id, history_version, generation_attempt, call_index, started_at)` and `RecordToolCompletion(chat_id, history_version, generation_attempt, call_index, completed_at)`: stamp per-occurrence local-tool intervals by unresolved-call position. Dispatch indexes are translated back through the generation path's `allowedIndexes`, so denied gaps and duplicate call IDs cannot move runtime to a different occurrence. Queued and undispatched calls are absent because neither has started, and the buffer lock serializes concurrent completion stamps.
- `ToolCompletions(chat_id, history_version, generation_attempt)`: returns a copy of the started tool intervals. Completed calls end at their completion stamp, still-running calls end at the interrupt cutoff, and unstarted calls contribute no interval. Interrupt handling reads the intervals before closing the episode.
...
Billable runtime belongs to `chat_execution_steps`, not to an individual message. A row is identified by the pre-commit episode key `(chat_id, history_version, generation_attempt)` and stores the operation (`model`, `local_tool_batch`, or `compaction`), outcome (`completed` or `interrupted`), aggregate `runtime_ms`, and billing timestamp. Every generated message from that attempt shares the row's `execution_step_id`; separately promoted user messages and hook-only commits have no execution step. The nullable `chat_messages.runtime_ms` column remains inert only so older replicas can complete their compiled inserts during a rolling upgrade.

Completed model and compaction steps use their provider-call runtime. Completed local-tool steps store the union of billed tool intervals, so parallel intervals count once and idle gaps are excluded. Interrupted model runtime is persisted only when model-generated assistant content became durable. Interrupted local-tool runtime uses the same interval-union rule, ending running calls at one captured interrupt cutoff. Attempts that durably produce messages but have no billable interval still create a zero-runtime grouping row; failed attempts that persist no messages create no row.

The execution row and all associated messages are inserted in the same chat-state transaction. If message insertion fails, the row rolls back. Hourly `hb_agent_runtime_v1` usage sums execution rows by `recorded_at`, including rows whose messages were later soft-deleted. Telemetry only includes a step while at least one associated message remains visible to its non-deleted message summary. Hard chat deletion cascades message deletion and sets `chat_execution_steps.chat_id` to `NULL`, retaining only de-identified runtime and episode metadata for billing and audit totals.
...
2. It reads the episode operation, model invocation stamp, and tool intervals, then captures one interrupt cutoff.
3. It closes the episode and reads its buffered parts. The cutoff captured before closure ends the interrupted model window, every still-running billed tool interval, and cancellation-part timestamps.
4. It converts durable partial content and unresolved tool calls to messages. Completed billed calls end at their own completion stamps, running billed calls end at the cutoff, and unstarted, rejected, or unbilled calls contribute no interval; overlapping intervals are reduced to their union.
5. It applies `FinishInterruption(partial?)` with one execution-step input. Partial and cancellation messages share the resulting step ID, while a separately promoted queue head remains unassociated. If no interruption messages become durable, it creates no step. If no operation metadata survives, buffer-only partials remain unassociated rather than guessing whether they came from model generation or compaction. If committed unresolved local calls still require cancellation messages, their durable state identifies a `local_tool_batch` step with zero reconstructed runtime.
