...
- A fresh `starting` claim is polled until its owner records a process or the claim goes stale. A stale claim is resolved through the agent's token index (the execution ID doubles as the agent-side idempotency token): a found process is adopted onto the stale claim's epoch and re-attached; a token the agent does not know, answered within the trust window, permits taking over the claim and re-dispatching under the same token, which the agent dedups against any race with the original dispatch; anything else (old agents without the probe route, answers past the trust window, unobservable outcomes) resolves to `unknown`. Recovery never mints a new token for a tool call. The unknown write matches only rows still `starting`: the owner's process record can land concurrently with the staleness verdict, and that write wins, with the attempt resuming the recorded process instead.
