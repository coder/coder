...
TODO: document the `disabled_workspace_mcp_servers` field. It replaces the chat's deny-list of workspace `.mcp.json` server names, emits no state transition, and takes effect on the next turn's workspace MCP tool set.

TODO: document the `mcp_server_ids` field. It is the canonical write path for the chat's Coder-managed MCP server selection and calls `Server.UpdateChatMCPServerIDs`, which applies the same explore-snapshot and Force On rules as `applyRequestedMCPServerIDs` on send. The `mcp_server_ids` fields on message send and edit are deprecated.
