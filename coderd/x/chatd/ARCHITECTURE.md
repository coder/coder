...
- `RequestCompaction` records a manual compaction request on an idle or errored chat by setting `compaction_requested_at` and landing in `running` without inserting any message. It clears `last_error` per the leave-error rule and resets `generation_attempt` so the compaction turn gets a fresh retry budget. The chat worker picks the chat up like any other running chat and consumes the request. See [Manual compaction](#manual-compaction).
