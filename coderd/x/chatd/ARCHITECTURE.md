...
The consumer can observe activity, add model-only or user-visible context, replace supported prompt or tool input, and deny prompts or tool calls. Prompt submission is evaluated once when the submission is accepted, including queued messages and subagent prompts. The prompt-submission event carries every caller-controlled instruction channel admitted with the turn (the per-chat system prompt and dynamic tool definitions at create, and the owner's stored custom prompt on every admission), so a policy is never asked to admit a turn it cannot fully see. The admitted custom prompt is persisted on the chat row (`chats.admitted_custom_prompt`) in the same transaction as the admitted send, and generation injects that snapshot instead of re-reading the live per-user config while hooks are enabled; the owner rewriting the stored prompt between admission and generation (queued sends stretch that window arbitrarily, and another replica may generate the turn) therefore cannot put instructions in front of the model that no policy admitted. A chat without a recorded admission injects no custom prompt while hooks are enabled. Returned context becomes part of the conversation for its intended audience, except that context returned before a compaction guides the compaction summary instead.
