...
- Invalidate: `Invalidate` records an outcome and an error for the open turn. The first call is kept, and a call after `Complete` is ignored: the finishing transition has committed, so a later failure in the same step does not change the outcome. A step whose `FinishError` transition commits, including a step that fails closed, records `error` with the failure. A step that exits with an expected, non-retryable error (a fence or history mismatch) while its task context is live records `abandoned`. Every other failure on a live context is retried by the task runner with the same trigger, so the retry's `Ensure` continues the same turn. A step whose task context is done records nothing: a timed-out attempt is retried as the same turn, and a canceled task's turn is closed by the interrupt, a newer prompt, or runner exit.
- Interrupt: the interrupt task confirms the chat is `interrupting`, then invalidates the open turn as `interrupted` with the error `chat interrupted` before `FinishInterruption` commits, so a queued prompt the commit promotes cannot close it first. After the commit it settles the turn, which closes it even when no generation task holds it, for example when the interrupt lands between two steps. `interrupted` therefore means the user stopped the turn, by an interrupt or by promoting a queued message on a running chat.
...
- Runner exit: the runner ends whatever span is still open when it exits, as `abandoned` unless the turn finished or was invalidated. This covers server shutdown and loss of ownership, whose canceled tasks record no outcome. When a replica hands a chat off during shutdown, the time between that close and the next owner's pickup is in no turn.

When the span closes by any of these paths it carries exactly one `turn_outcome` attribute: the outcome `Invalidate` recorded when there is one; otherwise `completed` for a turn `Complete` marked finished, and `abandoned` for a turn closed before it finished. An invalidated turn's span ends with the invalidation error, so the trace root reports error status; this includes `interrupted`. A turn closed as `abandoned` by a newer prompt or by runner exit was never invalidated, and its status stays unset.

The runner cancels the active task and spawns its replacement without waiting for the old goroutine to exit (see [Event processing](#event-processing)), so an old task can still be unwinding while the new one calls `Ensure` and rotates the turn. Each task carries the `turnToken` returned by its own `Ensure` call, and `Complete` and `Settle` do nothing when the token does not identify the open turn. A stale task therefore cannot close the turn that replaced its own. A task canceled before its `Ensure` call gets the zero token, which no turn matches, so it cannot pick up the token of a turn its replacement opened, and a task canceled after `Ensure` records no outcome. A message edit is a newer prompt: the replacement task's `Ensure` closes the edited turn as `abandoned`. The new task, or the interrupt task settling an interrupted turn, can close the old turn before the old task's `generation_step` ends, though: that step's span then outlives its `chat_turn` parent by the few milliseconds the old task takes to unwind.
