...
Every chatd model call goes through the in-process AI Gateway, which records each request as a row in `aibridge_interceptions`. Chatd saves that row's ID on the assistant messages the request produced, in `chat_messages.aibridge_interception_id`. This ties a message to the exact gateway request behind it and, through that request, to the provider's response ID on the gateway's usage rows (`aibridge_token_usages`, `aibridge_user_prompts`, and `aibridge_tool_usages`). Without the link, gateway records could only be matched to a chat, not to a message.

The gateway hands the ID to chatd in-process, through a callback in the request context, as soon as it has recorded the request and before it calls the provider. Chatd keeps the ID with the current generation attempt in the [Message part buffer](#message-part-buffer), so the ID is known even if the user interrupts the response mid-stream. If one attempt sends several requests, as when a compaction summary is retried without tool definitions, the last ID wins because that request produced the saved output.

Only assistant messages carry the ID: ordinary steps, the visible `chat_summarized` tool call that records a compaction, and partial messages saved on interrupt. User, tool, and system messages leave it NULL, as do messages saved before the column existed. Title generation and turn summaries also go through the gateway, but they produce no chat messages, so nothing links to their requests. The column has no foreign key because gateway records and chat messages are retained independently, and the chat API does not expose it.
