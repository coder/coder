...
Every stage carries `scope` (`turn` or `background`), `chat_kind` (`root` or `subagent`), `organization_name` (the chat's organization, resolved once per turn and cached for the life of the server), and once the model is resolved, `model`. Stages recorded without a known chat carry empty `chat_kind` and `organization_name` labels. Spans additionally carry `reasoning_effort`; it is not a metric label because it multiplies series per model. Stages that are not tied to a model call (`acquisition`, `queue_wait`, `capacity_wait`, `mcp_connect`, `retry_backoff`, `commit`) carry an empty `model` label. `prepare` is stamped with the model once preparation resolves it.
...
When a turn finishes normally, its stages are rolled up and emitted once, labelled with the turn's `chat_kind`, `organization_name`, and `model`. The model is the first one resolved in the turn. Turns that end interrupted, in error, or abandoned are invalidated and emit nothing.

Every closed turn is counted exactly once in `coderd_chatd_turn_outcomes_total{outcome, chat_kind, organization_name, model}`, registered at every level except `off`, so the outcomes sum to the closed turns. `completed` is counted when a finished turn's accounting is emitted, so it equals `coderd_chatd_turns_total` except for the turns counted under `nonpositive_turn`; `interrupted`, `error`, and `abandoned` are counted when the turn closes, with the same value as the span's `turn_outcome` attribute. The `model` label is empty when the turn ended before a stage resolved one.
