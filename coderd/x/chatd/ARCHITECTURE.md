...
An out-of-band write (even of a hidden or soft-deleted row) allocates no `snapshot_version` and publishes no notification. What it does to the chat row depends on the last transition. A transition leaves `history_version` behind `snapshot_version` unless it writes history or is `RequestCompaction`, which sets `history_version` to its own `snapshot_version` without writing history so that the compaction turn gets the fresh `generation_attempt` and retry state a history change would grant. After any other transition the triggers set `history_version` to the current `snapshot_version` without a matching bump. Every runner goroutine carries the `history_version` it started from as its fence and exits when the chat's value differs, checked whenever it loads the chat, including inside each transition it applies, so such a write ends the goroutine; the chat runner then treats the changed `history_version` as new state, see [Chat runner](#chat-runner). After a transition that left `history_version` equal to `snapshot_version`, the chat row does not change: the stream does not learn of the write until it next loads the full history, on reconnect or when a later transition deletes messages, and a generation goroutine sees it only when it reloads history for its next step.

Three trigger functions implement the mechanism. Their definitions are in `coderd/database/dump.sql`.

`set_chat_message_revision_before` runs `BEFORE INSERT` and `BEFORE UPDATE` on each `chat_messages` row. It raises an exception for rows that assign `revision` or change `chat_id`, ignores updates that change only the search columns (`search_tsv`, `search_tsv_config`), and otherwise sets the row's `revision` to the chat's current `snapshot_version`.

`update_chat_history_after_message_insert` and `update_chat_history_after_message_update` run `AFTER INSERT` and `AFTER UPDATE` once per statement over the statement's transition tables. For every chat with an inserted row, or a row changed in a column other than the search columns, they set `history_version = snapshot_version` and `generation_attempt = 0`, but only where `history_version` differs from `snapshot_version` or `generation_attempt` is non-zero. Inside a transition the first message write always meets that condition, because the transition allocated a new `snapshot_version` first; later writes in the same transition find `history_version` already equal to `snapshot_version` and `generation_attempt` already zero.
...
- Every receiver tracks the highest `snapshot_version` it has processed per chat and discards notifications below that watermark. The stream loop also discards notifications at the watermark. The chat runner applies its rule to every event it receives, database sync rows included: an event at the watermark is new when its `history_version`, status, or archived state differs, because a direct `chat_messages` write can change `history_version` without allocating a snapshot (see [Message revisions and history version](#message-revisions-and-history-version)). Only a sync row can carry such a change, since every notification is published from a transition that allocated a new `snapshot_version`, and the runner must restart the goroutine that the write ended (see [Chat runner](#chat-runner)).
...
- latest processed event's `Archived`;
...
The runner receives events from three sources: its bootstrap read of the chat row, pubsub notifications, and the database sync loop, which sends the current row of every registered chat on a fixed interval. Pubsub notifications can be dropped. A goroutine can also exit without any event being published, for example when its history fence fails. In both cases the next event the runner receives is the database sync's copy of the current row, and that row may be identical to an event the runner has already processed. The runner must therefore act on every event, including one it has already processed: if the latest known state requires work and no goroutine is performing it, the runner spawns that goroutine. Recovery from a lost notification or an exited goroutine takes at most one sync interval.

The snapshot version does not identify a chat state on its own: a direct write to `chat_messages` can change `history_version` without allocating a snapshot (see [Message revisions and history version](#message-revisions-and-history-version)), so two events with the same `SnapshotVersion` can carry different `HistoryVersion` values. An event is newer than the latest processed event if its `SnapshotVersion` is greater, or if the `SnapshotVersion` is equal and the `HistoryVersion`, `Status`, or `Archived` differs.
...
1. Iterate through the goroutine list and remove all goroutines that have finished, the active one included.
2. If the event is not newer than the runner's latest processed event, ignore the event and skip to step 6.
...
6. If there is no active goroutine and the latest processed event says the runner still owns the chat, spawn the goroutine that event needs and mark it as active:
    ...
    - If `Status` is `requires_action` (`A0` or `A1`), spawn a goroutine to wait for the dynamic tool timeout to pass. We call this the **dynamic tools timeout goroutine**.
    - Otherwise, spawn an **abandon chat goroutine**.
