...
- Message edit is the exception: it soft-deletes the issuing turn, so it commits no synthetic results and nothing can carry a background handle back to the user. It therefore maps the last assistant message's outstanding executions with background sparing disabled (every dispatched row becomes `cancel_requested` for the sweep to kill) and stamps `result_committed_at`, and this mapping MUST run before the soft-deletes: once the assistant message is deleted the outstanding-call scan cannot see it, and mapping after the delete would orphan the live process behind an uncommitted row no sweep can claim.
