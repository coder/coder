...
- The interrupt commit maps unresolved calls' rows: background to `detached`, `reserved` to `canceled`, foreground `starting`/`running`/`detached` to `cancel_requested`. A foreground row detached by a timed-out wait is reopened because its result, the only carrier of the process handle, never committed; leaving it detached would strand a running process behind a handle-less synthetic cancellation. Their synthetic cancellation results commit in the same transaction, which also sets `result_committed_at`. The mapping runs before the synthetic results are built and each result derives from its mapped row, so the spare-as-detached decision and the handle in the committed result are one observation: a handle landing concurrently either arrives before the mapping (the row maps `detached` and its result carries the handle) or after it (the row is already `cancel_requested` and the sweep kills the now-identified process).
- Every other transition that resolves outstanding local tool calls with synthetic cancellation results (a new user message during a run, queued-message promotion, requires-action cancellation and completion, invalid-state reconciliation) runs the same ledger mapping and `result_committed_at` stamp in its own committing transaction. These transitions run no immediate kill pass; rows they map to `cancel_requested` are converged by the periodic execution sweep. Spared background rows keep their handle in the committed synthetic result, exactly as at interrupt. Abandon and archive deliberately do not map: the turn's calls stay re-attachable.
- Message edit is the exception: it soft-deletes every message from the edited one onward, so the deleted turn's synthetic results are never part of surviving history and nothing can carry a background handle back to the user. (EditMessage still runs the synthesize pass after the deletes for any calls left outstanding; it finds none by construction because the deleted assistant message is invisible to the scan, and it runs with sparing disabled regardless.) The edit therefore maps two kinds of ledger rows before the soft-deletes, and this ordering is REQUIRED because after the delete the outstanding-call scan cannot see the assistant message and mapping late would orphan live processes behind rows no sweep can claim. First, the last assistant message's outstanding executions are mapped with background sparing disabled (every dispatched row becomes `cancel_requested` for the sweep to kill) and `result_committed_at` is stamped. Second, every process-bearing row (`running` or `detached`) anchored at or after the edited message (background starts, timed-out foregrounds, interrupt-spared backgrounds, and rows whose best-effort detach write failed and stayed `running`) is also routed to `cancel_requested`: the committed results that carried those rows' process handles are deleted with the suffix, and a dispatched process is spared only while a committed result carries its handle. This mapping re-stamps `result_committed_at` so the sweep's give-up bound starts at the edit, giving the kill its full retry budget instead of resolving a long-detached row `unknown` with zero kill attempts.

### Claim and re-attach

Before dispatching, the execute tool claims the row: a compare-and-set that takes a `reserved` intent or a stale `starting` claim (one whose `claimed_at` is older than the staleness window), advancing `claim_epoch`. A row missing entirely (an assistant message that predates the ledger) is claimed by insert. A claim that fails on infrastructure (not the input-hash guard) aborts the call instead of committing an error result: nothing was dispatched, so the task retry re-claims the intent. An abort fails the attempt with a retryable error and never commits a result for the aborted call, but completed sibling results from the same batch are committed first: their side effects already happened, so the retry re-runs only the calls left without a result. The claim records the dispatch target (`workspace_agent_id`) before the dispatch happens, so recovery can tell whether a later token probe reaches the agent that actually received it. The claimer calls `StartProcess` and records the process identity, guarded by `claim_epoch` so a superseded claimer never overwrites the current claim's process. A `StartProcess` failure with a structured agent response (other than the token conflict) resolves the row `no_effect` and commits the error: the agent released the token reservation, so nothing is running. A transport failure or token conflict instead aborts the attempt uncommitted with the row still `starting`, so the retried call resolves the dispatch through the agent's token index; recoverable stale-claim failures (a transient probe error, or a found process whose adoption write did not land) abort the same way. Lifecycle observations (`exited`, `detached`) are recorded only when that identity write landed: a terminal row without a process handle would strand a retry, while a still-`starting` row resolves through claim recovery.

An attempt that cannot claim decides from the row's status:

- A fresh `starting` claim is polled until its owner records a process or the claim goes stale. A stale claim is resolved through the agent's token index (the execution ID doubles as the agent-side idempotency token), but only when the current connection targets the agent the claim dispatched to; a chat rebound to a different agent cannot observe what the original target did, so a dispatch-target mismatch resolves to `unknown` without probing. On the matching agent: a found process is adopted onto the stale claim's epoch and re-attached; a token the agent does not know, answered within the trust window by a token index that predates the claim, permits taking over the claim and re-dispatching under the same token, which the agent dedups against any race with the original dispatch; anything else (old agents without the probe route, answers past the trust window, a token index younger than the claim after an agent restart, unobservable outcomes) resolves to `unknown`. Recovery never mints a new token for a tool call. The unknown write matches only rows still `starting`: the owner's process record can land concurrently with the staleness verdict, and that write wins, with the attempt resuming the recorded process instead.
...
The immediate post-interrupt pass ignores the task context's cancellation (the runner cancels it as soon as the interrupt transition lands) but aborts on the worker context, which is canceled at the start of worker shutdown, before the runner drain that awaits this pass: graceful shutdown is never blocked by a slow reconcile, and the sweep converges whatever the aborted pass abandoned.
