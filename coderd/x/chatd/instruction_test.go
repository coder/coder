...
func TestFormatSystemInstructionsFromParts(t *testing.T) {
	t.Parallel()

	t.Run("ExtractsOSAndDirectory", func(t *testing.T) {
		t.Parallel()
		...
		got := formatSystemInstructionsFromParts(parts)
		require.Contains(t, got, "Operating System: linux")
		require.Contains(t, got, "Working Directory: /project")
		require.Contains(t, got, "project rules")
	})

	t.Run("SkipsNonContextFileParts", func(t *testing.T) {
		t.Parallel()
		parts := []codersdk.ChatMessagePart{
			{
				Type:                 codersdk.ChatMessagePartTypeContextFile,
		...
		require.Contains(t, got, "Operating System: darwin")
		require.NotContains(t, got, "test-skill")
	})

	t.Run("EmptyParts", func(t *testing.T) {
		t.Parallel()
		got := formatSystemInstructionsFromParts(nil)
		require.Empty(t, got)
	})

	t.Run("MatchesFormatSystemInstructions", func(t *testing.T) {
		t.Parallel()
		parts := []codersdk.ChatMessagePart{
			{
				Type:                 codersdk.ChatMessagePartTypeContextFile,
				ContextFileContent:   "rules here",
				ContextFilePath:      "/home/AGENTS.md",
				ContextFileOS:        "linux",
				ContextFileDirectory: "/home",
			},
		}
