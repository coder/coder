...
func encodeNulInParts(parts []codersdk.ChatMessagePart) []codersdk.ChatMessagePart {
	...
	for i := range encoded {
		...
		p.ProviderMetadata = encodeNulInJSON(p.ProviderMetadata)
		p.ContextFileContent = encodeNulInString(p.ContextFileContent)
		p.ContextFilePath = encodeNulInString(p.ContextFilePath)
		p.ContextFileOS = encodeNulInString(p.ContextFileOS)
		p.ContextFileDirectory = encodeNulInString(p.ContextFileDirectory)
		p.Signature = encodeNulInString(p.Signature)
		p.ToolCallID = encodeNulInString(p.ToolCallID)
		p.ToolName = encodeNulInString(p.ToolName)
		p.Title = encodeNulInString(p.Title)
		p.URL = encodeNulInString(p.URL)
		p.Name = encodeNulInString(p.Name)
		p.FileName = encodeNulInString(p.FileName)
		p.SourceID = encodeNulInString(p.SourceID)
		p.MediaType = encodeNulInString(p.MediaType)
	}
	return encoded
}
...
func decodeNulInParts(parts []codersdk.ChatMessagePart) {
	for i := range parts {
		...
		p.ProviderMetadata = decodeNulInJSON(p.ProviderMetadata)
		p.ContextFileContent = decodeNulInString(p.ContextFileContent)
		p.ContextFilePath = decodeNulInString(p.ContextFilePath)
		p.ContextFileOS = decodeNulInString(p.ContextFileOS)
		p.ContextFileDirectory = decodeNulInString(p.ContextFileDirectory)
		p.Signature = decodeNulInString(p.Signature)
		p.ToolCallID = decodeNulInString(p.ToolCallID)
		p.ToolName = decodeNulInString(p.ToolName)
		p.Title = decodeNulInString(p.Title)
		p.URL = decodeNulInString(p.URL)
		p.Name = decodeNulInString(p.Name)
		p.FileName = decodeNulInString(p.FileName)
		p.SourceID = decodeNulInString(p.SourceID)
		p.MediaType = decodeNulInString(p.MediaType)
	}
}
