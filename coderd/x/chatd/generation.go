...
func (s *taskStarter) generateCompaction(
	...
	if override := prepared.CompactionConfig().Override; override != nil {
		...
		// rejected.
		if !sameCompactionProviderIdentity(prepared.CompactionConfig().ChatModelConfig, overrideModel.dbConfig) ||
			overrideModel.resolvedModel != prepared.CompactionConfig().Options.ResolvedModel {
	...
	messages, err := buildCompactionMessages(buildCompactionMessagesInput{
		...
		contentVersion:      chatprompt.CurrentContentVersion,
		pendingUserMessages: prepared.CompactionConfig().PendingUserRows,
	})
	if err != nil {
		s.server.metrics.RecordCompaction(metricProvider, metricModel, false, err)
		return s.finishGenerationError(ctx, machine, input, err, requireGenerationAttempt(attempt.number))
