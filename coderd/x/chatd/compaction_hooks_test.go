...
func TestCompactionHooksHintAndPostCommitResponses(t *testing.T) {
	...
	fixture := startCompactionHookChat(t,
		func(t *testing.T, db database.Store, request agenthooks.Request) (int, string) {
			...
			case agenthooks.EventPreCompact:
				return http.StatusOK, `{"model_context":"preserve deployment constraints","user_message":"compaction starting","allowed_tools":["read_file"],"end_chat":true}`
	...
	// post_compact dispatches before the commit so its effects land
	// atomically with the compaction step; the hook observes the
	// pre-compaction history.
	require.False(t, postSawCommitted.Load())
	require.Equal(t, int32(1), fixture.compactionCalls.Load())

	updated, err := fixture.db.GetChatByID(fixture.ctx, fixture.chat.ID)
...
func TestPreCompactHookFailureAbortsCompaction(t *testing.T) {
	...
	dispatch := lifecycleDispatch(t, fixture.db, fixture.chat.ID, agenthooks.EventPreCompact)
...
func TestPostCompactHookFailureKeepsCompaction(t *testing.T) {
	...
	waitCtx := testutil.Context(t, testutil.WaitLong)
	failed := waitForChatStatus(waitCtx, t, fixture.db, fixture.chat.ID, database.ChatStatusError)
	// The failure commits atomically with the compaction step, so the
	// hook fires before the rows exist while the rows still persist.
	require.False(t, postSawCommitted.Load())
	require.Equal(t, int32(1), fixture.compactionCalls.Load())
...
func startCompactionHookChat(
	...
	require.NoError(t, err)
	return compactionHookFixture{ctx: ctx, db: db, chat: chat, compactionCalls: &compactionCalls}
}
