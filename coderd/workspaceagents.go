...
func (api *API) workspaceAgentICEServers(rw http.ResponseWriter, r *http.Request) {
	httpapi.Write(r.Context(), rw, http.StatusOK, api.ICEServers)
}
...
func (api *API) workspaceAgentTurn(rw http.ResponseWriter, r *http.Request) {
	...
	localAddress, _ := ctx.Value(http.LocalAddrContextKey).(*net.TCPAddr)
	...
	ctx, wsNetConn := websocketNetConn(ctx, wsConn, websocket.MessageBinary)
	defer wsNetConn.Close() // Also closes conn.
	// end span so we don't get long lived trace data
	tracing.EndHTTPSpan(r, http.StatusOK, trace.SpanFromContext(ctx))
...
// ctx for cancellation if it's use is safe or r.Hijack() has
// not been performed.
...
func (api *API) workspaceAgentCoordinate(rw http.ResponseWriter, r *http.Request) {
	...
	err = api.TailnetCoordinator.ServeAgent(websocket.NetConn(ctx, conn, websocket.MessageBinary), workspaceAgent.ID)
	if err != nil {
