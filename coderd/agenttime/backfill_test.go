...
func TestAgentTimeBackfillExcludesNegativeRuntime(t *testing.T) {
	t.Parallel()

	ctx := testutil.Context(t, testutil.WaitLong)
	db, _, sqlDB := dbtestutil.NewDBWithSQLDB(t, dbtestutil.WithDumpOnFailure())
	fixture := setupAgentTimeFixture(t, db)
	day := time.Date(2025, 2, 1, 12, 0, 0, 0, time.UTC)
	positiveID := insertAgentTimeMessage(ctx, t, sqlDB, fixture.chat.ID, database.ChatMessageRoleAssistant, database.ChatMessageVisibilityBoth, day, int64Ptr(100), false)
	negativeID := insertAgentTimeMessage(ctx, t, sqlDB, fixture.chat.ID, database.ChatMessageRoleAssistant, database.ChatMessageVisibilityBoth, day, int64Ptr(-200), false)
	clearAgentTimeAccounting(ctx, t, sqlDB, []int64{positiveID, negativeID})
	...
	require.NoError(t, err)
	require.True(t, event.Completed)
	require.False(t, event.ResetCursor)
	requireDailyAgentTime(ctx, t, sqlDB, fixture.org.ID, fixture.user.ID, date(2025, 2, 1), 100)
	requireAgentTimeMarkerCount(ctx, t, sqlDB, []int64{positiveID}, 1)
	requireAgentTimeMarkerCount(ctx, t, sqlDB, []int64{negativeID}, 0)
}
