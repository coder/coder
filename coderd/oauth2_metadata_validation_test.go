	...
	t.Run("RedirectURIValidation", func(t *testing.T) {
		...
		tests := []struct {
			...
			},
			{
				...
				redirectURIs: []string{"myapp://auth/callback"},
				expectError:  false,
			},
			{
			...
			{
				name:         "ValidJavaScriptScheme",
				redirectURIs: []string{"javascript:alert('xss')"},
				expectError:  false, // Custom schemes are allowed per RFC 7591 for native apps
			},
			{
				name:         "ValidDataScheme",
			...
			{
				name:         "MultipleValid",
				redirectURIs: []string{"https://example.com/callback", "myapp://auth"},
				expectError:  false,
			},
			{
	...
	t.Run("TokenEndpointAuthMethodValidation", func(t *testing.T) {
		...
		tests := []struct {
			...
			},
			{
				name:        "InvalidNone",
				authMethod:  "none",
				expectError: true, // "none" is not currently supported
			},
			{
