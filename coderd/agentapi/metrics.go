...
// NewBuildDurationHistogram creates and registers a histogram that
// tracks the end-to-end duration from workspace build creation to
// agent ready, by template.
//
// This metric is recorded by the coderd replica handling the agent's
// connection when the last agent reports ready. In multi-replica
// deployments, each replica only has observations for agents it
// handles. Prometheus should be configured to scrape all replicas,
// and queries should aggregate across instances.
//
...
func NewBuildDurationHistogram(reg prometheus.Registerer) *prometheus.HistogramVec {
