...
import (
	...
	"github.com/coder/coder/v2/coderd/alerts/render"
...
type InvalidDefaultAlertMethodError struct {
	Method string
}

func (e InvalidDefaultAlertMethodError) Error() string {
...
type StoreEnqueuer struct {
	...
	defaultMethod  database.AlertMethod
	defaultEnabled bool
	inboxEnabled   bool
...
func NewStoreEnqueuer(cfg codersdk.NotificationsConfig, store Store, helpers template.FuncMap, log slog.Logger, clock quartz.Clock) (*StoreEnqueuer, error) {
	var method database.AlertMethod
	...
	// setting it.
	if err := method.Scan(cfg.Method.String()); err != nil || method == database.AlertMethodInbox {
		return nil, InvalidDefaultAlertMethodError{Method: cfg.Method.String()}
	}
...
func (s *StoreEnqueuer) EnqueueWithData(ctx context.Context, userID, templateID uuid.UUID, labels map[string]string, data map[string]any, createdBy string, targets ...uuid.UUID) ([]uuid.UUID, error) {
	...
	methods := []database.AlertMethod{}
	if metadata.CustomMethod.Valid {
		methods = append(methods, metadata.CustomMethod.AlertMethod)
	...
	if !slices.Contains(methods, database.AlertMethodInbox) && s.inboxEnabled {
		methods = append(methods, database.AlertMethodInbox)
	}
	...
	for _, method := range methods {
		...
		if method == database.AlertMethodInbox && templateID == TemplateUserRequestedOneTimePasscode {
			continue
		}

		id := uuid.New()
		err = s.store.EnqueueAlertMessage(ctx, database.EnqueueAlertMessageParams{
			ID:              id,
			UserID:          userID,
			AlertTemplateID: templateID,
			Method:          method,
		...
		if err != nil {
			...
			if database.IsUniqueViolation(err, database.UniqueAlertMessagesDedupeHashIndex) {
				s.log.Debug(ctx, "notification not enqueued",
					slog.F("template_id", templateID),
					slog.F("user_id", userID),
					slog.F("method", method),
...
func (s *StoreEnqueuer) buildPayload(metadata database.FetchNewMessageMetadataRow, labels map[string]string, data map[string]any, targets []uuid.UUID) (*types.MessagePayload, error) {
	payload := types.MessagePayload{
		...
		NotificationName: metadata.AlertName,
		AlertTemplateID:  metadata.AlertTemplateID.String(),
	...
	out, err := render.GoTemplate(string(metadata.Actions.RawMessage), payload, s.helpers)
	if err != nil {
