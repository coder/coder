...
func newNotifier(outerCtx context.Context, cfg codersdk.NotificationsConfig, id uuid.UUID, log slog.Logger, db Store,
	hr map[database.AlertMethod]Handler, helpers template.FuncMap, metrics *Metrics, clock quartz.Clock,
) *notifier {
	gracefulCtx, gracefulCancel := context.WithCancel(outerCtx)
	return &notifier{
		id:             id,
		cfg:            cfg,
		log:            log.Named("notifier").With(slog.F("alerter_id", id)),
		outerCtx:       outerCtx,
...
func (n *notifier) fetch(ctx context.Context) ([]database.AcquireAlertMessagesRow, error) {
	msgs, err := n.store.AcquireAlertMessages(ctx, database.AcquireAlertMessagesParams{
...
func (n *notifier) prepare(ctx context.Context, msg database.AcquireAlertMessagesRow) (dispatch.DeliveryFunc, error) {
	select {
	case <-ctx.Done():
		return nil, ctx.Err()
	default:
...
func (n *notifier) deliver(ctx context.Context, msg database.AcquireAlertMessagesRow, deliver dispatch.DeliveryFunc, success, failure chan<- dispatchResult) error {
	select {
	case <-ctx.Done():
		return ctx.Err()
	default:
...
func (n *notifier) newSuccessfulDispatch(msg database.AcquireAlertMessagesRow) dispatchResult {
...
func (n *notifier) newFailedDispatch(msg database.AcquireAlertMessagesRow, err error, retryable bool) dispatchResult {
	var result string
...
func (n *notifier) newInhibitedDispatch(msg database.AcquireAlertMessagesRow) dispatchResult {
	return dispatchResult{
		notifier:  n.id,
		msg:       msg.ID,
		ts:        dbtime.Time(n.clock.Now().UTC()),
