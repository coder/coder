...
func (s *InboxHandler) dispatch(payload types.MessagePayload, title, body string) DeliveryFunc {
	return func(ctx context.Context, msgID uuid.UUID) (bool, error) {
		...
		templateID, err := uuid.Parse(payload.AlertTemplateID)
		if err != nil {
		...
		// nolint:exhaustruct
		insertedNotif, err := s.store.InsertInboxAlert(ctx, database.InsertInboxAlertParams{
			ID:         msgID,
			UserID:     userID,
		...
		event := coderdpubsub.InboxAlertEvent{
			Kind: coderdpubsub.InboxAlertEventKindNew,
			InboxAlert: codersdk.InboxAlert{
				ID:         msgID,
				UserID:     userID,
				TemplateID: templateID,
				Targets:    payload.Targets,
				...
				Actions: func() []codersdk.InboxAlertAction {
					var actions []codersdk.InboxAlertAction
		...
		err = s.pubsub.Publish(coderdpubsub.InboxAlertForOwnerEventChannel(userID), payload)
		if err != nil {
