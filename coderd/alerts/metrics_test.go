...
func TestMetrics(t *testing.T) {
	...
	tmpl := alerts.TemplateWorkspaceDeleted
	...
	mgr, err := alerts.NewManager(cfg, store, pubsub, defaultHelpers(), metrics, logger.Named("manager"))
	...
	methodTemplateFP := fingerprintLabels(alerts.LabelMethod, string(method), alerts.LabelTemplateID, tmpl.String())
	methodTemplateFPWithInbox := fingerprintLabels(alerts.LabelMethod, string(database.AlertMethodInbox), alerts.LabelTemplateID, tmpl.String())

	methodFP := fingerprintLabels(alerts.LabelMethod, string(method))
	methodFPWithInbox := fingerprintLabels(alerts.LabelMethod, string(database.AlertMethodInbox))
	...
	expected := map[string]func(metric *dto.Metric, series string) bool{
		"coderd_notifications_dispatch_attempts_total": func(metric *dto.Metric, series string) bool {
			...
			results := map[string]float64{
				alerts.ResultSuccess:  1,               // Only 1 successful delivery.
				alerts.ResultTempFail: maxAttempts - 1, // 2 temp failures, on the 3rd it'll be marked permanent failure.
				alerts.ResultPermFail: 1,               // 1 permanent failure after retries exhausted.
			}

			var match string
			for result, val := range results {
				seriesFP := fingerprintLabels(alerts.LabelMethod, string(method), alerts.LabelTemplateID, tmpl.String(), alerts.LabelResult, result)
				seriesFPWithInbox := fingerprintLabels(alerts.LabelMethod, string(database.AlertMethodInbox), alerts.LabelTemplateID, tmpl.String(), alerts.LabelResult, result)
...
func TestPendingUpdatesMetric(t *testing.T) {
	...
	const method = database.AlertMethodSmtp
	...
	mgr, err := alerts.NewManager(cfg, interceptor, pubsub, defaultHelpers(), metrics, logger.Named("manager"),
		alerts.WithTestClock(mClock))
	require.NoError(t, err)
	t.Cleanup(func() {
		assert.NoError(t, mgr.Stop(ctx))
	})
	...
	mgr.WithHandlers(map[database.AlertMethod]alerts.Handler{
		method:                    handler,
		database.AlertMethodInbox: inboxHandler,
	})
...
func TestCustomMethodMetricCollection(t *testing.T) {
	...
	var (
		...
		metrics         = alerts.NewMetrics(reg)
		tmpl            = alerts.TemplateWorkspaceDeleted
		anotherTemplate = alerts.TemplateWorkspaceDormant
	)
	...
	require.Eventually(t, func() bool {
		return promtest.ToFloat64(metrics.DispatchAttempts.WithLabelValues(string(defaultMethod), anotherTemplate.String(), alerts.ResultSuccess)) > 0 &&
			promtest.ToFloat64(metrics.DispatchAttempts.WithLabelValues(string(customMethod), tmpl.String(), alerts.ResultSuccess)) > 0
	}, testutil.WaitShort, testutil.IntervalFast)
}
...
func newUpdateSignallingInterceptor(interceptor alerts.Store) *updateSignallingInterceptor {
...
func (u *updateSignallingInterceptor) BulkMarkAlertMessagesSent(ctx context.Context, arg database.BulkMarkAlertMessagesSentParams) (int64, error) {
	u.updateSuccess <- len(arg.IDs)
	return u.Store.BulkMarkAlertMessagesSent(ctx, arg)
}

func (u *updateSignallingInterceptor) BulkMarkAlertMessagesFailed(ctx context.Context, arg database.BulkMarkAlertMessagesFailedParams) (int64, error) {
	u.updateFailure <- len(arg.IDs)
	return u.Store.BulkMarkAlertMessagesFailed(ctx, arg)
}
...
func newBarrierHandler(total int, handler alerts.Handler) *barrierHandler {
