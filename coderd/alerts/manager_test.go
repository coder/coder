...
func TestBufferedUpdates(t *testing.T) {
	...
	mgr, err := alerts.NewManager(cfg, interceptor, ps, defaultHelpers(), createMetrics(), logger.Named("notifications-manager"))
	require.NoError(t, err)

	handlers := map[database.AlertMethod]alerts.Handler{
		database.AlertMethodSmtp:  santa,
		database.AlertMethodInbox: santaInbox,
	}

	mgr.WithHandlers(handlers)
	enq, err := alerts.NewStoreEnqueuer(cfg, interceptor, defaultHelpers(), logger.Named("notifications-enqueuer"), quartz.NewReal())
	require.NoError(t, err)

	user := dbgen.User(t, store, database.User{})

	// WHEN: notifications are enqueued which should succeed and fail
	_, err = enq.Enqueue(ctx, user.ID, alerts.TemplateWorkspaceDeleted, map[string]string{"nice": "true", "i": "0"}, "") // Will succeed.
	require.NoError(t, err)
	_, err = enq.Enqueue(ctx, user.ID, alerts.TemplateWorkspaceDeleted, map[string]string{"nice": "true", "i": "1"}, "") // Will succeed.
	require.NoError(t, err)
	_, err = enq.Enqueue(ctx, user.ID, alerts.TemplateWorkspaceDeleted, map[string]string{"nice": "false", "i": "2"}, "") // Will fail.
	require.NoError(t, err)
...
func TestBuildPayload(t *testing.T) {
	...
	interceptor := newEnqueueInterceptor(store,
		...
		func() database.FetchNewMessageMetadataRow {
			...
			return database.FetchNewMessageMetadataRow{
				AlertName: "My Notification",
				Actions:   pqtype.NullRawMessage{RawMessage: out, Valid: true},
				UserID:    uuid.New(),
	...
	enq, err := alerts.NewStoreEnqueuer(defaultNotificationsConfig(database.AlertMethodSmtp), interceptor, helpers, logger.Named("notifications-enqueuer"), quartz.NewReal())
	require.NoError(t, err)

	// WHEN: a notification is enqueued
	_, err = enq.Enqueue(ctx, uuid.New(), alerts.TemplateWorkspaceDeleted, map[string]string{
		"name": "my-workspace",
	}, "test")
	require.NoError(t, err)
...
func TestStopBeforeRun(t *testing.T) {
	...
	mgr, err := alerts.NewManager(defaultNotificationsConfig(database.AlertMethodSmtp), store, ps, defaultHelpers(), createMetrics(), logger.Named("notifications-manager"))
	require.NoError(t, err)
...
func (b *syncInterceptor) BulkMarkAlertMessagesSent(ctx context.Context, arg database.BulkMarkAlertMessagesSentParams) (int64, error) {
	updated, err := b.Store.BulkMarkAlertMessagesSent(ctx, arg)
...
func (b *syncInterceptor) BulkMarkAlertMessagesFailed(ctx context.Context, arg database.BulkMarkAlertMessagesFailedParams) (int64, error) {
	updated, err := b.Store.BulkMarkAlertMessagesFailed(ctx, arg)
	// #nosec G115 - Safe conversion as the count of updated notification messages is expected to be within int32 range
...
func newEnqueueInterceptor(db alerts.Store, metadataFn func() database.FetchNewMessageMetadataRow) *enqueueInterceptor {
	return &enqueueInterceptor{Store: db, payload: make(chan types.MessagePayload, 2), metadataFn: metadataFn}
}

func (e *enqueueInterceptor) EnqueueAlertMessage(_ context.Context, arg database.EnqueueAlertMessageParams) error {
