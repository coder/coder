...
var _ alerts.Enqueuer = &FakeEnqueuer{}
...
func (f *FakeEnqueuer) assertRBACNoLock(ctx context.Context) {
	...
	for _, a := range []policy.Action{policy.ActionCreate, policy.ActionRead} {
		err := f.authorizer.Authorize(ctx, act, a, rbac.ResourceAlertMessage)
		if err == nil {
			return
		}
		...
		if rbac.IsUnauthorizedError(err) {
			panic(fmt.Sprintf("Developer error: not authorized to %s %s. "+
				...
				"policy.ActionCreate on rbac.ResourceAlertMessage", a, rbac.ResourceAlertMessage.Type))
		}
