...
type Manager struct {
	...
	handlers map[database.AlertMethod]Handler
	method   database.AlertMethod
...
func NewManager(cfg codersdk.NotificationsConfig, store Store, ps pubsub.Pubsub, helpers template.FuncMap, metrics *Metrics, log slog.Logger, opts ...ManagerOption) (*Manager, error) {
	...
	m := &Manager{
		...
		// see BulkMarkAlertMessagesSent/BulkMarkAlertMessagesFailed. If we had the ability to batch updates,
...
func defaultHandlers(cfg codersdk.NotificationsConfig, log slog.Logger, store Store, ps pubsub.Pubsub) map[database.AlertMethod]Handler {
	return map[database.AlertMethod]Handler{
		database.AlertMethodSmtp:    dispatch.NewSMTPHandler(cfg.SMTP, log.Named("dispatcher.smtp")),
		database.AlertMethodWebhook: dispatch.NewWebhookHandler(cfg.Webhook, log.Named("dispatcher.webhook")),
		database.AlertMethodInbox:   dispatch.NewInboxHandler(log.Named("dispatcher.inbox"), store, ps),
	}
}

// WithHandlers allows for tests to inject their own handlers to verify functionality.
func (m *Manager) WithHandlers(reg map[database.AlertMethod]Handler) {
	m.handlers = reg
}
...
func (m *Manager) syncUpdates(ctx context.Context) {
	...
	var (
		successParams database.BulkMarkAlertMessagesSentParams
		failureParams database.BulkMarkAlertMessagesFailedParams
	)
	...
	for i := 0; i < nFailure; i++ {
		...
		var (
			reason string
			status database.AlertMessageStatus
		)

		switch {
		case res.retryable:
			status = database.AlertMessageStatusTemporaryFailure
		case res.inhibited:
			status = database.AlertMessageStatusInhibited
			reason = "disabled by user"
		default:
			status = database.AlertMessageStatusPermanentFailure
		}
	...
	go func() {
		...
		n, err := m.store.BulkMarkAlertMessagesSent(uctx, successParams)
		if err != nil {
	...
	go func() {
		...
		n, err := m.store.BulkMarkAlertMessagesFailed(uctx, failureParams)
		if err != nil {
			logger.Error(ctx, "bulk update failed", slog.Error(err))
			return
		}
