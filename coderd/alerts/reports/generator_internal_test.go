...
func TestReportFailedWorkspaceBuilds(t *testing.T) {
	...
	t.Run("FailedBuilds_SecondRun_Report_ThirdRunTooEarly_NoReport_FourthRun_Report", func(t *testing.T) {
		...
		verifyNotification := func(t *testing.T, recipientID uuid.UUID, notif *alertstest.FakeNotification, templates []map[string]any) {
			t.Helper()

			require.Equal(t, recipientID, notif.UserID)
			require.Equal(t, alerts.TemplateWorkspaceBuildsFailedReport, notif.TemplateID)
	...
	t.Run("TooManyFailedBuilds_SecondRun_Report", func(t *testing.T) {
		...
		verifyNotification := func(t *testing.T, recipient database.User, notif *alertstest.FakeNotification, templates []map[string]any) {
			t.Helper()
...
func setup(t *testing.T) (context.Context, slog.Logger, database.Store, pubsub.Pubsub, *alertstest.FakeEnqueuer, *quartz.Mock) {
	t.Helper()
