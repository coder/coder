...
func NewReportGenerator(ctx context.Context, logger slog.Logger, db database.Store, enqueuer alerts.Enqueuer, clk quartz.Clock) io.Closer {
...
func reportFailedWorkspaceBuilds(ctx context.Context, logger slog.Logger, db database.Store, enqueuer alerts.Enqueuer, clk quartz.Clock) error {
	now := clk.Now()
	since := now.Add(-failedWorkspaceBuildsReportFrequency)

	// Firstly, check if this is the first run of the job ever
	reportLog, err := db.GetAlertReportGeneratorLogByTemplate(ctx, alerts.TemplateWorkspaceBuildsFailedReport)
	...
	if xerrors.Is(err, sql.ErrNoRows) {
		...
		logger.Info(ctx, "report generator is executing the job for the first time", slog.F("alert_template_id", alerts.TemplateWorkspaceBuildsFailedReport))

		err = db.UpsertAlertReportGeneratorLog(ctx, database.UpsertAlertReportGeneratorLogParams{
			AlertTemplateID: alerts.TemplateWorkspaceBuildsFailedReport,
	...
	for templateAdmin, reports := range reports {
		...
		if _, err := enqueuer.EnqueueWithData(ctx, templateAdmin, alerts.TemplateWorkspaceBuildsFailedReport,
			map[string]string{},
			reportData,
			"report_generator",
