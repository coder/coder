...
func defaultNotificationsConfig(method database.AlertMethod) codersdk.NotificationsConfig {
	var (
	...
	switch method {
	case database.AlertMethodSmtp:
		smtp.Smarthost = serpent.String("localhost:1337")
	case database.AlertMethodWebhook:
...
func createMetrics() *alerts.Metrics {
	return alerts.NewMetrics(prometheus.NewRegistry())
}
...
func newDispatchInterceptor(h alerts.Handler) *dispatchInterceptor {
...
var _ alerts.Handler = &chanHandler{}
