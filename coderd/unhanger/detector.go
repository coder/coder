...
func unhangJob(ctx context.Context, log slog.Logger, db database.Store, pub pubsub.Pubsub, jobID uuid.UUID) error {
	...
	err := db.InTx(func(db database.Store) error {
		...
		if err != nil {
			return fmt.Errorf("insert logs for hung job: %w", err)
		}
		...
		if job.Type == database.ProvisionerJobTypeWorkspaceBuild {
			...
			if err != nil {
				return fmt.Errorf("get workspace build for workspace build job by job id: %w", err)
			}
			...
			if len(build.ProvisionerState) == 0 {
				...
				if err == nil {
					...
					if err != nil {
						return fmt.Errorf("update workspace build by id: %w", err)
					}
				}
			}
		}
