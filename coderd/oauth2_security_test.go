...
func TestOAuth2PrivilegeEscalation(t *testing.T) {
	...
	t.Run("CustomSchemeRedirectURIsAllowed", func(t *testing.T) {
		t.Parallel()
		...
		// Per RFC 7591, custom schemes are allowed for native applications
		// This includes potentially dangerous schemes that should be validated
		// at authorization time, not registration time
		customSchemeRequests := []codersdk.OAuth2ClientRegistrationRequest{
			{
			...
			{
				// Even potentially dangerous schemes are allowed per RFC 7591
				// Security validation happens at authorization time
		...
		for i, req := range customSchemeRequests {
			t.Run(fmt.Sprintf("CustomSchemeRequest_%d", i), func(t *testing.T) {
				t.Parallel()

				_, err := client.PostOAuth2ClientRegistration(ctx, req)
				// Custom schemes should be allowed per RFC 7591
				require.NoError(t, err)
			})
		}
	})
