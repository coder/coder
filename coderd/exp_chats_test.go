...
func findProviderInList(providers []codersdk.ChatModelProvider, provider string) *codersdk.ChatModelProvider {
...
func requireAvailableProvider(
	...
	availableProvider := findProviderInList(providers, provider)
	require.NotNil(t, availableProvider)
	require.True(t, availableProvider.Available)
	return availableProvider
}

func createChatModelConfigWithProviderIDs(
	ctx context.Context,
	t *testing.T,
	client *codersdk.ExperimentalClient,
	...
	req.ProviderConfigIDs = append([]uuid.UUID(nil), providerConfigIDs...)
	if req.ContextLimit == nil {
		contextLimit := int64(4096)
		req.ContextLimit = &contextLimit
	}

	modelConfig, err := client.CreateChatModelConfig(ctx, req)
	require.NoError(t, err)
	return modelConfig
}

func defaultCreateChatModelConfigRequest(
	provider string,
	model string,
	providerConfigIDs ...uuid.UUID,
) codersdk.CreateChatModelConfigRequest {
	contextLimit := int64(4096)
	return codersdk.CreateChatModelConfigRequest{
		Provider:          provider,
		Model:             model,
		ContextLimit:      &contextLimit,
		ProviderConfigIDs: append([]uuid.UUID(nil), providerConfigIDs...),
	}
}
...
func createProviderBackedChatModelConfig(
	...
	req codersdk.CreateChatModelConfigRequest,
) (codersdk.ChatProviderConfig, codersdk.ChatModelConfig) {
	t.Helper()

	providerConfig := createProviderConfig(ctx, t, client, provider, providerDisplayName, nil)
	if req.Provider == "" {
		req.Provider = provider
	}

	modelConfig := createChatModelConfigWithProviderIDs(ctx, t, client, req, providerConfig.ID)
	return providerConfig, modelConfig
}

func requireChatModelProviderAttachment(
	t *testing.T,
	attachment codersdk.ChatModelProviderAttachment,
	providerConfig codersdk.ChatProviderConfig,
	wantPriority int32,
) {
	t.Helper()

	require.Equal(t, providerConfig.ID, attachment.ProviderConfigID)
	require.EqualValues(t, wantPriority, attachment.Priority)
	require.Equal(t, providerConfig.Provider, attachment.Provider)
...
func TestListChats(t *testing.T) {
	...
	t.Run("PrimaryDisabledAttachmentStillListed", func(t *testing.T) {
		t.Parallel()

		ctx := testutil.Context(t, testutil.WaitLong)
		client := newChatClient(t)
		_ = coderdtest.CreateFirstUser(t, client.Client)
		primary := createProviderConfig(ctx, t, client, "openai", "Primary OpenAI", nil)
		fallback := createProviderConfig(ctx, t, client, "openai", "Fallback OpenAI", nil)
		createChatModelConfigWithProviderIDs(ctx, t, client, codersdk.CreateChatModelConfigRequest{
			Provider:    "openai",
			Model:       "gpt-4o-mini",
			DisplayName: "OpenAI fallback model",
		}, primary.ID, fallback.ID)

		_, err := client.UpdateChatProvider(ctx, primary.ID, codersdk.UpdateChatProviderConfigRequest{
			Enabled: ptr.Ref(false),
		})
		require.NoError(t, err)
...
func TestListChatModels(t *testing.T) {
	...
	t.Run("UserOnlyProviderRequiresUserKey", func(t *testing.T) {
		...
		anthropicProvider := findProviderInList(models.Providers, "anthropic")
		...
		requireAvailableProvider(t, models.Providers, "anthropic")
	})
	...
	t.Run("CentralAndUserWithFallback", func(t *testing.T) {
		...
		requireAvailableProvider(t, models.Providers, "google")
	...
	t.Run("EnvPresetProviderWithDeploymentKeyOnlyStaysEnabled", func(t *testing.T) {
		...
		store, ps, _ := dbtestutil.NewDBWithSQLDB(t)
		...
		firstUser := coderdtest.CreateFirstUser(t, client.Client)
		providerConfig, err := store.InsertChatProvider(dbauthz.AsSystemRestricted(ctx), database.InsertChatProviderParams{
			Provider:             "openai",
			DisplayName:          "OpenAI Deployment Only",
	...
	t.Run("DisabledProvidersAndModelsAreFilteredOut", func(t *testing.T) {
		...
		_ = coderdtest.CreateFirstUser(t, client.Client)
		provider := createProviderConfig(ctx, t, client, "openai", "OpenAI Enabled", nil)
...
func TestCreateChatProvider(t *testing.T) {
	...
	t.Run("Conflict", func(t *testing.T) {
		...
		require.NoError(t, err, "multi-provider: creating a second config for the same provider should succeed")
	})
...
func TestDeleteChatProvider(t *testing.T) {
	...
	t.Run("DeleteProviderSoftDeletesBoundModelsPreservingChatHistory", func(t *testing.T) {
		...
		survivorProviderConfig := createProviderConfig(ctx, t, client, "anthropic", "Anthropic Survivor", nil)
		...
		survivorModel, err := client.CreateChatModelConfig(ctx, codersdk.CreateChatModelConfigRequest{
			...
			DisplayName:       "Anthropic Survivor",
			ContextLimit:      &contextLimit,
			ProviderConfigIDs: []uuid.UUID{survivorProviderConfig.ID},
		})
		require.NoError(t, err)
		...
		err = sqlDB.QueryRowContext(ctx, `
			...
			WHERE id = $1
		`, boundModel.ID).Scan(&deleted)
		require.NoError(t, err)
		require.True(t, deleted,
			"bound model row should be soft-deleted in place")

		var remainingBoundAttachments int
		err = sqlDB.QueryRowContext(ctx, `
			SELECT COUNT(*)
			FROM chat_model_provider_configs
			WHERE model_config_id = $1
		`, boundModel.ID).Scan(&remainingBoundAttachments)
		require.NoError(t, err)
		require.Zero(t, remainingBoundAttachments,
			"soft-deleted bound model attachments should be removed by provider cascade")

		configs, err := client.ListChatModelConfigs(ctx)
		require.NoError(t, err)
		var foundSurvivor bool
		var foundLegacyUnbound bool
		for _, c := range configs {
			require.NotEqual(t, boundModel.ID, c.ID,
				"bound model should be soft-deleted after provider deletion")
			if c.ID == unboundRow.ID {
				foundLegacyUnbound = true
				require.Empty(t, c.ProviderConfigs,
					"legacy zero-attachment models should remain admin-visible")
			}
			if c.ID == survivorModel.ID {
				foundSurvivor = true
				require.Len(t, c.ProviderConfigs, 1,
					"surviving model should keep its remaining provider attachment")
				require.Equal(t, survivorProviderConfig.ID, c.ProviderConfigs[0].ProviderConfigID,
					"surviving model should remain attached to the non-deleted provider")
			}
		}
		require.True(t, foundSurvivor, "surviving model config must still exist")
		require.True(t, foundLegacyUnbound, "legacy zero-attachment model should remain in admin listings")

		runtimeModels, err := client.ListChatModels(ctx)
		require.NoError(t, err)
		openAIProvider := findProviderInList(runtimeModels.Providers, "openai")
		anthropicProvider := findProviderInList(runtimeModels.Providers, "anthropic")
		require.Nil(t, openAIProvider,
			"bound model should disappear from runtime listings after losing its last attachment")
		require.NotNil(t, anthropicProvider)
		require.Len(t, anthropicProvider.Models, 1)
		require.Equal(t, survivorModel.Model, anthropicProvider.Models[0].Model)
		...
		require.NotEmpty(t, messagesResult.Messages,
			"existing chat messages should survive provider deletion")
	})
	...
	t.Run("ReassignsDefaultAfterDeletingProviderOwnedDefault", func(t *testing.T) {
		...
		survivorProviderConfig := createProviderConfig(ctx, t, client, "anthropic", "Anthropic Survivor Provider", nil)
		...
		survivor, err := client.CreateChatModelConfig(ctx, codersdk.CreateChatModelConfigRequest{
			Provider:          "anthropic",
			Model:             "000-survivor",
			DisplayName:       "Anthropic Survivor",
			ContextLimit:      &contextLimit,
		...
		for _, c := range configs {
			...
			if c.ID == survivor.ID {
				foundSurvivor = true
				require.Len(t, c.ProviderConfigs, 1,
					"surviving model config should keep its provider attachment")
				require.Equal(t, survivorProviderConfig.ID, c.ProviderConfigs[0].ProviderConfigID,
					"surviving model config should remain attached to the non-deleted provider")
	...
	t.Run("DeletePrimaryAttachmentKeepsFallbackModel", func(t *testing.T) {
		t.Parallel()
		...
		primaryProviderConfig := createProviderConfig(ctx, t, client, "openai", "OpenAI Primary", nil)
		fallbackProviderConfig := createProviderConfig(ctx, t, client, "openai", "OpenAI Fallback", nil)
		...
		modelConfig, err := client.CreateChatModelConfig(ctx, codersdk.CreateChatModelConfigRequest{
			...
			DisplayName:       "OpenAI Fallback Model",
			ContextLimit:      &contextLimit,
			ProviderConfigIDs: []uuid.UUID{primaryProviderConfig.ID, fallbackProviderConfig.ID},
		})
		require.NoError(t, err)
		...
		require.Equal(t, fallbackProviderConfig.ID, attachments[0].ProviderConfigID)
		require.EqualValues(t, 1, attachments[0].Priority)
		...
		for _, config := range configs {
			...
			require.Len(t, config.ProviderConfigs, 1)
			require.Equal(t, fallbackProviderConfig.ID, config.ProviderConfigs[0].ProviderConfigID)
			require.EqualValues(t, 1, config.ProviderConfigs[0].Priority)
		}
		require.True(t, foundModel, "model with fallback attachment should survive provider deletion")

		runtimeModels, err := client.ListChatModels(ctx)
		require.NoError(t, err)
		var foundRuntimeModel bool
		for _, provider := range runtimeModels.Providers {
			if provider.Provider != "openai" {
				continue
			}
			for _, runtimeModel := range provider.Models {
				if runtimeModel.Model == modelConfig.Model {
					foundRuntimeModel = true
				}
			}
		}
		require.True(t, foundRuntimeModel, "model with surviving fallback should remain runtime-visible")
	})
	...
	t.Run("DeleteProviderWithActiveChatsSoftDeletesBoundModels", func(t *testing.T) {
		...
		modelConfig, err := client.CreateChatModelConfig(ctx, codersdk.CreateChatModelConfigRequest{
			...
			Model:             "gpt-4o-mini",
			ContextLimit:      &contextLimit,
			ProviderConfigIDs: []uuid.UUID{provider.ID},
		})
		require.NoError(t, err)
...
func TestChatProviderAPIKeysFromDeploymentValues(t *testing.T) {
	...
	t.Run("ReturnsBridgeConfigKeys", func(t *testing.T) {
		t.Parallel()
		...
		values.AI.BridgeConfig.Anthropic.BaseURL = serpent.String("https://custom-anthropic.example.com")

		keys := coderd.ChatProviderAPIKeysFromDeploymentValues(values)
		require.Equal(t, "deployment-openai-key", keys.OpenAI)
		require.Equal(t, "deployment-anthropic-key", keys.Anthropic)
		require.Equal(t, map[string]string{
			"openai":    "https://custom-openai.example.com",
			"anthropic": "https://custom-anthropic.example.com",
		}, keys.BaseURLByProvider)
	})

	t.Run("SkipsEmptyBridgeBaseURLs", func(t *testing.T) {
		t.Parallel()

		values := chatDeploymentValues(t)
		values.AI.BridgeConfig.OpenAI.Key = serpent.String("deployment-openai-key")
		values.AI.BridgeConfig.OpenAI.BaseURL = serpent.String("   ")
		values.AI.BridgeConfig.Anthropic.BaseURL = serpent.String("   ")

		keys := coderd.ChatProviderAPIKeysFromDeploymentValues(values)
		require.Equal(t, "deployment-openai-key", keys.OpenAI)
		require.Nil(t, keys.BaseURLByProvider)
	})
...
func TestListChatModelConfigs(t *testing.T) {
	...
	t.Run("AdminIncludesDisabledModelConfigs", func(t *testing.T) {
		...
		client := newChatClient(t)
		_ = coderdtest.CreateFirstUser(t, client.Client)
		_, disabledConfig := createProviderBackedChatModelConfig(ctx, t, client, "openai", "OpenAI Disabled", codersdk.CreateChatModelConfigRequest{
	...
	t.Run("NonAdminExcludesDisabledModelConfigs", func(t *testing.T) {
		...
		_, _ = createProviderBackedChatModelConfig(ctx, t, adminClient, "openai", "OpenAI Disabled", codersdk.CreateChatModelConfigRequest{
			Model:       "gpt-4o-disabled",
			DisplayName: "GPT-4o Disabled",
			Enabled:     ptr.Ref(false),
		})
	...
	t.Run("IncludesProviderConfigs", func(t *testing.T) {
		t.Parallel()

		ctx := testutil.Context(t, testutil.WaitLong)
		client := newChatClient(t)
		_ = coderdtest.CreateFirstUser(t, client.Client)
		providerConfig, modelConfig := createProviderBackedChatModelConfig(ctx, t, client, "openai", "OpenAI List Bound", codersdk.CreateChatModelConfigRequest{
		...
		for _, config := range configs {
			if config.ID == modelConfig.ID {
				found = true
				require.Len(t, config.ProviderConfigs, 1)
				require.Equal(t, providerConfig.ID, config.ProviderConfigs[0].ProviderConfigID)
			}
		}
		require.True(t, found)
	})

	t.Run("ListIncludesOrderedProviderConfigs", func(t *testing.T) {
		t.Parallel()

		ctx := testutil.Context(t, testutil.WaitLong)
		client := newChatClient(t)
		_ = coderdtest.CreateFirstUser(t, client.Client)
		firstProviderConfig := createProviderConfig(ctx, t, client, "openai", "OpenAI List First", nil)
		secondProviderConfig := createProviderConfig(ctx, t, client, "openai", "OpenAI List Second", nil)
		modelConfig := createChatModelConfigWithProviderIDs(ctx, t, client, codersdk.CreateChatModelConfigRequest{
			Provider:    "openai",
			Model:       "gpt-4o-mini-list-ordered",
			DisplayName: "GPT-4o Mini List Ordered",
		}, secondProviderConfig.ID, firstProviderConfig.ID)
		...
		for _, config := range configs {
			if config.ID == modelConfig.ID {
				found = true
				require.Len(t, config.ProviderConfigs, 2)
				require.Equal(t, secondProviderConfig.ID, config.ProviderConfigs[0].ProviderConfigID)
				require.EqualValues(t, 0, config.ProviderConfigs[0].Priority)
				require.Equal(t, "openai", config.ProviderConfigs[0].Provider)
				require.True(t, config.ProviderConfigs[0].HasAPIKey)
				require.Equal(t, firstProviderConfig.ID, config.ProviderConfigs[1].ProviderConfigID)
				require.EqualValues(t, 1, config.ProviderConfigs[1].Priority)
				require.Equal(t, "openai", config.ProviderConfigs[1].Provider)
				require.True(t, config.ProviderConfigs[1].HasAPIKey)
			}
		}
		require.True(t, found)
	})
	t.Run("NonAdminExcludesEnabledConfigsWithoutRunnableProviderRow", func(t *testing.T) {
		...
		adminClient, db := newChatClientWithDatabase(t)
		firstUser := coderdtest.CreateFirstUser(t, adminClient.Client)
		visibleConfig := createChatModelConfig(t, adminClient)
		memberClientRaw, _ := coderdtest.CreateAnotherUser(t, adminClient.Client, firstUser.OrganizationID)
		memberClient := codersdk.NewExperimentalClient(memberClientRaw)

		legacyConfig, err := db.InsertChatModelConfig(dbauthz.AsSystemRestricted(ctx), database.InsertChatModelConfigParams{
			Provider:             "openai",
		...
		require.NoError(t, err)
		var foundLegacyForAdmin bool
		for _, config := range adminConfigs {
			if config.ID != legacyConfig.ID {
				continue
			}
			foundLegacyForAdmin = true
			require.Empty(t, config.ProviderConfigs)
		}
		require.True(t, foundLegacyForAdmin)

		memberConfigs, err := memberClient.ListChatModelConfigs(ctx)
		require.NoError(t, err)

		var foundVisibleForMember bool
		for _, config := range memberConfigs {
			require.NotEqual(t, legacyConfig.ID, config.ID)
			if config.ID == visibleConfig.ID {
				foundVisibleForMember = true
			}
		}
		require.True(t, foundVisibleForMember)
	})
	...
	t.Run("NonAdminExcludesModelsBoundToDisabledProviderConfigs", func(t *testing.T) {
		...
		providerConfig, modelConfig := createProviderBackedChatModelConfig(ctx, t, adminClient, "openai", "OpenAI Member Hidden", codersdk.CreateChatModelConfigRequest{
...
func TestCreateChatModelConfig(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		_, modelConfig := createProviderBackedChatModelConfig(ctx, t, client, "openai", "OpenAI Success", codersdk.CreateChatModelConfigRequest{
			Model:       "gpt-4o-mini",
			IsDefault:   &isDefault,
			ModelConfig: pricing,
		})
	...
	t.Run("RejectsNegativePricing", func(t *testing.T) {
		...
		client := newChatClient(t)
		_ = coderdtest.CreateFirstUser(t, client.Client)
		providerConfig := createProviderConfig(ctx, t, client, "openai", "OpenAI Negative Pricing", nil)

		req := defaultCreateChatModelConfigRequest("openai", "gpt-4o-mini", providerConfig.ID)
		req.ModelConfig = &codersdk.ChatModelCallConfig{
			Cost: &codersdk.ModelCostConfig{
				InputPricePerMillionTokens: decRef("-0.01"),
			},
		}
		_, err := client.CreateChatModelConfig(ctx, req)
		sdkErr := requireSDKError(t, err, http.StatusBadRequest)
	...
	t.Run("MissingContextLimit", func(t *testing.T) {
		...
		client := newChatClient(t)
		_ = coderdtest.CreateFirstUser(t, client.Client)
		providerConfig := createProviderConfig(ctx, t, client, "openai", "OpenAI Missing Context Limit", nil)
	...
	t.Run("RejectsCreateWithoutProviderConfigIDs", func(t *testing.T) {
		t.Parallel()

		ctx := testutil.Context(t, testutil.WaitLong)
		client := newChatClient(t)
		_ = coderdtest.CreateFirstUser(t, client.Client)
		_, err := client.CreateChatModelConfig(ctx, defaultCreateChatModelConfigRequest("openai", "gpt-4o-mini"))
		sdkErr := requireSDKError(t, err, http.StatusBadRequest)
		require.Equal(t, "At least one provider config ID is required.", sdkErr.Message)
	})
	...
	t.Run("ForbiddenForOrganizationMember", func(t *testing.T) {
		...
		providerConfig := createProviderConfig(ctx, t, adminClient, "openai", "OpenAI Forbidden", nil)

		_, err := memberClient.CreateChatModelConfig(ctx, defaultCreateChatModelConfigRequest("openai", "gpt-4o-mini", providerConfig.ID))
		requireSDKError(t, err, http.StatusForbidden)
	})

	t.Run("SuccessWithProviderConfigIDs", func(t *testing.T) {
		t.Parallel()

		ctx := testutil.Context(t, testutil.WaitLong)
		client, db := newChatClientWithDatabase(t)
		_ = coderdtest.CreateFirstUser(t, client.Client)
		providerConfig, modelConfig := createProviderBackedChatModelConfig(ctx, t, client, "openai", "OpenAI Bound", codersdk.CreateChatModelConfigRequest{
			Model: "gpt-4o-mini",
		})
	...
	t.Run("CreateWithMultipleProviderConfigs", func(t *testing.T) {
		t.Parallel()

		ctx := testutil.Context(t, testutil.WaitLong)
		client, db := newChatClientWithDatabase(t)
		_ = coderdtest.CreateFirstUser(t, client.Client)
		firstProviderConfig := createProviderConfig(ctx, t, client, "openai", "OpenAI First", nil)
		secondProviderConfig := createProviderConfig(ctx, t, client, "openai", "OpenAI Second", nil)
		modelConfig := createChatModelConfigWithProviderIDs(ctx, t, client, codersdk.CreateChatModelConfigRequest{
			Provider: "openai",
			Model:    "gpt-4o-mini-multi-provider-configs",
		}, secondProviderConfig.ID, firstProviderConfig.ID)
		require.Len(t, modelConfig.ProviderConfigs, 2)
		requireChatModelProviderAttachment(t, modelConfig.ProviderConfigs[0], secondProviderConfig, 0)
		require.Equal(t, secondProviderConfig.DisplayName, modelConfig.ProviderConfigs[0].DisplayName)
		require.True(t, modelConfig.ProviderConfigs[0].Enabled)
		requireChatModelProviderAttachment(t, modelConfig.ProviderConfigs[1], firstProviderConfig, 1)
	...
	t.Run("DuplicateProviderConfigIDs", func(t *testing.T) {
		...
		client := newChatClient(t)
		_ = coderdtest.CreateFirstUser(t, client.Client)
		providerConfig := createProviderConfig(ctx, t, client, "openai", "OpenAI Duplicate", nil)
		_, err := client.CreateChatModelConfig(ctx, defaultCreateChatModelConfigRequest("openai", "gpt-4o-mini-duplicate-provider-configs", providerConfig.ID, providerConfig.ID))
		sdkErr := requireSDKError(t, err, http.StatusBadRequest)
		require.Equal(t, "Duplicate provider config ID.", sdkErr.Message)
	})
	t.Run("RejectsCreateWithoutProviderConfigIDsWithExistingProviderRows", func(t *testing.T) {
		t.Parallel()

		testCases := []struct {
			name  string
			setup func(context.Context, *codersdk.ExperimentalClient)
		}{
			{
				name: "EnabledProvidersExist",
				setup: func(ctx context.Context, client *codersdk.ExperimentalClient) {
					_ = createProviderConfig(ctx, t, client, "openai", "OpenAI Older", nil)
					_ = createProviderConfig(ctx, t, client, "openai", "OpenAI Newer", nil)
				},
			},
			{
				name: "NoEnabledProviderExists",
				setup: func(ctx context.Context, client *codersdk.ExperimentalClient) {
...
func TestUpdateChatModelConfig(t *testing.T) {
	...
	t.Run("ReEnableRestoresVisibilityForNonAdmins", func(t *testing.T) {
		...
		providerConfig := createProviderConfig(ctx, t, adminClient, "openai", "OpenAI Re-enable", nil)
	...
	t.Run("RebindProviderConfig", func(t *testing.T) {
		...
		}, firstProviderConfig.ID)

		updated, err := client.UpdateChatModelConfig(ctx, modelConfig.ID, codersdk.UpdateChatModelConfigRequest{
			ProviderConfigIDs: ptr.Ref([]uuid.UUID{secondProviderConfig.ID}),
		})
		require.NoError(t, err)
		require.Len(t, updated.ProviderConfigs, 1)
		requireChatModelProviderAttachment(t, updated.ProviderConfigs[0], secondProviderConfig, 0)
	...
	t.Run("UpdateReorderProviderConfigs", func(t *testing.T) {
		t.Parallel()

		ctx := testutil.Context(t, testutil.WaitLong)
		client, db := newChatClientWithDatabase(t)
		_ = coderdtest.CreateFirstUser(t, client.Client)
		firstProviderConfig := createProviderConfig(ctx, t, client, "openai", "OpenAI Reorder First", nil)
		secondProviderConfig := createProviderConfig(ctx, t, client, "openai", "OpenAI Reorder Second", nil)
		modelConfig := createChatModelConfigWithProviderIDs(ctx, t, client, codersdk.CreateChatModelConfigRequest{
			Provider: "openai",
			Model:    "gpt-4o-mini-reorder-provider-configs",
		}, firstProviderConfig.ID, secondProviderConfig.ID)

		updated, err := client.UpdateChatModelConfig(ctx, modelConfig.ID, codersdk.UpdateChatModelConfigRequest{
			ProviderConfigIDs: ptr.Ref([]uuid.UUID{secondProviderConfig.ID, firstProviderConfig.ID}),
		})
		require.NoError(t, err)
		require.Len(t, updated.ProviderConfigs, 2)
		requireChatModelProviderAttachment(t, updated.ProviderConfigs[0], secondProviderConfig, 0)
		requireChatModelProviderAttachment(t, updated.ProviderConfigs[1], firstProviderConfig, 1)
	...
	t.Run("UpdateDuplicateProviderConfigIDs", func(t *testing.T) {
		...
		client := newChatClient(t)
		_ = coderdtest.CreateFirstUser(t, client.Client)
		providerConfig, modelConfig := createProviderBackedChatModelConfig(ctx, t, client, "openai", "OpenAI Update Duplicate", codersdk.CreateChatModelConfigRequest{
			Model: "gpt-4o-mini-update-duplicate-provider-configs",
		})

		_, err := client.UpdateChatModelConfig(ctx, modelConfig.ID, codersdk.UpdateChatModelConfigRequest{
			ProviderConfigIDs: ptr.Ref([]uuid.UUID{providerConfig.ID, providerConfig.ID}),
		})
		sdkErr := requireSDKError(t, err, http.StatusBadRequest)
		require.Equal(t, "Duplicate provider config ID.", sdkErr.Message)
	})
	t.Run("OmitProviderConfigIDPreservesBinding", func(t *testing.T) {
		...
		client, db := newChatClientWithDatabase(t)
		_ = coderdtest.CreateFirstUser(t, client.Client)
		providerConfig, modelConfig := createProviderBackedChatModelConfig(ctx, t, client, "openai", "OpenAI Preserve Bound", codersdk.CreateChatModelConfigRequest{
		...
		require.Len(t, updated.ProviderConfigs, 1)
		requireChatModelProviderAttachment(t, updated.ProviderConfigs[0], providerConfig, 0)
	...
	t.Run("UnrelatedUpdatesFailWhenBoundProviderConfigIsDisabled", func(t *testing.T) {
		...
		client := newChatClient(t)
		_ = coderdtest.CreateFirstUser(t, client.Client)
		providerConfig, modelConfig := createProviderBackedChatModelConfig(ctx, t, client, "openai", "OpenAI Disabled Bound", codersdk.CreateChatModelConfigRequest{
		...
		require.Equal(t, fmt.Sprintf("provider_config_id %s is not enabled", providerConfig.ID), sdkErr.Detail)
	})

	t.Run("EmptyProviderConfigIDsRejected", func(t *testing.T) {
		t.Parallel()

		ctx := testutil.Context(t, testutil.WaitLong)
		client := newChatClient(t)
		_ = coderdtest.CreateFirstUser(t, client.Client)
		_, modelConfig := createProviderBackedChatModelConfig(ctx, t, client, "openai", "OpenAI Bound", codersdk.CreateChatModelConfigRequest{
			Model: "gpt-4o-mini-unbind",
		})

		_, err := client.UpdateChatModelConfig(ctx, modelConfig.ID, codersdk.UpdateChatModelConfigRequest{
			ProviderConfigIDs: ptr.Ref([]uuid.UUID{}),
		})
		sdkErr := requireSDKError(t, err, http.StatusBadRequest)
		require.Equal(t, "At least one provider config ID is required.", sdkErr.Message)
	})

	t.Run("ProviderChangeWithoutIDsRejected", func(t *testing.T) {
		t.Parallel()
		...
		testCases := []struct {
			...
			{
				name:         "WithAlternateProviderConfigPresent",
				model:        "gpt-4o-mini-provider-switch",
				displayName:  "OpenAI Bound",
				addAnthropic: true,
			},
			{
				name:        "WithoutMatchingProviderConfig",
				model:       "gpt-4o-mini-clear-binding",
				displayName: "OpenAI Clear Bound",
			},
		}
		...
		for _, tc := range testCases {
			...
			t.Run(tc.name, func(t *testing.T) {
				...
				_ = coderdtest.CreateFirstUser(t, client.Client)
				if tc.addAnthropic {
					_ = createProviderConfig(ctx, t, client, "anthropic", "Anthropic Bound", nil)
				}
				_, modelConfig := createProviderBackedChatModelConfig(ctx, t, client, "openai", tc.displayName, codersdk.CreateChatModelConfigRequest{Model: tc.model})

				_, err := client.UpdateChatModelConfig(ctx, modelConfig.ID, codersdk.UpdateChatModelConfigRequest{Provider: "anthropic"})
				sdkErr := requireSDKError(t, err, http.StatusBadRequest)
				require.Equal(t, "Provider config IDs are required when changing the provider.", sdkErr.Message)
			})
		}
	})
}
...
func createChatModelConfig(t *testing.T, client *codersdk.ExperimentalClient) codersdk.ChatModelConfig {
	...
	ctx := testutil.Context(t, testutil.WaitLong)
	isDefault := true
	_, modelConfig := createProviderBackedChatModelConfig(ctx, t, client, "openai", "OpenAI Default", codersdk.CreateChatModelConfigRequest{
		Model:     "gpt-4o-mini",
		IsDefault: &isDefault,
	})
	return modelConfig
