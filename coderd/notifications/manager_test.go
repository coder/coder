...
func TestBufferedUpdates(t *testing.T) {
	...
	mgr, err := notifications.NewManager(cfg, coderd.NewRuntimeConfigStore(api.Database), interceptor, defaultHelpers(), createMetrics(), api.Logger.Named("notifications-manager"))
	...
	_, err = enq.Enqueue(ctx, member.UserID, member.OrganizationID, notifications.TemplateWorkspaceDeleted, map[string]string{"nice": "true", "i": "0"}, "") // Will succeed.
	require.NoError(t, err)
	_, err = enq.Enqueue(ctx, member.UserID, member.OrganizationID, notifications.TemplateWorkspaceDeleted, map[string]string{"nice": "true", "i": "1"}, "") // Will succeed.
	require.NoError(t, err)
	_, err = enq.Enqueue(ctx, member.UserID, member.OrganizationID, notifications.TemplateWorkspaceDeleted, map[string]string{"nice": "false", "i": "2"}, "") // Will fail.
	require.NoError(t, err)
...
func TestBuildPayload(t *testing.T) {
	...
	enq, err := notifications.NewStoreEnqueuer(cfg, interceptor, helpers,
		api.Logger.Named("notifications-enqueuer"), quartz.NewReal())
	require.NoError(t, err)

	// WHEN: a notification is enqueued
	_, err = enq.Enqueue(ctx, uuid.New(), uuid.New(), notifications.TemplateWorkspaceDeleted, map[string]string{
		"name": "my-workspace",
	}, "test")
	require.NoError(t, err)
...
func TestStopBeforeRun(t *testing.T) {
	...
	mgr, err := notifications.NewManager(cfg, coderd.NewRuntimeConfigStore(api.Database),
		api.Database, defaultHelpers(), createMetrics(), api.Logger.Named("notifications-manager"))
	require.NoError(t, err)
