...
// 	_, err = enq.Enqueue(ctx, user.ID, uuid.Nil, template, map[string]string{"type": "success"}, "test") // this will succeed
// 	require.NoError(t, err)
// 	_, err = enq.Enqueue(ctx, user.ID, uuid.Nil, template, map[string]string{"type": "failure"}, "test2") // this will fail and retry (maxAttempts - 1) times
// 	require.NoError(t, err)
//
// 	mgr.Run(ctx)
//
...
// 		_, err = enq.Enqueue(ctx, user.ID, uuid.Nil, template, map[string]string{"type": "success", "i": strconv.Itoa(i)}, "test")
// 		require.NoError(t, err)
// 	}
//
// 	mgr.Run(ctx)
...
// 	user := createSampleUser(t, api.Database)
//
// 	_, err = enq.Enqueue(ctx, user.ID, uuid.Nil, template, map[string]string{"type": "success"}, "test")
// 	require.NoError(t, err)
// 	_, err = enq.Enqueue(ctx, user.ID, uuid.Nil, anotherTemplate, map[string]string{"type": "success"}, "test")
// 	require.NoError(t, err)
//
// 	mgr.Run(ctx)
//
