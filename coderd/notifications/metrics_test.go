...
func TestMetrics(t *testing.T) {
	...
	mgr, err := notifications.NewManager(cfg, coderd.NewRuntimeConfigStore(api.Database), api.Database, defaultHelpers(), metrics, api.Logger.Named("manager"))
	...
	_, err = enq.Enqueue(ctx, member.UserID, member.OrganizationID, template, map[string]string{"type": "success"}, "test") // this will succeed
	require.NoError(t, err)
	_, err = enq.Enqueue(ctx, member.UserID, member.OrganizationID, template, map[string]string{"type": "failure"}, "test2") // this will fail and retry (maxAttempts - 1) times
	require.NoError(t, err)
...
func TestPendingUpdatesMetric(t *testing.T) {
	...
	mgr, err := notifications.NewManager(cfg, coderd.NewRuntimeConfigStore(api.Database), interceptor, defaultHelpers(), metrics, api.Logger.Named("manager"))
	require.NoError(t, err)
	t.Cleanup(func() {
		assert.NoError(t, mgr.Stop(ctx))
	})
...
func TestInflightDispatchesMetric(t *testing.T) {
	...
	for i := 0; i < msgCount; i++ {
		_, err = enq.Enqueue(ctx, member.UserID, member.OrganizationID, template, map[string]string{"type": "success", "i": strconv.Itoa(i)}, "test")
		require.NoError(t, err)
	}
...
func TestCustomMethodMetricCollection(t *testing.T) {
	...
	cfg := coderdtest.DeploymentValues(t, defaultNotificationsMutator(defaultMethod)).Notifications
	mgr, err := notifications.NewManager(cfg, coderd.NewRuntimeConfigStore(api.Database), api.Database, defaultHelpers(), metrics, api.Logger.Named("manager"))
	require.NoError(t, err)
	t.Cleanup(func() {
		assert.NoError(t, mgr.Stop(ctx))
	...
	_, err = enq.Enqueue(ctx, member.UserID, member.OrganizationID, template, map[string]string{"type": "success"}, "test")
	require.NoError(t, err)
	_, err = enq.Enqueue(ctx, member.UserID, member.OrganizationID, anotherTemplate, map[string]string{"type": "success"}, "test")
	require.NoError(t, err)
...
func (d *delayingHandler) Dispatcher(payload types.MessagePayload, title, body string) (dispatch.DeliveryFunc, error) {
	...
	return func(ctx context.Context, cfgResolver runtimeconfig.Resolver, msgID uuid.UUID) (retryable bool, err error) {
		...
		return deliverFn(ctx, cfgResolver, msgID)
	}, nil
}
