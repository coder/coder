...
// 	sid, err := enq.Enqueue(ctx, user.ID, uuid.Nil, notifications.TemplateWorkspaceDeleted, map[string]string{"type": "success"}, "test")
// 	require.NoError(t, err)
// 	fid, err := enq.Enqueue(ctx, user.ID, uuid.Nil, notifications.TemplateWorkspaceDeleted, map[string]string{"type": "failure"}, "test")
// 	require.NoError(t, err)
//
// 	mgr.Run(ctx)
//
...
// 	msgID, err := enq.Enqueue(ctx, user.ID, uuid.Nil, notifications.TemplateWorkspaceDeleted, map[string]string{}, "test")
// 	require.NoError(t, err)
//
// 	mgr.Run(ctx)
//
...
// 		"c": "d",
// 	}
// 	msgID, err := enq.Enqueue(ctx, user.ID, uuid.Nil, notifications.TemplateWorkspaceDeleted, input, "test")
// 	require.NoError(t, err)
//
// 	mgr.Run(ctx)
//
...
// 		_, err = enq.Enqueue(ctx, user.ID, uuid.Nil, notifications.TemplateWorkspaceDeleted, map[string]string{"i": fmt.Sprintf("%d", i)}, "test")
// 		require.NoError(t, err)
// 	}
//
// 	// Start the notifier.
...
// 	var msgs []string
// 	for i := 0; i < msgCount; i++ {
// 		id, err := enq.Enqueue(ctx, user.ID, uuid.Nil, notifications.TemplateWorkspaceDeleted, map[string]string{"type": "success", "index": fmt.Sprintf("%d", i)}, "test")
// 		require.NoError(t, err)
...
// 	sid, err := enq.Enqueue(ctx, user.ID, uuid.Nil, notifications.TemplateWorkspaceDeleted, map[string]string{"type": "success", "i": "1"}, "test")
// 	require.NoError(t, err)
//
...
// 	_, err = enq.Enqueue(ctx, user.ID, uuid.Nil, templateID, map[string]string{}, "test")
...
// 	templateID := notifications.TemplateWorkspaceDeleted
// 	msgID, err := enq.Enqueue(ctx, user.ID, uuid.Nil, templateID, map[string]string{}, "test")
// 	require.NoError(t, err)
//
...
// 	user := createSampleUser(t, api.Database)
// 	msgID, err := enq.Enqueue(ctx, user.ID, uuid.Nil, template, map[string]string{}, "test")
// 	require.NoError(t, err)
//
...
// 	_, err = enq.Enqueue(ctx, user.ID, uuid.Nil, notifications.TemplateWorkspaceDeleted, map[string]string{"initiator": "danny"}, "test", user.ID)
// 	require.NoError(t, err)
//
...
func TestOrgLevelOverrides(t *testing.T) {
	...
	// GIVEN:
	require.NoError(t, cfg.Notifications.SMTP.From.Save(ctx, orgMutator, serpent.StringOf(ptr.Ref(orgOverriddenFrom))))
	val, err := cfg.Notifications.SMTP.From.Coalesce(orgResolver)
	require.NoError(t, err)
	require.Equal(t, orgOverriddenFrom, val.String())

	handler := newDispatchInterceptor(dispatch.NewSMTPHandler(cfg.Notifications.SMTP, defaultHelpers(), api.Logger.Named("smtp")))
	mgr, err := notifications.NewManager(cfg.Notifications, store, api.Database, defaultHelpers(), createMetrics(), api.Logger.Named("manager"))
	require.NoError(t, err)
	mgr.WithHandlers(map[database.NotificationMethod]notifications.Handler{method: handler})
	t.Cleanup(func() {
		assert.NoError(t, mgr.Stop(ctx))
	...
	// WHEN: a message is enqueued
	msgID, err := enq.Enqueue(ctx, user.ID, altOrg.ID, notifications.TemplateWorkspaceDeleted, map[string]string{}, "test")
	require.NoError(t, err)
