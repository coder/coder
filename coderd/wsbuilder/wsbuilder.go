...
func (b *Builder) getTemplate() (*database.Template, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("get template %s: %w", b.workspace.TemplateID, err)
	}
	b.template = &t
	return b.template, nil
}
...
func (b *Builder) getLastBuild() (*database.WorkspaceBuild, error) {
	...
	if err != nil {
		err = fmt.Errorf("get workspace %s last build: %w", b.workspace.ID, err)
		b.lastBuildErr = &err
		return nil, err
	}
	b.lastBuild = &bld
...
func (b *Builder) getBuildNumber() (int32, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("get last build to compute build number: %w", err)
	}
...
func (b *Builder) getState() ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("get last build to get state: %w", err)
	}
...
func (b *Builder) getLastBuildParameters() ([]database.WorkspaceBuildParameter, error) {
	...
	if err != nil && !errors.Is(err, sql.ErrNoRows) {
		return nil, fmt.Errorf("get last build %s parameters: %w", bld.ID, err)
	}
...
func (b *Builder) getTemplateVersionParameters() ([]database.TemplateVersionParameter, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("get template version ID to get parameters: %w", err)
	}
	tvp, err := b.store.GetTemplateVersionParameters(b.ctx, tvID)
	if err != nil && !errors.Is(err, sql.ErrNoRows) {
		return nil, fmt.Errorf("get template version %s parameters: %w", tvID, err)
	}
...
func (b *Builder) verifyNoLegacyParameters() error {
	...
	templateVersionJob, err := b.getTemplateVersionJob()
	if err != nil {
		return fmt.Errorf("failed to fetch template version job: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("failed to get parameter schemas: %w", err)
	}
	if len(parameterSchemas) > 0 {
		return fmt.Errorf("Legacy parameters in use on this version are not supported anymore. Contact your administrator for assistance.")
	}
	return nil
}
func (b *Builder) getLastBuildJob() (*database.ProvisionerJob, error) {
	...
	bld, err := b.getLastBuild()
	if err != nil {
		return nil, fmt.Errorf("get last build to get job: %w", err)
	}
	job, err := b.store.GetProvisionerJobByID(b.ctx, bld.JobID)
	if err != nil {
		return nil, fmt.Errorf("get build provisioner job %s: %w", bld.JobID, err)
	}
	b.lastBuildJob = &job
	return b.lastBuildJob, nil
}
...
func (b *Builder) authorize(authFunc func(action policy.Action, object rbac.Objecter) bool) error {
	...
	default:
		...
		return BuildError{http.StatusBadRequest, msg, errors.New(msg)}
	}
	if !authFunc(action, b.workspace) {
		if authFunc(policy.ActionRead, b.workspace) {
			...
			return BuildError{
				...
				Wrapped: errors.New(httpapi.ResourceForbiddenResponse.Detail),
			}
		}
		// We use the same wording as the httpapi to avoid leaking the existence of the workspace
		return BuildError{http.StatusNotFound, httpapi.ResourceNotFoundResponse.Message, errors.New(httpapi.ResourceNotFoundResponse.Message)}
	}
	template, err := b.getTemplate()
	if err != nil {
	...
	if b.logLevel != "" && b.deploymentValues != nil && !b.deploymentValues.EnableTerraformDebugMode {
		return BuildError{
			...
			errors.New("Terraform debug mode is disabled in the deployment configuration."),
		}
	}
	return nil
}
