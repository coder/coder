...
func TestPatchChatQueuedMessage(t *testing.T) {
	...
	t.Run("Guards", func(t *testing.T) {
		...
		err = codersdk.NewExperimentalClient(adminRaw).EditChatQueuedMessage(ctx, chat.ID, queued.ID, codersdk.EditChatQueuedMessageRequest{Editing: boolPtr(true)})
		...
		require.NoError(t, err)
		err = client.EditChatQueuedMessage(ctx, chat.ID, queued.ID, codersdk.EditChatQueuedMessageRequest{Editing: boolPtr(true)})
