...
// sessionCountsFromProto is the composition the batcher applies on ingest,
// without the overflow logging in between.
...
func TestSessionCountsFromProto(t *testing.T) {
	...
	t.Run("MapTakesPrecedence", func(t *testing.T) {
		...
		require.Equal(t, map[string]int64{"cursor": 2, "ssh": 1}, sessionCountsFromProto(st))
	})
	...
	t.Run("DropsNonPositiveEntries", func(t *testing.T) {
		...
		require.Equal(t, map[string]int64{"vscode": 1}, sessionCountsFromProto(st))
	})
	...
	t.Run("CapsEntriesAggregatingIntoUnknown", func(t *testing.T) {
		...
		got := sessionCountsFromProto(&agentproto.Stats{SessionCounts: sessionCounts})
	...
	t.Run("Empty", func(t *testing.T) {
		t.Parallel()
		require.Empty(t, sessionCountsFromProto(&agentproto.Stats{}))
	})
}
...
func TestHasSessionCounts(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			require.Equal(t, tc.want, HasSessionCounts(tc.st))
		})
	}
}
