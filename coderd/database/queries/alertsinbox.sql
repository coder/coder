-- name: GetInboxAlertsByUserID :many
...
SELECT * FROM inbox_alerts WHERE
	user_id = @user_id AND
	(@read_status::inbox_alert_read_status = 'all' OR (@read_status::inbox_alert_read_status = 'unread' AND read_at IS NULL) OR (@read_status::inbox_alert_read_status = 'read' AND read_at IS NOT NULL)) AND
...
-- name: GetFilteredInboxAlertsByUserID :many
-- Fetches inbox alerts for a user filtered by templates and targets
-- param user_id: The user ID
...
-- name: GetInboxAlertByID :one
SELECT * FROM inbox_alerts WHERE id = $1;

-- name: CountUnreadInboxAlertsByUserID :one
SELECT COUNT(*) FROM inbox_alerts WHERE user_id = $1 AND read_at IS NULL;

-- name: InsertInboxAlert :one
INSERT INTO
    inbox_alerts (
        id,
        user_id,
...
-- name: UpdateInboxAlertReadStatus :exec
UPDATE
    inbox_alerts
SET
    read_at = $1
...
-- name: MarkAllInboxAlertsAsRead :exec
UPDATE
	inbox_alerts
SET
	read_at = $1
