...
session_data AS (
	-- @session_counts is a JSONB array of {app name: count} objects, one
	-- per stats row, zipped with @id by position like @connections_by_proto.
	SELECT
...
-- Single-use CTEs so the planner inlines them instead of materializing
-- the stats window twice.
WITH byte_stats AS (
    SELECT
