...
WHERE
	id = (SELECT COALESCE(root.root_chat_id, root.id) FROM chats root WHERE root.id = @chat_id)
	AND occupancy_count = 0;

-- name: VacateChatTree :execrows
