...
WHERE
    ...
    CASE WHEN @resource_type::text != '' THEN
        resource_type = @resource_type::resource_type
    ELSE
        TRUE
    END
    -- Filter resource_id
    AND CASE WHEN @resource_id::uuid != '00000000-0000-0000-0000-000000000000'::uuid THEN
        resource_id = @resource_id
    ELSE
        TRUE
    END
    -- Filter by resource_target
    AND CASE WHEN @resource_target::text != '' THEN
        resource_target = @resource_target
    ELSE
        TRUE
    END
    -- Filter action
    AND CASE WHEN @action::text != '' THEN
        action = @action::audit_action
    ELSE
        TRUE
    END
    -- Filter by username
    AND CASE WHEN @username::text != '' THEN
    ...
    END
    -- Filter by date_from
    AND CASE WHEN @date_from::timestamp with time zone != '0001-01-01 00:00:00' THEN
        "time" >= @date_from
    ELSE
        TRUE
    END
    -- Filter by date_to
    AND CASE WHEN @date_to::timestamp with time zone != '0001-01-01 00:00:00' THEN
        "time" <= @date_to
    ELSE
        TRUE
    END
    -- Filter by build_reason
    AND CASE WHEN @build_reason::text != '' THEN
