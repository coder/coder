	...
	array_cat(
		...
		(
			...
			FROM
				...
				-- All org members get an implied role for their orgs. Most members get organization-member, but service accounts will get organization-service-account instead. They're largely the same, but having them be distinct means we can allow configuring service-accounts to have slightly broader permissions–such as for workspace sharing.
				unnest(
					array_append(
						roles,
