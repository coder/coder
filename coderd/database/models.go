...
// Running processes for an exit node. Each process has its own replica ID and heartbeat.
type ExitNodeReplica struct {
	// Random identifier generated by the replica process.
	ID uuid.UUID `db:"id" json:"id"`
	// Logical exit node this replica belongs to.
	ExitNodeID uuid.UUID `db:"exit_node_id" json:"exit_node_id"`
	// Hostname reported by the replica process.
	Hostname string `db:"hostname" json:"hostname"`
	// Coder version reported by the replica process.
	Version string `db:"version" json:"version"`
	// Public ip:port pairs agents may use for direct WireGuard connections to the replica.
	WireguardEndpoints []string `db:"wireguard_endpoints" json:"wireguard_endpoints"`
	// Hash of the policy the replica enforces, so coderd can flag replicas of one exit node that disagree.
	PolicyHash string `db:"policy_hash" json:"policy_hash"`
	// Time the replica was first registered.
	CreatedAt time.Time `db:"created_at" json:"created_at"`
	// Time the current replica process started.
	StartedAt time.Time `db:"started_at" json:"started_at"`
	// Last heartbeat. A replica is live while stopped_at IS NULL and updated_at is recent.
	UpdatedAt time.Time `db:"updated_at" json:"updated_at"`
	// Set by explicit deregistration; a stopped replica may not re-register.
