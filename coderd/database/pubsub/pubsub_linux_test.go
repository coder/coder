...
func TestPubsub_Disconnect(t *testing.T) {
	...
	connectionURL, closePg, err := dbtestutil.OpenContainerized(dbtestutil.DBContainerOptions{
		Port: disconnectTestPort,
	})
	require.NoError(t, err)
	defer closePg()
	db, err := sql.Open("postgres", connectionURL)
	...
	_, closeNewPg, err := dbtestutil.OpenContainerized(dbtestutil.DBContainerOptions{
		Port: disconnectTestPort,
	})
	require.NoError(t, err)
	defer closeNewPg()
