...
-- goal_order is the recency key the current-goal lookup orders by;
-- created_at cannot sit between the columns or the index cannot
-- produce the requested order.
CREATE INDEX idx_chat_goals_root_goal_order
    ON chat_goals(root_chat_id, goal_order DESC);
