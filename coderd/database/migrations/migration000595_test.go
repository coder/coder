...
// TestMigration000595TemplateUsageStatsSessionUsage covers the conversion of
...
func TestMigration000595TemplateUsageStatsSessionUsage(t *testing.T) {
	t.Parallel()
	...
	migrationSQL, err := os.ReadFile("000595_template_usage_stats_session_usage.up.sql")
	require.NoError(t, err)
...
// TestMigration000595ChainFrom589 walks the whole window this change spans,
// 589 up to 595 and back down to 589, with data present at every step. The
// isolated 595 tests start at 594, so they never see 590 converting raw
// session counts the rollup has not consumed, which is the state an upgrade
// actually finds.
//
//nolint:tparallel,paralleltest // Subtests share one database with transaction-local fixtures.
func TestMigration000595ChainFrom589(t *testing.T) {
	t.Parallel()
	...
	require.NoError(t, err)
	up595, err := os.ReadFile("000595_template_usage_stats_session_usage.up.sql")
	require.NoError(t, err)
	down595, err := os.ReadFile("000595_template_usage_stats_session_usage.down.sql")
	require.NoError(t, err)
	...
	// against, and so 595 has a row whose fixed family minutes must convert.
	_, err = tx.ExecContext(ctx, `
		INSERT INTO template_usage_stats (
			start_time, end_time, template_id, user_id, median_latency_ms,
			usage_mins, ssh_mins, sftp_mins, reconnecting_pty_mins,
	...
	for _, step := range []chainStep{{"590 up", up590}, {"595 up", up595}} {
	...
	// 595 converted the fixed family minutes and recorded no per-app usage.
	require.Equal(t, []familyRow{{"sftp", 2}, {"ssh", 3}, {"vscode", 4}},
		sessionRows(t, tx, "template_usage_stats_session_families", "family"))
	require.Empty(t, sessionRows(t, tx, "template_usage_stats_session_apps", "app_name"))

	// Back down: 595 restores the fixed columns, then 590 restores the fixed
	// session counts, landing on the 589 schema.
	for _, step := range []chainStep{{"595 down", down595}, {"590 down", down590}} {
		_, err = tx.ExecContext(ctx, string(step.sql))
		require.NoError(t, err, "%s", step.name)
	}
