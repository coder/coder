...
// TestMigration000592LegacyGoalRows verifies the continuation migration
...
func TestMigration000592LegacyGoalRows(t *testing.T) {
	t.Parallel()
	...
	fixtures := []struct {
	...
	}{
		{
			`INSERT INTO ai_providers (id, type, name, enabled, base_url, created_at, updated_at)
			VALUES ($1, $2, $3, $4, $5, $6, $7)`,
			[]any{providerID, "openai", "openai-592", true, "https://api.openai.com/v1", now, now},
		},
		{
			...
			[]any{modelConfigID, "gpt-5.2", "Model 592", true, 200000, 70, providerID, now},
		},
		{
			...
			[]any{activeChatID, modelConfigID, "Active goal 592", "waiting", now},
		},
		{
			`INSERT INTO chats (id, owner_id, last_model_config_id, organization_id, title, status, created_at, updated_at)
			SELECT $1, u.id, $2, o.id, $3, $4::chat_status, $5, $5
			FROM users u, organizations o
			ORDER BY u.created_at, u.id, o.created_at, o.id LIMIT 1`,
			[]any{pausedChatID, modelConfigID, "Paused goal 592", "waiting", now},
		},
		{
			`INSERT INTO chats (id, owner_id, last_model_config_id, organization_id, title, status, created_at, updated_at)
			SELECT $1, u.id, $2, o.id, $3, $4::chat_status, $5, $5
			FROM users u, organizations o
			ORDER BY u.created_at, u.id, o.created_at, o.id LIMIT 1`,
			[]any{runningChatID, modelConfigID, "Running goal 592", "running", now},
		},
		{
			`INSERT INTO chats (id, owner_id, last_model_config_id, organization_id, title, status, created_at, updated_at)
			SELECT $1, u.id, $2, o.id, $3, $4::chat_status, $5, $5
			FROM users u, organizations o
			ORDER BY u.created_at, u.id, o.created_at, o.id LIMIT 1`,
			[]any{errorChatID, modelConfigID, "Errored goal 592", "error", now},
		},
		{
