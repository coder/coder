...
// TestMigration000543OAuth2PublicClientTokensBackfill seeds a pre-migration
...
func TestMigration000543OAuth2PublicClientTokensBackfill(t *testing.T) {
	t.Parallel()
	...
	secretID := uuid.New()
	tokenID := uuid.New()
	const apiKeyID = "test543apikeyid"
	...
	_, err = tx.ExecContext(ctx, `
		INSERT INTO users (id, username, email, hashed_password, created_at, updated_at, status, rbac_roles, login_type)
		VALUES ($1, 'test-user-543', 'test-543@example.com', ''::bytea, $2, $2, 'active', '{}', 'password')
	`, userID, now)
	require.NoError(t, err)
	...
	_, err = tx.ExecContext(ctx, `
		...
		VALUES ($1, $2, $3, 'prefix543'::bytea, ''::bytea, $4, $5, $6)
	`, tokenID, now, now.Add(time.Hour), secretID, apiKeyID, userID)
	require.NoError(t, err)

	require.NoError(t, tx.Commit())

	migrationSQL, err := os.ReadFile("000543_oauth2_public_client_tokens.up.sql")
	require.NoError(t, err)
	_, err = sqlDB.ExecContext(ctx, string(migrationSQL))
	require.NoError(t, err)
