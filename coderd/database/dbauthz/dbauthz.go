...
func (q *querier) SetExternalAuthLinkRefreshLease(ctx context.Context, arg database.SetExternalAuthLinkRefreshLeaseParams) error {
	fetch := func(ctx context.Context, arg database.SetExternalAuthLinkRefreshLeaseParams) (database.ExternalAuthLink, error) {
		return q.db.GetExternalAuthLink(ctx, database.GetExternalAuthLinkParams{UserID: arg.UserID, ProviderID: arg.ProviderID})
	}
	return fetchAndExec(q.log, q.auth, policy.ActionUpdatePersonal, fetch, q.db.SetExternalAuthLinkRefreshLease)(ctx, arg)
}
