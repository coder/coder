...
func (q *querier) UpdateMemberRoles(ctx context.Context, arg database.UpdateMemberRolesParams) (database.OrganizationMember, error) {
	...
	members, err := q.OrganizationMembers(ctx, database.OrganizationMembersParams{
	...
	if len(members) != 1 {
		return database.OrganizationMember{}, xerrors.Errorf("too many rows returned, expected 1")
	}
