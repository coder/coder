...
func (s *MethodTestSuite) TestChats() {
	...
	s.Run("AdvanceChatHistoryVersion", s.Mocked(func(dbm *dbmock.MockStore, faker *gofakeit.Faker, check *expects) {
		chat := testutil.Fake(s.T(), faker, database.Chat{})
		dbm.EXPECT().GetChatByID(gomock.Any(), chat.ID).Return(chat, nil).AnyTimes()
		dbm.EXPECT().AdvanceChatHistoryVersion(gomock.Any(), chat.ID).Return(nil).AnyTimes()
