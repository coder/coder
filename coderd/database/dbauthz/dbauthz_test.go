...
func (s *MethodTestSuite) TestChats() {
	...
	s.Run("SearchChatMessages", s.Mocked(func(dbm *dbmock.MockStore, faker *gofakeit.Faker, check *expects) {
		chat := testutil.Fake(s.T(), faker, database.Chat{})
		rows := []database.SearchChatMessagesRow{testutil.Fake(s.T(), faker, database.SearchChatMessagesRow{})}
		arg := database.SearchChatMessagesParams{ChatID: chat.ID, Query: "needle", LimitVal: 10, ContextChars: 160, ExcerptChars: 326}
		dbm.EXPECT().GetChatByID(gomock.Any(), chat.ID).Return(chat, nil).AnyTimes()
		dbm.EXPECT().SearchChatMessages(gomock.Any(), arg).Return(rows, nil).AnyTimes()
