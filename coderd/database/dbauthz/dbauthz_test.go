...
func (s *MethodTestSuite) TestChats() {
	...
	s.Run("GetChatDebugLoggingEnabled", s.Mocked(func(dbm *dbmock.MockStore, _ *gofakeit.Faker, check *expects) {
		dbm.EXPECT().GetChatDebugLoggingEnabled(gomock.Any()).Return(true, nil).AnyTimes()
	...
	s.Run("GetChatDebugRunsByChat", s.Mocked(func(dbm *dbmock.MockStore, faker *gofakeit.Faker, check *expects) {
		chat := testutil.Fake(s.T(), faker, database.Chat{})
		runs := []database.ChatDebugRun{{ID: uuid.New(), ChatID: chat.ID}}
		dbm.EXPECT().GetChatByID(gomock.Any(), chat.ID).Return(chat, nil).AnyTimes()
		dbm.EXPECT().GetChatDebugRunsByChat(gomock.Any(), chat.ID).Return(runs, nil).AnyTimes()
		check.Args(chat.ID).Asserts(chat, policy.ActionRead).Returns(runs)
	}))
	...
	s.Run("InsertChatDebugRun", s.Mocked(func(dbm *dbmock.MockStore, faker *gofakeit.Faker, check *expects) {
		chat := testutil.Fake(s.T(), faker, database.Chat{})
		arg := database.InsertChatDebugRunParams{ChatID: chat.ID, Kind: "chat_turn", Status: "running"}
		run := database.ChatDebugRun{ID: uuid.New(), ChatID: chat.ID}
		dbm.EXPECT().GetChatByID(gomock.Any(), chat.ID).Return(chat, nil).AnyTimes()
	...
	s.Run("InsertChatDebugStep", s.Mocked(func(dbm *dbmock.MockStore, faker *gofakeit.Faker, check *expects) {
		chat := testutil.Fake(s.T(), faker, database.Chat{})
		arg := database.InsertChatDebugStepParams{RunID: uuid.New(), ChatID: chat.ID, StepNumber: 1, Operation: "stream", Status: "running"}
	...
	s.Run("UpdateChatDebugLogsEnabledOverride", s.Mocked(func(dbm *dbmock.MockStore, faker *gofakeit.Faker, check *expects) {
		chat := testutil.Fake(s.T(), faker, database.Chat{})
		arg := database.UpdateChatDebugLogsEnabledOverrideParams{
			ID:                       chat.ID,
			DebugLogsEnabledOverride: sql.NullBool{Bool: true, Valid: true},
		}
		updatedChat := testutil.Fake(s.T(), faker, database.Chat{ID: chat.ID})
		dbm.EXPECT().GetChatByID(gomock.Any(), chat.ID).Return(chat, nil).AnyTimes()
		dbm.EXPECT().UpdateChatDebugLogsEnabledOverride(gomock.Any(), arg).Return(updatedChat, nil).AnyTimes()
	...
	s.Run("UpsertChatDebugLoggingEnabled", s.Mocked(func(dbm *dbmock.MockStore, _ *gofakeit.Faker, check *expects) {
		dbm.EXPECT().UpsertChatDebugLoggingEnabled(gomock.Any(), true).Return(nil).AnyTimes()
