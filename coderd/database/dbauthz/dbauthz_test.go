...
func (s *MethodTestSuite) TestAPIKey() {
	...
	s.Run("UpdateAPIKeyAuthorization", s.Mocked(func(dbm *dbmock.MockStore, faker *gofakeit.Faker, check *expects) {
		original := testutil.Fake(s.T(), faker, database.APIKey{})
		params := database.UpdateAPIKeyAuthorizationParams{
			ID:              original.ID,
		...
		dbm.EXPECT().UpdateAPIKeyAuthorization(gomock.Any(), params).Return(updated, nil).AnyTimes()
