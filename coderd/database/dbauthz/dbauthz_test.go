...
func (s *MethodTestSuite) TestChats() {
	...
	s.Run("GetChatGoalMessageIDsByRootChatAndMessageIDs", s.Mocked(func(dbm *dbmock.MockStore, faker *gofakeit.Faker, check *expects) {
		chat := testutil.Fake(s.T(), faker, database.Chat{})
		goalMessageIDs := []int64{2}
		arg := database.GetChatGoalMessageIDsByRootChatAndMessageIDsParams{RootChatID: chat.ID, MessageIds: []int64{1, 2}}
		dbm.EXPECT().GetChatByID(gomock.Any(), chat.ID).Return(chat, nil).AnyTimes()
		dbm.EXPECT().GetChatGoalMessageIDsByRootChatAndMessageIDs(gomock.Any(), arg).Return(goalMessageIDs, nil).AnyTimes()
