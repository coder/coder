...
func (s *MethodTestSuite) TestChats() {
	...
	s.Run("UpdateChatDisabledWorkspaceMCPServersByID", s.Mocked(func(dbm *dbmock.MockStore, faker *gofakeit.Faker, check *expects) {
		chat := testutil.Fake(s.T(), faker, database.Chat{})
		arg := database.UpdateChatDisabledWorkspaceMCPServersByIDParams{
			ID:                          chat.ID,
			DisabledWorkspaceMCPServers: []string{"github"},
		}
		dbm.EXPECT().GetChatByID(gomock.Any(), chat.ID).Return(chat, nil).AnyTimes()
		dbm.EXPECT().UpdateChatDisabledWorkspaceMCPServersByID(gomock.Any(), arg).Return(chat, nil).AnyTimes()
		check.Args(arg).Asserts(chat, policy.ActionUpdate).Returns(chat)
	}))
