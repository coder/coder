...
//nolint:paralleltest // It uses LockIDDBPurge.
func TestDeleteCompromisedModuleCacheFiles(t *testing.T) {
	ctx := testutil.Context(t, testutil.WaitShort)
	clk := quartz.NewMock(t)
	...
	// Compromised: a provisionerd module archive cached inside the window.
	compromised, compromisedTV := mkVersion("compromised", uuid.Nil, "application/x-tar", inWindow)
	...
	// then the compromised archives are deleted and their references cleared
	assertFileDeleted(compromised.ID, "archive inside the window")
	assertCacheRef(compromisedTV, uuid.Nil, false, "archive inside the window")
