...
func main() {
	...
	if connection == "" {
		...
		if err != nil {
			err = fmt.Errorf("open containerized database failed: %w", err)
			panic(err)
		}
	...
	if err != nil {
		err = fmt.Errorf("open database failed: %w", err)
		panic(err)
	}
	defer db.Close()
	err = migrations.Up(db)
	if err != nil {
		err = fmt.Errorf("run migrations failed: %w", err)
		panic(err)

	}
	dumpBytes, err := dbtestutil.PGDumpSchemaOnly(connection)
	if err != nil {
		err = fmt.Errorf("dump schema failed: %w", err)
		panic(err)
	}
	...
	if err != nil {
		err = fmt.Errorf("write dump failed: %w", err)
