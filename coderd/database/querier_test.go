...
func TestConnectionLogsOffsetFilters(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			name: "StartedAfter",
			params: database.GetConnectionLogsOffsetParams{
				StartedAfter: now.Add(-90 * time.Minute), // 1.5 hours ago
			},
			expectedLogIDs: []uuid.UUID{log4.ID},
		},
		{
			name: "StartedBefore",
			params: database.GetConnectionLogsOffsetParams{
				StartedBefore: now.Add(-150 * time.Minute),
			},
			expectedLogIDs: []uuid.UUID{log1.ID, log2.ID},
		},
		{
			name: "ClosedAfter",
			params: database.GetConnectionLogsOffsetParams{
				ClosedAfter: now.Add(-90 * time.Minute),
			},
		...
		},
		{
			name: "ClosedBefore",
			params: database.GetConnectionLogsOffsetParams{
				ClosedBefore: now.Add(-90 * time.Minute),
			},
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			count, err := db.CountConnectionLogs(ctx, database.CountConnectionLogsParams{
				...
				ClosedAfter:    tc.params.ClosedAfter,
				ClosedBefore:   tc.params.ClosedBefore,
