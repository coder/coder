...
func seedOrganizationAISpend(t *testing.T, db database.Store) organizationAISpendFixture {
	...
	claudeCode := sql.NullString{String: "claude-code", Valid: true}
	cursor := sql.NullString{String: "cursor", Valid: true}
	for _, u := range []usage{
		// alice: 1500 priced plus one unpriced usage without a recorded client.
		{user: alice, group: inGroup, at: start, providerName: "anthropic-prod", model: "claude", client: claudeCode, cost: priced(1000)},
		...
		intc := dbgen.AIBridgeInterception(t, db, database.InsertAIBridgeInterceptionParams{
			InitiatorID: u.user.ID, Provider: "provider", ProviderName: u.providerName, Model: u.model, StartedAt: u.at, Client: u.client,
		}, nil)
		dbgen.AIBridgeTokenUsage(t, db, database.InsertAIBridgeTokenUsageParams{
...
func TestListOrganizationAISpendUsers(t *testing.T) {
	...
	t.Run("AllUsers", func(t *testing.T) {
		...
		require.Equal(t, []database.ListOrganizationAISpendUsersRow{
			row(bob, 3000, 0, 4, 6100, 1),
			row(alice, 1500, 1, 4, 6100, 1),
			row(carol, 1500, 0, 4, 6100, 1),
			row(dave, 100, 0, 4, 6100, 1),
		}, rows)
	})
	...
	t.Run("Filters", func(t *testing.T) {
		...
		cases := []struct {
		...
		}{
			{
				...
				want:   []database.ListOrganizationAISpendUsersRow{row(alice, 500, 1, 1, 500, 1)},
			},
			{
				name:   "Model",
				mutate: func(p *database.ListOrganizationAISpendUsersParams) { p.Model = "claude" },
				want: []database.ListOrganizationAISpendUsersRow{
					row(bob, 3000, 0, 4, 5600, 0),
					row(carol, 1500, 0, 4, 5600, 0),
					row(alice, 1000, 0, 4, 5600, 0),
					row(dave, 100, 0, 4, 5600, 0),
				},
			},
			{
				name:   "Client",
				...
				want: []database.ListOrganizationAISpendUsersRow{
					row(bob, 3000, 0, 2, 4500, 0),
					row(carol, 1500, 0, 2, 4500, 0),
				},
			},
			{
				...
				want: []database.ListOrganizationAISpendUsersRow{
					row(alice, 500, 1, 2, 600, 1),
					row(dave, 100, 0, 2, 600, 1),
				},
			},
			{
				name: "Combined",
				mutate: func(p *database.ListOrganizationAISpendUsersParams) {
					...
					p.Model = "claude"
					p.Client = "claude-code"
				},
				want: []database.ListOrganizationAISpendUsersRow{row(alice, 1000, 0, 1, 1000, 0)},
			},
			{
				name:   "NoMatch",
