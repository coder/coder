...
func TestAIBridgeSpendSortTies(t *testing.T) {
	...
	for _, order := range []string{"asc", "desc"} {
		for _, offset := range []int32{0, 1} {
			...
			rows, err := db.ListAIBridgeSpendPerUser(ctx, database.ListAIBridgeSpendPerUserParams{
...
func TestAIBridgeSpend(t *testing.T) {
	...
	aliceTotals := database.ListAIBridgeSpendPerUserRow{
	...
	t.Run("ListByUser", func(t *testing.T) {
		...
		require.Equal(t, database.ListAIBridgeSpendPerUserRow{
			UserID:                bob.ID,
			Username:              "bob",
			Name:                  "Bob Builder",
	...
	t.Run("ListByUserSearch", func(t *testing.T) {
		t.Parallel()
		ctx := testutil.Context(t, testutil.WaitLong)
		byName, err := db.ListAIBridgeSpendPerUser(ctx, database.ListAIBridgeSpendPerUserParams{
			StartDate: start,
			EndDate:   end,
			Search:    "LIDD",
			PageLimit: 10,
		...
		byUsername, err := db.ListAIBridgeSpendPerUser(ctx, database.ListAIBridgeSpendPerUserParams{
			StartDate: start,
			EndDate:   end,
			Search:    "bob",
			PageLimit: 10,
		...
		none, err := db.ListAIBridgeSpendPerUser(ctx, database.ListAIBridgeSpendPerUserParams{
			StartDate: start,
			EndDate:   end,
			Search:    "carol",
			PageLimit: 10,
	...
	t.Run("ListByUserPagination", func(t *testing.T) {
		t.Parallel()
		ctx := testutil.Context(t, testutil.WaitLong)
		first, err := db.ListAIBridgeSpendPerUser(ctx, database.ListAIBridgeSpendPerUserParams{
			StartDate: start,
			EndDate:   end,
			PageLimit: 1,
		})
		...
		second, err := db.ListAIBridgeSpendPerUser(ctx, database.ListAIBridgeSpendPerUserParams{
			StartDate:  start,
			EndDate:    end,
			PageLimit:  1,
			PageOffset: 1,
