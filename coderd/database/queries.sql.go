...
WHERE
	user_id = $1
	AND group_id = ANY(
		SELECT id FROM groups WHERE organization_id = $2
	)
`

type RemoveUserFromAllGroupsParams struct {
	UserID         uuid.UUID `db:"user_id" json:"user_id"`
	OrganizationID uuid.UUID `db:"organization_id" json:"organization_id"`
}

func (q *sqlQuerier) RemoveUserFromAllGroups(ctx context.Context, arg RemoveUserFromAllGroupsParams) error {
	_, err := q.db.ExecContext(ctx, removeUserFromAllGroups, arg.UserID, arg.OrganizationID)
	return err
}
