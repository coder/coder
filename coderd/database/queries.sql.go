...
WHERE
	...
	AND CASE
		WHEN $5 :: text != '' THEN
			workspaces.owner_id = (SELECT id FROM users WHERE lower(username) = lower($5) AND deleted = false)
		ELSE true
	END
	...
	AND CASE
		WHEN $6 :: text != '' THEN
			workspaces.template_id = ANY(SELECT id FROM templates WHERE lower(name) = lower($6) AND deleted = false)
		ELSE true
	END
	-- Filter by template_ids
	AND CASE
		WHEN array_length($7 :: uuid[], 1) > 0 THEN
			workspaces.template_id = ANY($7)
		ELSE true
	END
	...
	AND CASE
		  WHEN $14 :: timestamp with time zone > '0001-01-01 00:00:00Z' THEN
				  workspaces.last_used_at >= $14
		  ELSE true
	END
  	AND CASE
		  WHEN $15 :: boolean IS NOT NULL THEN
			  (latest_build.template_version_id = template.active_version_id) = $15 :: boolean
		  ELSE true
	END
	...
	ORDER BY
		...
		CASE WHEN owner_id = $16 AND favorite THEN 0 ELSE 1 END ASC,
