...
SELECT tvp.id, tvp.template_version_id, tvp.name, tvp.created_at, tvp.desired_instances, tvp.invalidate_after_secs, tvp.prebuild_status, tvp.autoscaling_enabled, tvp.autoscaling_timezone, tv.template_id, tv.organization_id FROM
...
SELECT
	id, template_version_id, name, created_at, desired_instances, invalidate_after_secs, prebuild_status, autoscaling_enabled, autoscaling_timezone
FROM
	template_version_presets
WHERE
...
VALUES (
    ...
    $7,
    $8
) RETURNING id, template_version_id, name, created_at, desired_instances, invalidate_after_secs, prebuild_status, autoscaling_enabled, autoscaling_timezone
`
...
const insertPresetPrebuildSchedule = `-- name: InsertPresetPrebuildSchedule :many
...
func (q *sqlQuerier) InsertPresetPrebuildSchedule(ctx context.Context, arg InsertPresetPrebuildScheduleParams) ([]TemplateVersionPresetPrebuildSchedule, error) {
	rows, err := q.db.QueryContext(ctx, insertPresetPrebuildSchedule, arg.PresetID, arg.CronExpression, arg.Instances)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
