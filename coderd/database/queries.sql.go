...
func (q *sqlQuerier) GetUsersCount(ctx context.Context, arg GetUsersCountParams) (int64, error) {
...
SELECT
	id, email, username, hashed_password, created_at, updated_at, status, rbac_roles, login_type, avatar_url, deleted, last_seen_at -- @gen_copy GetUsersCount :one COUNT(*)
FROM
	users
WHERE
	users.deleted = $1
...
const getWorkspacesCount = `-- name: GetWorkspacesCount :one
SELECT
	COUNT(workspaces.*)
FROM
	workspaces
...
type GetWorkspacesCountParams struct {
	Deleted       bool        `db:"deleted" json:"deleted"`
...
func (q *sqlQuerier) GetWorkspacesCount(ctx context.Context, arg GetWorkspacesCountParams) (int64, error) {
	row := q.db.QueryRowContext(ctx, getWorkspacesCount,
		arg.Deleted,
		arg.Status,
		arg.OwnerID,
		arg.OwnerUsername,
...
SELECT
	workspaces.id, workspaces.created_at, workspaces.updated_at, workspaces.owner_id, workspaces.organization_id, workspaces.template_id, workspaces.deleted, workspaces.name, workspaces.autostart_schedule, workspaces.ttl, workspaces.last_used_at -- @gen_copy GetWorkspacesCount :one COUNT(workspaces.*)
FROM
	workspaces
LEFT JOIN LATERAL (
	SELECT
