...
SET
    ...
    response_message_id = $3::bigint,
    first_message_id = $4::bigint,
    last_message_id = $5::bigint,
    input_tokens = $6::integer,
    output_tokens = $7::integer,
    total_tokens = $8::integer,
    reasoning_tokens = $9::integer,
    cache_creation_tokens = $10::integer,
    cache_read_tokens = $11::integer,
    context_limit = $12::integer,
    total_cost_micros = $13::bigint,
    tool_calls_total = $14::integer,
    tool_calls_completed = $15::integer,
    tool_calls_errored = $16::integer
WHERE
    id = $17::uuid
...
RETURNING
    id, chat_run_id, chat_id, number, model_config_id, started_at, heartbeat_at, completed_at, interrupted_at, error, continuation_reason, response_message_id, first_message_id, last_message_id, input_tokens, output_tokens, total_tokens, reasoning_tokens, cache_creation_tokens, cache_read_tokens, context_limit, total_cost_micros, tool_calls_total, tool_calls_completed, tool_calls_errored
`
...
SELECT
    s.id, s.chat_run_id, s.chat_id, s.number, s.model_config_id, s.started_at, s.heartbeat_at, s.completed_at, s.interrupted_at, s.error, s.continuation_reason, s.response_message_id, s.first_message_id, s.last_message_id, s.input_tokens, s.output_tokens, s.total_tokens, s.reasoning_tokens, s.cache_creation_tokens, s.cache_read_tokens, s.context_limit, s.total_cost_micros, s.tool_calls_total, s.tool_calls_completed, s.tool_calls_errored,
...
SELECT
    id, owner_id, workspace_id, title, created_at, updated_at, parent_chat_id, root_chat_id, last_model_config_id, archived, mode, last_run_number
FROM
    chats
WHERE
    id = $1::uuid
...
SELECT id, owner_id, workspace_id, title, created_at, updated_at, parent_chat_id, root_chat_id, last_model_config_id, archived, mode, last_run_number FROM chats WHERE id = $1::uuid FOR UPDATE
`
...
type GetChatCostPerChatRow struct {
	...
	StepCount                int64     `db:"step_count" json:"step_count"`
...
type GetChatCostSummaryRow struct {
	TotalCostMicros          int64 `db:"total_cost_micros" json:"total_cost_micros"`
	PricedStepCount          int64 `db:"priced_step_count" json:"priced_step_count"`
	UnpricedStepCount        int64 `db:"unpriced_step_count" json:"unpriced_step_count"`
	TotalInputTokens         int64 `db:"total_input_tokens" json:"total_input_tokens"`
	TotalOutputTokens        int64 `db:"total_output_tokens" json:"total_output_tokens"`
	TotalCacheReadTokens     int64 `db:"total_cache_read_tokens" json:"total_cache_read_tokens"`
	TotalCacheCreationTokens int64 `db:"total_cache_creation_tokens" json:"total_cache_creation_tokens"`
...
SELECT
    id, chat_id, model_config_id, created_at, role, content, visibility, compressed, created_by, content_version, chat_run_id, chat_run_step_id
FROM
    chat_messages
WHERE
    id = $1::bigint
...
SELECT
    id, owner_id, workspace_id, title, created_at, updated_at, parent_chat_id, root_chat_id, last_model_config_id, archived, mode, last_run_number, computed_status, last_run_error, last_run_id
FROM
    chats_with_status
WHERE
    id = $1::uuid
