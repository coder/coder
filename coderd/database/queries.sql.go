            ...
            WHERE id IN (SELECT nm.id
                         ...
                         ORDER BY nm.created_at ASC
                             ...
                             FOR UPDATE OF nm
                                 SKIP LOCKED
                         LIMIT $4)
            RETURNING id, notification_template_id, user_id, method, status, status_reason, created_by, payload, attempt_count, targets, created_at, updated_at, leased_until, next_retry_after, queued_seconds, dedupe_hash, org_id)
SELECT
    -- message
    nm.id,
    nm.payload,
...
SELECT id, notification_template_id, user_id, method, status, status_reason, created_by, payload, attempt_count, targets, created_at, updated_at, leased_until, next_retry_after, queued_seconds, dedupe_hash, org_id
FROM notification_messages
WHERE status = $1
LIMIT $2::int
`
