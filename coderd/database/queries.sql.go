...
const getUsersWithUserAdminPrivileges = `-- name: GetUsersWithUserAdminPrivileges :many
SELECT id, username
FROM users
WHERE
    deleted = 'f'
...
type GetUsersWithUserAdminPrivilegesRow struct {
	ID       uuid.UUID `db:"id" json:"id"`
	Username string    `db:"username" json:"username"`
}
...
func (q *sqlQuerier) GetUsersWithUserAdminPrivileges(ctx context.Context) ([]GetUsersWithUserAdminPrivilegesRow, error) {
	rows, err := q.db.QueryContext(ctx, getUsersWithUserAdminPrivileges)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetUsersWithUserAdminPrivilegesRow
	for rows.Next() {
		var i GetUsersWithUserAdminPrivilegesRow
