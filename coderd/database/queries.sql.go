...
SELECT tvp.id, tvp.template_version_id, tvp.name, tvp.created_at, tvp.desired_instances, tvp.invalidate_after_secs, tvp.prebuild_status, tvp.is_default, tv.template_id, tv.organization_id FROM
...
SELECT
	id, template_version_id, name, created_at, desired_instances, invalidate_after_secs, prebuild_status, is_default
FROM
	template_version_presets
WHERE
...
VALUES (
	...
	$6,
	$7
) RETURNING id, template_version_id, name, created_at, desired_instances, invalidate_after_secs, prebuild_status, is_default
`
