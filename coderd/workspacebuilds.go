...
func (api *API) postWorkspaceBuildsInternal(
	...
	apiBuild, err := api.convertWorkspaceBuild(
		*workspaceBuild,
		workspace,
		queuePos,
		groupWorkspaceBuildChildren(nil, nil, nil, nil, nil, nil, nil),
...
func (api *API) workspaceBuildsData(ctx context.Context, workspaceBuilds []database.WorkspaceBuild) (workspaceBuildsData, error) {
	...
	// Cheap lookup first, so we only ask for queue position on jobs that are
	// actually pending. For non-pending jobs the expensive query returns
	// queue_position=0, queue_size=0 anyway (via its LEFT JOIN + COALESCE),
	// so we synthesize the same shape locally.
...
// convertWorkspaceBuild converts a single workspace build using pre-built
// per-page groupings. Single-build callers can construct a groupings struct
// inline via groupWorkspaceBuildChildren; convertWorkspaceBuilds builds it
// once per page and reuses it across builds. The provisionerDaemons slice is
// filtered per-build here (it is small relative to the other slices).
