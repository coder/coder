...
func TestPostWorkspaceBuildsOnSuccessUnpinnedChildUsesActiveTemplateVersion(t *testing.T) {
	...
	client, provisionerCloser, api := coderdtest.NewWithAPI(t, &coderdtest.Options{IncludeProvisionerDaemon: true})
	db := api.Database
	first := coderdtest.CreateFirstUser(t, client)
	...
	// WHEN: a non-template-admin queues an on_success child build
	// with a requested child version pin.
	...
	// THEN: the requested child version pin is not persisted.
	orchestration, err := db.GetWorkspaceBuildOrchestrationByParentBuildID(dbauthz.AsSystemRestricted(ctx), stopBuild.ID)
	require.NoError(t, err)
	require.False(t, orchestration.ChildTemplateVersionID.Valid, "requested child version pin should not have been persisted")
...
func TestPostWorkspaceBuildsOnSuccessRetryableChildBuildFailureDoesNotBlockLaterRestart(t *testing.T) {
	...
	require.Eventually(t, func() bool {
		badOrchestration, err = db.GetWorkspaceBuildOrchestrationByParentBuildID(dbauthz.AsSystemRestricted(ctx), badStopBuild.ID)
		return err == nil &&
