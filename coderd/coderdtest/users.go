...
// TestUsersPagination creates a set of users for testing paginaton.  It can be
// used to test paginating both users and group members.
func TestUsersPagination(
	ctx context.Context,
	t *testing.T,
...
// TestUsersFilter creates a set of users to run various filters against for
// testing.  It can be used to test filtering both users and group members.
func TestUsersFilter(
	...
	for i := range 15 {
		...
		userClient, userData := CreateAnotherUserMutators(t, client, orgID, roles, func(r *codersdk.CreateUserRequestWithOrgs) {
			if i%7 == 0 {
				r.Username += fmt.Sprintf("-github-%d", i)
	...
	testCases := []struct {
		...
		},
		{
			...
			FilterF: func(_ codersdk.UsersRequest, u codersdk.User) bool {
				return strings.HasSuffix(u.Username, "-github-7")
			},
		},
		{
