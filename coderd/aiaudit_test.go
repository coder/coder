...
func TestAIAuditAgents(t *testing.T) {
	...
	t.Run("SponsorScope", func(t *testing.T) {
		...
		agent := seedAIAgent(ctx, t, db, owner.OrganizationID, member.ID)
		...
		require.Equal(t, member.ID, agents[0].OwnerID)
		require.Equal(t, string(entity.CreationSiteTypeWorkspace), agents[0].CreationSiteType)
		require.NotEmpty(t, agents[0].DisplayName)
		require.Equal(t, string(entity.AIAgentStateActive), agents[0].State)
...
func TestAIAuditTimeline(t *testing.T) {
	...
	t.Run("EndToEnd", func(t *testing.T) {
		...
		_, err = db.InsertAISandboxNetworkEvents(sysCtx, database.InsertAISandboxNetworkEventsParams{
			...
			AIAgentID:      []uuid.UUID{agent.ID, agent.ID, agent.ID, agent.ID},
		...
		require.Equal(t, agent.ID, toolCall.AIAgentID)
...
func seedAIAgent(ctx context.Context, t *testing.T, db database.Store, orgID, ownerID uuid.UUID) database.AIAgentLedger {
	t.Helper()

	created, err := aiagentidentity.Create(ctx, db, aiagentidentity.CreateParams{
		OwnerID:        ownerID,
	...
	require.NoError(t, err)
	//nolint:gocritic // Unit test reads the ledger row it just created.
	agent, err := db.GetAIAgentLedgerRowByID(dbauthz.AsSystemRestricted(ctx), created.ID)
	require.NoError(t, err)
	return agent
}
