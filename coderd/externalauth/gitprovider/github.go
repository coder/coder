...
// parseRetryAfter extracts the retry-after time from GitHub rate-limit
// headers. Returns zero time if no recognizable header is present.
func parseRetryAfter(h http.Header) time.Time {
	...
	return time.Time{} // zero — no rate-limit header found
}
