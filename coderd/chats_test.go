...
func TestWebSearchSourceCitations(t *testing.T) {
	...
	// Load ANTHROPIC_API_KEY from the environment, falling back to
	// ~/.env.local so the test works in local dev without exporting
	// the variable by hand.
	apiKey := os.Getenv("ANTHROPIC_API_KEY")
	if apiKey == "" {
		envPath := filepath.Join(os.Getenv("HOME"), ".env.local")
	...
	// Create an Anthropic provider backed by a real API key.
	_, err := client.CreateChatProvider(ctx, codersdk.CreateChatProviderConfigRequest{
		Provider: "anthropic",
		APIKey:   apiKey,
	})
	...
	// Create a model config for Claude with web search enabled.
	isDefault := true
	...
	// Create a chat that will trigger web search.
	chat, err := client.CreateChat(ctx, codersdk.CreateChatRequest{
		Content: []codersdk.ChatInputPart{
			{
				Type: codersdk.ChatInputPartTypeText,
				Text: "What is the current date today? Use web search.",
			},
		},
	})
	require.NoError(t, err)

	// Stream all events until the chat reaches "waiting" status.
	events, closer, err := client.StreamChat(ctx, chat.ID, nil)
	require.NoError(t, err)
	defer closer.Close()

	var allEvents []codersdk.ChatStreamEvent
	for {
		select {
		case <-ctx.Done():
			require.FailNow(t, "timed out waiting for chat to finish")
		case event, ok := <-events:
			if !ok {
				require.FailNow(t, "stream closed before chat reached waiting status")
			}
			allEvents = append(allEvents, event)

			if event.Type == codersdk.ChatStreamEventTypeError && event.Error != nil {
				t.Fatalf("stream error: %s", event.Error.Message)
			}

			// The chat worker sets status to "waiting" when the
	...
	t.Logf("Collected %d stream events", len(allEvents))

	// Fetch the persisted chat from the DB via the API to verify
	// the round-trip.
	chatWithMessages, err := client.GetChat(ctx, chat.ID)
	require.NoError(t, err)

	// Walk persisted messages and look for the expected parts.
	var (
		foundWebSearchToolCall bool
	...
	for _, msg := range chatWithMessages.Messages {
		for _, part := range msg.Content {
			...
			case part.Type == codersdk.ChatMessagePartTypeToolCall &&
				...
				foundWebSearchToolCall = true
				t.Logf("Found web_search tool-call (id=%s)", part.ToolCallID)

			case part.Type == codersdk.ChatMessagePartTypeToolResult &&
				part.ProviderExecuted:
				foundToolResult = true
				t.Logf("Found provider-executed tool-result (id=%s)", part.ToolCallID)

			case part.Type == codersdk.ChatMessagePartTypeSource &&
				part.URL != "":
				sourceCount++
				t.Logf("Found source: %s (%s)", part.URL, part.Title)
			}
		}
	}

	t.Logf("Found %d sources", sourceCount)

	require.True(t, foundWebSearchToolCall,
		"expected at least one assistant tool-call part with tool_name=web_search and provider_executed=true")
	require.True(t, foundToolResult,
		"expected at least one tool-result part with provider_executed=true")
	require.Greater(t, sourceCount, 0,
		"expected at least one source part with a non-empty URL")
}
