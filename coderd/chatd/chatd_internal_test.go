...
func TestPublishToStreamDeliversAllEventsToSubscribers(t *testing.T) {
	...
	// subscriber's buffer filled up and events were dropped.
	select {
	case <-overflow:
		// Expected: overflow was signaled.
	default:
...
func TestPublishToStreamOverflowSignalsSubscriber(t *testing.T) {
	t.Parallel()
	...
	// Publish 200 events without consuming. The buffer (128)
	// will fill, and the overflow channel should be closed.
	const totalPublished = 200
	for range totalPublished {
		srv.publishToStream(chatID, codersdk.ChatStreamEvent{
			Type:   codersdk.ChatStreamEventTypeMessagePart,
	...
	// The subscriber channel should have 128 buffered events.
	var received int
drain:
	for {
		select {
