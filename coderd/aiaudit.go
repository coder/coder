...
func (api *API) resolveAIAuditSponsor(rw http.ResponseWriter, r *http.Request) (uuid.UUID, bool) {
	...
	// The audit surface is management plane: a sponsor is a human user. An
	// AI-held key is refused rather than silently treated as its sponsor.
	callerID, isUser := apiKey.UserID()
	if !isUser {
...
func (api *API) aiAuditAgents(rw http.ResponseWriter, r *http.Request) {
	...
	rows, err := api.Database.GetAIAgentsByOwner(dbauthz.AsSystemRestricted(ctx), sponsorID)
	if err != nil {
		httpapi.Write(ctx, rw, http.StatusInternalServerError, codersdk.Response{
	...
	for _, row := range rows {
		// Ownership is a typed pair; only user-owned agents belong to a
		// sponsor. A non-user owner sharing the UUID would be a different
		// principal entirely.
		...
		agents = append(agents, codersdk.AIAuditAgent{
			...
			OwnerID:          row.OwnerID,
			CreationSiteType: row.CreationSiteType,
