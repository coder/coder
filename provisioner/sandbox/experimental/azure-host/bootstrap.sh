...
fail() {
	record_status "failed: $PHASE: $STEP: $*"
	printf '[sandbox-host] ERROR: %s\n' "$*" >&2
	exit 1
}
on_error() {
	local rc=$1 line=$2
...
cleanup_smoke_workspace() (
	...
	assert_smoke_inventory_empty() {
		...
		[[ -z $containers ]] || {
			log 'Smoke container metadata remains.' >&2
			return 1
		}
		...
		[[ ! -s $runsc_errors ]] || {
			log 'runsc inventory emitted diagnostics; cleanup could not be verified.' >&2
			return 1
		}
...
PYCODE
	...
	[[ -n $workspace_id ]] || {
		log 'Could not identify the smoke workspace after uncertain create.' >&2
		return 1
	}
	...
	gone) ;;
	*)
		log 'Smoke provisioning job did not reach a terminal state.' >&2
		return 1
		;;
	esac
