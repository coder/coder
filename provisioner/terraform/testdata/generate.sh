...
minimize_diff() {
	for f in *.tf*.json; do
		...
		while read -r line; do
			...
			git diff --no-index -- "$scriptdir/resources/$name/$f" "$f" |
...
run() {
	...
	if [[ -d .terraform/providers ]]; then
		mkdir -p "$workdir/providers/resources/$name/.terraform"
...
if [[ "$(cd "$scriptdir/resources" && fingerprint)" != "$inputs" ]]; then
