...
func Test_WorkspaceTagDefaultsFromFile(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		*/
		{
			name: "main.tf with dynamic parameter default value from variable ",
			files: map[string]string{
				"main.tf": `
					variable "a" {
						type = string
					...
					data "coder_parameter" "a" {
						name = "a"
						type = "string"
						default = length(toset(split(",", var.a))) < 2 ? tolist(toset(split(",", var.a)))[0] : null
					}
					data "coder_workspace_tags" "tags" {
						tags = {
							"a"        = data.coder_parameter.a.value
						}
					}`,
			},
			expectTags:  map[string]string{"a": "a"},
			expectError: ``,
		},
		/*
			{
