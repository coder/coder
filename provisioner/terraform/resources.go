...
type agentScriptAttributes struct {
	AgentID string `mapstructure:"agent_id"`
	// ID is generated by the coder_script resource at apply, so it is empty
	// during plan.
	ID               string `mapstructure:"id"`
	DisplayName      string `mapstructure:"display_name"`
	Icon             string `mapstructure:"icon"`
...
type State struct {
	...
	// Warnings are surfaced to the user in the build log.
	Warnings []string
}
...
func ConvertState(ctx context.Context, modules []*tfjson.StateModule, rawGraph string, logger slog.Logger) (*State, error) {
	...
	findTerraformResources = func(mod *tfjson.StateModule) {
		...
		for _, resource := range mod.Resources {
			...
			if resource.Type == "coder_script_order" {
				tfResourcesScriptOrder[resource.Address] = scriptOrderDataSource{
					resource:      resource,
	...
	for _, tfResource := range sortedResources["coder_agent"] {
		...
		// Support the legacy script attributes in the agent! These do not
		// participate in coder_script_order rules; only coder_script
		// resources are orderable. Migrate the script body into a
		// coder_script resource to order it.
	...
	orderableScripts := make([]*orderableScript, 0, len(sortedScriptResources))
	for _, resource := range sortedScriptResources {
		...
		orderable := &orderableScript{
			address: resource.Address,
			script:  script,
		}
		orderableScripts = append(orderableScripts, orderable)
		...
		for _, agents := range resourceAgents {
			for _, agent := range agents {
				...
				if dependsOnAgent(graph, agent, attrs.AgentID, resource) {
					...
					orderable.owner = agent
					orderable.ownerName = agent.Name
