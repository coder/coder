...
// TestSensitiveParameterFromState covers decoding the coder_parameter
// sensitive attribute from state ahead of the pinned provider exposing it, so
// the fixture is constructed in memory rather than generated by terraform.
func TestSensitiveParameterFromState(t *testing.T) {
	t.Parallel()
	ctx, logger := context.Background(), slogtest.Make(t, nil).Leveled(slog.LevelDebug)

	parameter := func(name string, attrs map[string]interface{}) *tfjson.StateResource {
		...
		return &tfjson.StateResource{
			Address:         "data.coder_parameter." + name,
			Type:            "coder_parameter",
			Name:            name,
			Mode:            tfjson.DataResourceMode,
			AttributeValues: base,
		}
	}
	...
	state, err := terraform.ConvertState(ctx, []*tfjson.StateModule{{
		Resources: []*tfjson.StateResource{
			...
			parameter("api_key", map[string]interface{}{"sensitive": true}),
		},
	}}, `digraph {}`, logger)
	require.NoError(t, err)
	require.Len(t, state.Parameters, 2)

	byName := make(map[string]*proto.RichParameter, len(state.Parameters))
	for _, p := range state.Parameters {
		byName[p.Name] = p
	}
	require.False(t, byName["region"].Sensitive)
	require.True(t, byName["api_key"].Sensitive)
}
