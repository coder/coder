...
func ParseRPCVersion(str string) (RPCVersion, error) {
	...
	if len(split) != 2 {
		return RPCVersion{}, fmt.Errorf("invalid version string: %s", str)
	}
...
func ParseRPCVersionList(str string) (RPCVersionList, error) {
	...
	for i, v := range split {
		...
		if err != nil {
			return RPCVersionList{}, fmt.Errorf("invalid version list: %s", str)
		}
		versions[i] = version
	}
	...
	err := vl.Validate()
	if err != nil {
		return RPCVersionList{}, fmt.Errorf("invalid parsed version list %q: %w", str, err)
	}
	return vl, nil
}
...
func (vl RPCVersionList) Validate() error {
	...
	for i := 0; i < len(vl.Versions); i++ {
		if vl.Versions[i].Major == 0 {
			return fmt.Errorf("invalid version: %s", vl.Versions[i].String())
		}
		if i > 0 && vl.Versions[i-1].Major == vl.Versions[i].Major {
			return fmt.Errorf("duplicate major version: %d", vl.Versions[i].Major)
		}
		if i > 0 && vl.Versions[i-1].Major > vl.Versions[i].Major {
			return fmt.Errorf("versions are not sorted")
		}
	}
	return nil
}
