...
func NewBidirectionalPipe(readFd, writeFd uintptr) (BidirectionalPipe, error) {
	...
	_, err := read.Stat()
	if err != nil {
		return BidirectionalPipe{}, fmt.Errorf("stat pipe_read (fd=%v): %w", readFd, err)
	}
	write := os.NewFile(writeFd, "pipe_write")
	_, err = write.Stat()
	if err != nil {
		return BidirectionalPipe{}, fmt.Errorf("stat pipe_write (fd=%v): %w", writeFd, err)
	}
...
func (b BidirectionalPipe) Read(p []byte) (int, error) {
	n, err := b.read.Read(p)
	if err != nil {
		return n, fmt.Errorf("read from pipe_read (fd=%v): %w", b.read.Fd(), err)
	}
	return n, nil
}
...
func (b BidirectionalPipe) Write(p []byte) (n int, err error) {
	...
	if err != nil {
		return n, fmt.Errorf("write to pipe_write (fd=%v): %w", b.write.Fd(), err)
	}
	return n, nil
}
...
func (b BidirectionalPipe) Close() error {
	...
	rErr := b.read.Close()
	if rErr != nil {
		err = multierror.Append(err, fmt.Errorf("close pipe_read (fd=%v): %w", b.read.Fd(), rErr))
	}
	wErr := b.write.Close()
	if err != nil {
		err = multierror.Append(err, fmt.Errorf("close pipe_write (fd=%v): %w", b.write.Fd(), wErr))
	}
	return err
}
