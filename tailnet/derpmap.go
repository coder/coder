...
func NewDERPMap(ctx context.Context, region *tailcfg.DERPRegion, stunAddrs []string, remoteURL, localPath string) (*tailcfg.DERPMap, error) {
	if remoteURL != "" && localPath != "" {
		return nil, fmt.Errorf("a remote URL or local path must be specified, not both")
	}
	if region != nil {
	...
	if region != nil {
		...
		if conflicts {
			return nil, fmt.Errorf("the default region ID conflicts with a remote region from %q", remoteURL)
		}
