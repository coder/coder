...
func NewConn(options *Options) (conn *Conn, err error) {
	...
	if len(options.Addresses) == 0 {
		return nil, errors.New("At least one IP range must be provided")
	}
...
func (c *Conn) SetDNSHosts(hosts map[dnsname.FQDN][]netip.Addr) error {
	if c.dnsConfigurator == nil {
		return errors.New("no DNSConfigurator set")
	}
...
func (c *Conn) Listen(network, addr string) (net.Listener, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("tailnet: split host port for listen: %w", err)
	}
	...
	if _, ok := c.listeners[lk]; ok {
		c.mutex.Unlock()
		return nil, fmt.Errorf("tailnet: listener already open for %s, %s", network, addr)
	}
	c.listeners[lk] = ln
	c.mutex.Unlock()
	return ln, nil
...
func (ln *listener) Accept() (net.Conn, error) {
	...
	case c = <-ln.conn:
	case <-ln.closed:
		return nil, fmt.Errorf("tailnet: %w", net.ErrClosed)
	}
	return c, nil
}
