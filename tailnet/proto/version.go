...
//   - Added agent_plugins_enabled to Manifest. coderd sets it when the
//     agent-plugins experiment is enabled; the agent only discovers
//     plugins and emits the plugin variant when it is true, so older
//     coderd versions (which reject unknown body variants) never
//     receive one. Agents keep dialing v2.10.
const (
	CurrentMajor = 2
	CurrentMinor = 13
)
