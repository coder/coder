...
func createProvisionerDaemonClient(t *testing.T, server provisionerDaemonTestServer) proto.DRPCProvisionerDaemonClient {
	clientPipe, serverPipe := createTransports(t)
	t.Cleanup(func() {
		_ = clientPipe.Close()
		_ = serverPipe.Close()
	})
	...
	go func() {
		...
		err := srv.Serve(ctx, serverPipe)
		require.NoError(t, err)
	}()
	return proto.NewDRPCProvisionerDaemonClient(&multiplexedDRPC{clientPipe})
}
...
func createProvisionerClient(t *testing.T, server provisionerTestServer) sdkproto.DRPCProvisionerClient {
	...
	return sdkproto.NewDRPCProvisionerClient(&multiplexedDRPC{clientPipe})
}
...
// Creates an in-memory pipe of two yamux sessions.
func createTransports(t *testing.T) (*yamux.Session, *yamux.Session) {
