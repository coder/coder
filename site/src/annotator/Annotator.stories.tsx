...
export const WithPinsAndOutput: Story = {
	args: { picking: true },
	play: async ({ canvasElement }) => {
		const canvas = within(canvasElement);
		const host = () => document.getElementById(annotatorHostId);
		const annotate = async (target: HTMLElement, comment: string) => {
			await userEvent.click(target);
			const textarea = host()?.shadowRoot?.querySelector("textarea");
			if (textarea) {
				await userEvent.type(textarea, `${comment}{enter}`);
			}
		};
		await annotate(canvas.getByTestId("save-button"), "Use the primary style");
		await annotate(
			canvas.getByRole("heading", { name: "Acme Settings" }),
			"Rename to Account settings",
		);
		const send = host()?.shadowRoot?.querySelector<HTMLButtonElement>(
			'[aria-label="Send annotations to Coder"]',
		);
		if (send) {
