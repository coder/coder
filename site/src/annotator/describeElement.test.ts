...
describe("buildSelector", () => {
	...
	it("ignores ids and test ids too long to be worth a selector", () => {
		...
		const note = body.querySelector("p");
		expect(note && buildSelector(note)).toBe("p.note");
	});

	it("names the document roots rather than returning nothing", () => {
		const body = render("<p>Hi</p>");
		expect(buildSelector(body)).toBe("body");
		expect(buildSelector(body.ownerDocument.documentElement)).toBe("html");
	});
});
...
describe("describeElement text capture", () => {
	...
	it("skips text hidden through CSS", () => {
		...
		expect(describeElement(menu).text).toBe("Open");
	});
