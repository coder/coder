...
describe("selectTrialCta", () => {
	const start = dayjs("2026-08-25T12:00:00Z");
	const now = start.unix();
	const admin = { canViewLicenses: true, now };
	const inDays = (days: number) => start.add(days, "day").unix();
	const inHours = (hours: number) => start.add(hours, "hour").unix();
