...
export const ChatSharePopover: FC<ChatSharePopoverProps> = ({
	...
	return (
		<>
			...
			<ConfirmShareDialog
				open={sharing.pendingConfirmation !== null}
				isMutating={sharing.isMutating}
				requiresToolCalls={
					sharing.pendingConfirmation?.requiresToolCalls ?? false
				}
				requiresAttachments={
					sharing.pendingConfirmation?.requiresAttachments ?? false
				}
				message={sharing.pendingConfirmation?.message}
				detail={sharing.pendingConfirmation?.detail}
				onConfirm={sharing.confirmPending}
				onCancel={sharing.cancelPending}
			/>
		</>
	);
};
