...
type ChatSharingFormProps = {
	...
	onUpdateUserEntry: (user: ChatUser, entry: ChatShareEntry) => unknown;
	onUpdateGroupEntry: (group: ChatGroup, entry: ChatShareEntry) => unknown;
...
export const ChatSharingForm: FC<ChatSharingFormProps> = ({
	...
	const handleAdd = async () => {
		...
		if (isGroup(selectedOption)) {
			...
			// MinimalUser fields we need. New entries start with
			// share_tool_calls and share_attachments disabled so viewers
			// see redacted markers until the owner opts them in.
			const user: ChatUser = {
		...
		<div className="flex flex-col gap-4">
			...
			<div>
				<Table>
					<TableHeader>
						<TableRow>
							<TableHead className="w-[40%] py-2">Member</TableHead>
							<TableHead className="w-[15%] py-2">Role</TableHead>
							<TableHead className="w-[15%] py-2">Tool calls</TableHead>
							<TableHead className="w-[15%] py-2">Attachments</TableHead>
							...
							<>
								{chatACL.groups.map((group) => (
									<TableRow key={group.id}>
										...
										<TableCell className="py-2 w-[15%]">
											<Checkbox
												aria-label={`Share tool calls with ${group.display_name || group.name}`}
												checked={group.share_tool_calls}
												disabled={
													!canUpdatePermissions ||
												...
												onCheckedChange={(next) =>
													onUpdateGroupEntry(group, {
														role: group.role,
														share_tool_calls: next === true,
														share_attachments: group.share_attachments,
													})
												}
												data-testid={`group-${group.id}-tool-calls`}
											/>
										</TableCell>
										<TableCell className="py-2 w-[15%]">
											<Checkbox
												aria-label={`Share attachments with ${group.display_name || group.name}`}
												checked={group.share_attachments}
												disabled={
													!canUpdatePermissions ||
													updatingGroupId === group.id ||
													isMutating
												...
												onCheckedChange={(next) =>
													onUpdateGroupEntry(group, {
														role: group.role,
														share_tool_calls: group.share_tool_calls,
														share_attachments: next === true,
													})
												}
												data-testid={`group-${group.id}-attachments`}
											/>
										</TableCell>
										<TableCell className="py-2 w-[15%]">
											{canUpdatePermissions && (
								...
								{chatACL.users.map((user) => (
									<TableRow key={user.id}>
										...
										<TableCell className="py-2 w-[15%]">
											<Checkbox
												aria-label={`Share tool calls with ${user.username}`}
												checked={user.share_tool_calls}
												disabled={
													!canUpdatePermissions ||
												...
												onCheckedChange={(next) =>
													onUpdateUserEntry(user, {
														role: user.role,
														share_tool_calls: next === true,
														share_attachments: user.share_attachments,
													})
												}
												data-testid={`user-${user.id}-tool-calls`}
											/>
										</TableCell>
										<TableCell className="py-2 w-[15%]">
											<Checkbox
												aria-label={`Share attachments with ${user.username}`}
												checked={user.share_attachments}
												disabled={
													!canUpdatePermissions ||
													updatingUserId === user.id ||
													isMutating
												...
												onCheckedChange={(next) =>
													onUpdateUserEntry(user, {
														role: user.role,
														share_tool_calls: user.share_tool_calls,
														share_attachments: next === true,
													})
												}
												data-testid={`user-${user.id}-attachments`}
											/>
										</TableCell>
										<TableCell className="py-2 w-[15%]">
											{canUpdatePermissions && (
