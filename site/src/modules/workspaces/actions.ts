...
export const abilitiesByWorkspaceStatus = (
	...
	switch (status) {
		...
		case "running": {
			// When the workspace is running but agents are unhealthy (e.g.
			// envbuilder is still building the container image), disable
			// restart-related actions since the workspace isn't fully ready yet.
			// Stop is still available so users can abort if needed.
