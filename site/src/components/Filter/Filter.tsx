...
/**
 * Layout container for a filter bar. Handles responsive wrapping and spacing.
 * Use this when composing filter pieces directly instead of using the
 * all-in-one <Filter> component.
 */
export const FilterBar: FC<{
	children: ReactNode;
	singleRowBreakpoint?: Breakpoint;
}> = ({ children, singleRowBreakpoint = "lg" }) => {
...
/**
 * The preset quick-filter dropdown ("Filters" button). Can be used standalone
 * inside a <FilterBar> when you need custom layout or additional controls.
 */
export const FilterPresetMenu: FC<FilterPresetMenuProps> = ({
	value,
	presets,
	learnMoreLink,
	learnMoreLabel2,
...
/**
 * The search text field with inline error display. Maintains a local copy of
 * the query for snappy typing while debouncing updates to the parent filter
 * state. Can be used standalone inside a <FilterBar>.
 */
...
/**
 * All-in-one filter bar. Composes <FilterBar>, <FilterPresetMenu>, and
 * <FilterSearchField> with loading-skeleton support.
 *
 * Use the individual pieces directly when you need custom layout or want to
 * insert controls between the preset menu and search field.
 */
export const Filter: FC<FilterProps> = ({
	...
	return (
		<FilterBar singleRowBreakpoint={singleRowBreakpoint}>
			{isLoading ? (
				<>
				...
				<>
					...
					/>
					<FilterSearchField filter={filter} error={error} />
					{options}
				</>
			)}
		</FilterBar>
