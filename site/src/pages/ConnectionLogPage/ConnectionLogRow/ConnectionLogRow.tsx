						...
						>
							...
							>
								...
								<span css={styles.connectionLogTime}>
									{new Date(connectionLog.time).toLocaleTimeString()}
									{connectionLog.ssh_info?.close_time &&
										` → ${new Date(connectionLog.ssh_info.close_time).toLocaleTimeString()}`}
								</span>
							</Stack>
								...
								<Tooltip
									title={
										<div css={styles.connectionLogInfoTooltip}>
											...
											{connectionLog.ssh_info?.close_reason && (
												<div>
													<h4 css={styles.connectionLogInfoheader}>
														Close Reason:
													</h4>
													<div>{connectionLog.ssh_info?.close_reason}</div>
												</div>
											)}
										</div>
									}
