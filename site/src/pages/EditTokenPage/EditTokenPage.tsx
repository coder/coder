...
const convertTokenToFormValues = (
	...
	const compositeScopes = token.scopes.filter((scope) => compositeNames.has(scope));
	const lowLevelScopes = token.scopes.filter((scope) => !compositeNames.has(scope));

	return {
		name: token.token_name,
		lifetime: lifetimeDaysFromSeconds(token.lifetime_seconds),
		scopeMode:
			compositeScopes.length > 0 ? "composite" : "low_level",
		compositeScopes,
		lowLevelScopes,
...
const EditTokenPage = () => {
	...
	if (!tokenName || tokenFailed || !token) {
		return <ErrorAlert error={tokenError ?? new Error("Token could not be loaded")} />;
	}
