...
describe("resolveParsedConversation", () => {
	it("reuses parsed output when message references are unchanged", () => {
		...
		expect(second.computerUseSubagentIds).toBe(first.computerUseSubagentIds);
	});
	...
	it("re-parses when any message reference changes", () => {
		...
		expect(second.subagentTitles).not.toBe(first.subagentTitles);
		expect(second.computerUseSubagentIds).not.toBe(
			first.computerUseSubagentIds,
		);
	});
