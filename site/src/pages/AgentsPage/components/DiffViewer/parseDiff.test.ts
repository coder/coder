...
describe("parseDiffString", () => {
	...
	it("decodes C-quoted non-ASCII paths", () => {
		// Git quotes paths with non-ASCII bytes as octal escapes by default
		// (core.quotePath).
		...
		expect(files.map((f) => f.name)).toEqual(["café.txt"]);
	});

	it("names files in diffs produced with diff.noprefix", () => {
		// The library logs the prefix-less `diff --git` header as invalid but
		// still takes the name from the `---`/`+++` lines.
		...
		try {
			...
			expect(files.map((f) => f.name)).toEqual(["foo.txt"]);
		} finally {
			error.mockRestore();
		}
	});

	it("keeps a blank-line addition where the patch placed it", () => {
		// The added blank line is the third of three. Moving it to the top of
		// the blank run would misplace highlights and review comments.
		const files = parseDiffString(
			[
		...
		const change = files[0].hunks[0].hunkContent.find(
			(block) => block.type === "change",
		);
		expect(change?.additionLineIndex).toBe(2);
	});
