...
/** Segmented unified/split control for a diff sub-header. */
export const DiffStyleToggle: FC<{
	value: DiffStyle;
	onChange: (style: DiffStyle) => void;
}> = ({ value, onChange }) => {
	const segment = (style: DiffStyle) =>
		cn(
			"flex cursor-pointer items-center border-0 px-1.5 transition-colors",
			value === style
	...
	return (
		<div className="flex h-6 shrink-0 items-stretch overflow-hidden rounded-md border border-solid border-border-default">
			<button
				type="button"
				onClick={() => onChange("unified")}
				aria-label="Unified diff"
				aria-pressed={value === "unified"}
				className={segment("unified")}
			>
			...
			<button
				...
				className={cn(
					segment("split"),
					"border-l border-solid border-border-default",
				)}
			>
