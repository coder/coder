...
export const SingleStepSuccessfulRun: Story = {
	...
	play: async ({ canvasElement }) => {
		...
		// First step auto-opens; verify Input/Output sections appear.
		await waitFor(() => {
			expect(canvas.getByText("Step 1")).toBeVisible();
		});
		expect(canvas.getByText(/Input/i)).toBeVisible();
		expect(canvas.getByText(/Output/i)).toBeVisible();

		// Request body toggle should be available.
...
export const MultiStepRunWithRetries: Story = {
	...
	play: async ({ canvasElement }) => {
		...
		// Both steps visible in collapsed list.
		await waitFor(() => {
			expect(canvas.getByText("Step 1")).toBeVisible();
			expect(canvas.getByText("Step 2")).toBeVisible();
		});

		// Step 1 auto-opens; its raw attempts should show 3 entries.
		await waitFor(() => {
...
export const ErrorStateWithRedactedHeaders: Story = {
	...
	play: async ({ canvasElement }) => {
		...
		// Step auto-opens; error section and redaction markers visible.
		await waitFor(() => {
			expect(canvas.getByText(/upstream_unauthorized/i)).toBeVisible();
		});
		expect(canvasElement.textContent).toContain("[REDACTED]");

		// Expand request body to see redacted normalized fields.
		await user.click(canvas.getByText("Request body"));
		await waitFor(() => {
			expect(canvasElement.textContent).toContain("[REDACTED]");
		});
...
export const LongRawPayloads: Story = {
	...
	play: async ({ canvasElement }) => {
		...
		// Step auto-opens with usage pill visible (the step has tokens).
		await waitFor(() => {
			expect(canvas.getByText("Step 1")).toBeVisible();
		});
...
export const RichPayloadWithTranscript: Story = {
	...
	play: async ({ canvasElement }) => {
		...
		// Step auto-opens with transcript visible.
		await waitFor(() => {
		...
		expect(
			canvas.getByText(/Write me a hello world function in Python/),
		).toBeVisible();
		...
		// operation label.
		const stepTrigger = canvas.getByRole("button", { name: /Step 1/i });
...
export const ToolCallStep: Story = {
	...
	play: async ({ canvasElement }) => {
		...
		// Tool call rendered in output.
		await waitFor(() => {
			expect(canvas.getByText("run_code")).toBeVisible();
		});

		await user.click(canvas.getByRole("button", { name: "Show Arguments" }));
		await waitFor(() => {
			expect(canvas.getByText(/print\('hello'\)/i)).toBeVisible();
		});
...
const backendNormalizedRequest = {
	messages: [
		{
			role: "system",
			parts: [
...
const backendNormalizedResponse = {
	content: [
		{
			type: "tool_call",
			tool_call_id: "call_abc123",
...
const backendShapeRunDetail = makeRun({
	...
	steps: [
		makeStep({
			...
			run_id: "run-backend",
			operation: "stream",
			normalized_request: backendNormalizedRequest as unknown as Record<
				string,
				string
			>,
			normalized_response: backendNormalizedResponse as unknown as Record<
				string,
				string
			>,
			usage: {
...
export const BackendNormalizedShape: Story = {
	...
	play: async ({ canvasElement }) => {
		...
		// Step auto-opens; only last 2 messages visible by default.
		// The 4-message transcript should be truncated.
		await waitFor(() => {
		...
		await user.click(canvas.getByRole("button", { name: "Show JSON schema" }));
		await waitFor(() => {
			expect(canvas.getByText(/expression/)).toBeVisible();
		});
		...
		const showArgumentButtons = canvas.getAllByRole("button", {
			name: "Show Arguments",
		});
		expect(showArgumentButtons.length).toBeGreaterThanOrEqual(2);
		await user.click(showArgumentButtons[0]!);
		await user.click(canvas.getByRole("button", { name: "Show Result" }));
		await waitFor(() => {
