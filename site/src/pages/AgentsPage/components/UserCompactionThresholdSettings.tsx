...
interface UserCompactionThresholdSettingsProps {
	...
	thresholds: readonly TypesGen.UserChatCompactionThreshold[];
	thresholdsError: unknown;
	isLoadingThresholds: boolean;
	updateThresholdMutation: UseMutationResult<
	...
	>;
	deleteThresholdMutation: UseMutationResult<void, unknown, string>;
}
...
> = ({
	...
	const overridesByModelID = new Map(
		thresholds.map((threshold: TypesGen.UserChatCompactionThreshold) => [
											...
											onClick={() => {
												...
												updateThresholdMutation.mutate(
													...
													},
													{
														...
														onSettled: () => {
															setPendingModels((currentPendingModels) => {
																const nextPendingModels = new Set(
																	currentPendingModels,
																);
																nextPendingModels.delete(modelConfig.id);
										...
										{hasOverride && (
											<Button
												...
												onClick={() => {
													...
													deleteThresholdMutation.mutate(modelConfig.id, {
														onSuccess: () => {
															clearDraft(modelConfig.id);
															clearRowError(modelConfig.id);
														},
