...
describe("useChatStore", () => {
	...
	it("preserves stream state when clearStreamState is not called during active stream", async () => {
		immediateAnimationFrame();

		const chatID = "chat-queued-preserve";
		const existingMessage = makeMessage(chatID, 1, "user", "hello");
		const mockSocket = createMockSocket();
		vi.mocked(watchChat).mockReturnValue(mockSocket as never);
		...
		// Send a message_part to build stream state.
		act(() => {
			mockSocket.emitData({
				type: "message_part",
				chat_id: chatID,
		...
		await waitFor(() => {
			expect(result.current.streamState).not.toBeNull();
			expect(result.current.streamState?.blocks).toEqual([
				{ type: "response", text: "I am helping you" },
			]);
		});

		// Simulate what happens in a queued send: do NOT call
		// clearStreamState. The stream state must persist because
		// nothing cleared it. This is the invariant the
		// queued-send fix relies on.
		expect(result.current.streamState).not.toBeNull();
		expect(result.current.streamState?.blocks).toEqual([
			{ type: "response", text: "I am helping you" },
		]);
	...
	it("clears stream state when clearStreamState is called after non-queued send", async () => {
		immediateAnimationFrame();

		const chatID = "chat-non-queued-clear";
		const existingMessage = makeMessage(chatID, 1, "user", "hello");
		const mockSocket = createMockSocket();
		vi.mocked(watchChat).mockReturnValue(mockSocket as never);
		...
		// Simulate the non-queued send path: explicitly call
		// clearStreamState to wipe the in-progress stream.
		act(() => {
			result.current.store.clearStreamState();
		});

		// The stream state must now be null.
		await waitFor(() => {
			expect(result.current.streamState).toBeNull();
		});
	});
