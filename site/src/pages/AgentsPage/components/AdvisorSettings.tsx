...
export const AdvisorSettings: FC<AdvisorSettingsProps> = ({
	...
	const form = useFormik<AdvisorSettingsFormValues>({
		...
		onSubmit: (values, { resetForm }) => {
			// The advisor model override lives with the organization-scoped
			// defaults; the deployment PUT accepts only the runtime limits.
			...
			onSaveAdvisorConfig(request, {
				onSuccess: () => {
					const nextValues = normalizeAdvisorConfig({
						enabled: advisorConfigData?.enabled ?? true,
						...request,
					});
					showSavedState();
	...
	return (
		<AgentSettingLayout
			title="Advisor"
			description="Cap advisor usage per turn. The advisor provides strategic guidance to root agent chats. Set limits to 0 for unlimited. The advisor model is an organization-scoped default under Models."
			showSave={canSave}
