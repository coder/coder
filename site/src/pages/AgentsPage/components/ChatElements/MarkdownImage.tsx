...
export const MarkdownImage = ({ src, alt }: { src?: string; alt?: string }) => {
	...
	if (src && (consented || !isExternalImageSource(src))) {
		return (
			<img src={src} alt={alt ?? ""} loading="lazy" className="max-w-full" />
		);
	}

	const host = src ? externalImageHost(src) : undefined;
	// An absent source, or one without a resolvable host (for example
	// javascript: or otherwise malformed URLs), is never safe to load, so
	// it gets a placeholder without a load affordance.
	if (!host) {
		return (
