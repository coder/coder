...
const CURL_LINES = `  - Transcript: curl -sH "$H" "$CODER_URL/api/v2/chats/<id>/messages?limit=50" gives messages newest first; the first entries say what happened most recently. Page further back with before_id=<oldest id seen>. User messages say what a chat is for; read them first.
