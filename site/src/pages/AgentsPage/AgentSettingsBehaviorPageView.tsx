...
> = ({
	...
	const handleSaveRetentionDays = (event: FormEvent) => {
		...
		onSaveRetentionDays(
			...
			{ onSuccess: () => setLocalRetentionDays(null) },
		);
	};
			...
			{canSetSystemPrompt && (
				<>
					...
					>
						<div className="flex items-center gap-2">
							<h3 className="m-0 text-[13px] font-semibold text-content-primary">
								Chat Retention Period (days)
							</h3>
							<AdminBadge />
						</div>
						<p className="!mt-0.5 m-0 text-xs text-content-secondary">
							Archived chats and orphaned files older than this are
						...
						{isSaveRetentionDaysError && (
							<p className="m-0 text-xs text-content-destructive">
								Failed to save retention days setting.
							</p>
						)}
						{isRetentionDaysLoadError && (
							<p className="m-0 text-xs text-content-destructive">
								Failed to load retention days setting.
							</p>
						)}
					</form>
				</>
