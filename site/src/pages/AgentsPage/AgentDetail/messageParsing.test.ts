...
describe("parseMessageContent", () => {
	...
	it("parses a file-reference block into fileReferences and blocks", () => {
		const result = parseMessageContent([
			{
				type: "file-reference",
	...
	it("extracts all file-reference fields correctly", () => {
		const result = parseMessageContent([
			{
				type: "file-reference",
				file_name: "pkg/util.ts",
				start_line: 1,
				end_line: 1,
				content: "deleted code line",
				text: "Unused import.",
			},
		]);
		const ref = result.fileReferences[0];
		expect(ref.fileName).toBe("pkg/util.ts");
		expect(ref.startLine).toBe(1);
		expect(ref.endLine).toBe(1);
		expect(ref.content).toBe("deleted code line");
		expect(ref.text).toBe("Unused import.");
	});
