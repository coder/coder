...
describe(AgentsSidebar.name, () => {
	...
	it("shows executing indicator for running delegated chats", () => {
		...
		renderSidebar({ chats, initialPath: "/agents/child-running" });
	...
	it("shows executing indicator for pending delegated chats", () => {
		const chats = [
		...
		renderSidebar({ chats, initialPath: "/agents/child-pending" });

		expect(
			screen.getByTestId("agents-tree-executing-child-pending"),
		).toBeInTheDocument();
	});
	...
	it("supports expand and collapse for parent nodes", async () => {
		...
		renderSidebar({ chats, initialPath: "/agents/child-collapse" });
	...
	it("expands the active chat ancestry by default", async () => {
		const chats = [
