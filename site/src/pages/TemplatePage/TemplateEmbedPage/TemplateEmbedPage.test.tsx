...
test("Users can fill the parameters and copy the open in coder url", async () => {
	...
	expect(window.navigator.clipboard.writeText).toBeCalledWith(
		`[![Open in Coder](${window.location.origin}/open-in-coder.svg)](${window.location.origin}/templates/${MockTemplate.organization_name}/${MockTemplate.name}/workspace?mode=manual&name=my-first-workspace&param.first_parameter=firstParameterValue&param.second_parameter=123456)`,
	);
});
