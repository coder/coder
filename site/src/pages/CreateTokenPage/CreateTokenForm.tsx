...
export const CreateTokenForm: FC<CreateTokenFormProps> = ({
	...
	const autoLowLevelScopes = useMemo(() => {
		return expandCompositeScopes(form.values.compositeScopes, compositeExpansionMap);
	}, [form.values.compositeScopes, compositeExpansionMap]);

	const effectiveLowLevelScopes = useMemo(() => {
		return sortScopes([...new Set([...autoLowLevelScopes, ...form.values.lowLevelScopes])]);
	...
	const [hydratedAllowList, setHydratedAllowList] = useState<Record<string, Option>>({});
				...
				<FormFields>
					<Stack spacing={3}>
								...
								{(scopeCatalog?.composites ?? []).map((composite) => {
									...
									return (
										...
										>
											<Checkbox
												checked={checked}
												onCheckedChange={() => handleCompositeToggle(composite.name)}
											/>
											<div>
												...
												<div className="text-sm text-content-secondary">
													Includes {formatCompositeExpansions(composite.expands_to ?? [])}
												</div>
											</div>
										</div>
									);
						...
						<div className="rounded-md bg-surface-secondary px-3 py-2 text-sm text-content-secondary">
							<div className="font-medium text-content-primary">Selected scopes preview</div>
							<div className="mt-1">
								<strong>Composite:</strong>{" "}
								{effectiveCompositeScopes.length === 0
									? "None"
									: effectiveCompositeScopes.map(formatCompositeLabel).join(", ")}
							</div>
							<div className="mt-1">
			...
			>
				<FormFields>
					<MultiSelectCombobox
						...
						placeholder="Type a resource prefix, e.g. workspace:"
						delay={200}
						groupBy="group"
						triggerSearchOnFocus
					/>
					<p className="text-xs text-content-secondary mt-1">
						Examples: <code>workspace:All workspaces</code>, <code>template:my-template</code>, <code>Any resource</code>
					</p>
				</FormFields>
			</FormSection>
