...
const UpdateModelPage: FC = () => {
	...
	// Lets the user pick a different provider while editing. Falls back to the
	// provider that currently owns the model until they choose another.
				...
				<UpdateModelPageView
					...
					onSetDefault={() => {
						updateMutation.mutate(
							{ modelConfigId: model.id, req: { is_default: true } },
							{
								onSuccess: () => {
									toast.success(
										`Model "${model.display_name || model.model}" set as default.`,
									);
								},
								onError: (error) => {
									toast.error(
										getErrorMessage(error, "Failed to set default model."),
									);
								},
							},
						);
