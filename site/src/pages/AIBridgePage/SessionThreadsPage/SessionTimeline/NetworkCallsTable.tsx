...
interface NetworkCallsTableProps {
	// summary drives the header count and blocked badge. It reflects the whole
		...
		<div className="border border-solid rounded-md">
			<div className="flex items-center justify-between gap-2 px-2 py-1">
				<button
					...
					className="flex items-center gap-4 p-1 bg-transparent border-none cursor-pointer text-sm font-normal text-content-secondary"
				>
					<ChevronRightIcon
						className={cn(
			...
			{isOpen && (
				<div className="border-0 border-t border-solid">
					{calls.length === 0 ? (
	...
	return (
		<li className="border-0 border-t border-solid first:border-t-0">
			<button
				...
				aria-expanded={isOpen}
				onClick={() => setIsOpen((open) => !open)}
				className="flex items-center gap-3 w-full px-2 py-2 text-left bg-transparent border-none cursor-pointer hover:bg-surface-secondary"
			>
				<ChevronRightIcon
					className={cn(
						"size-3.5 flex-shrink-0 text-content-secondary transition-transform",
						isOpen && "rotate-90",
					)}
				/>
				{call.method && (
