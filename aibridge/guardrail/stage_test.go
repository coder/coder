...
func TestStage_EnforcingEditAppliedAndAnnotated(t *testing.T) {
	...
	st, err := guardrail.NewStage(guardrail.Member{Guardrail: g, Mode: guardrail.ModeEnforcing})
...
func TestStage_BlockWinsLowestName(t *testing.T) {
	...
	st, err := guardrail.NewStage(
		guardrail.Member{Guardrail: b, Mode: guardrail.ModeEnforcing},
		guardrail.Member{Guardrail: a, Mode: guardrail.ModeEnforcing},
	)
	require.NoError(t, err)
...
func TestStage_BlockSuppressesBodyEdit(t *testing.T) {
	...
	st, err := guardrail.NewStage(
		guardrail.Member{Guardrail: masker, Mode: guardrail.ModeEnforcing},
		guardrail.Member{Guardrail: blocker, Mode: guardrail.ModeEnforcing},
	)
	require.NoError(t, err)
...
func TestStage_AdvisoryDropsBlockAndEdits(t *testing.T) {
	...
	g := fake{name: "scorer", res: guardrail.Result{
		...
		Edits:       []guardrail.Edit{{Pointer: "messages.0.content", Value: "<X>"}},
	}}
	st, err := guardrail.NewStage(guardrail.Member{Guardrail: g, Mode: guardrail.ModeAdvisory})
	require.NoError(t, err)
...
func TestStage_FailModes(t *testing.T) {
	...
	closed, err := guardrail.NewStage(guardrail.Member{Guardrail: boom, Mode: guardrail.ModeEnforcing, FailMode: guardrail.FailClosed})
	require.NoError(t, err)
