...
test("stripHtmlComments reports an unclosed comment by its opening line", () => {
	// Opens on line 5 and never closes; the returned 1-based line lets the caller fail.
...
test("normalizeAngleBrackets rewrites autolinks and escapes stray <", () => {
	...
	assert.equal(normalizeAngleBrackets("`a < b`"), "`a < b`");
	// A CommonMark angle-bracket link destination is left intact (rewriting it would nest brackets).
	assert.equal(
		normalizeAngleBrackets(
	...
	// A plain parenthetical autolink has no "]" before "(", so it is still rewritten.
	assert.equal(
		normalizeAngleBrackets("(<https://x.dev>)"),
		"([https://x.dev](https://x.dev))",
	);
...
test("extractTitle reads frontmatter and ignores the make gen comment", () => {
	// make gen prepends a "# Code generated ..." line that would match the H1 scan;
	// the title must come from frontmatter and the whole block must be stripped.
	const makeGen =
	...
	// A frontmatter title wins over a body H1, but the H1 is still located to strip it.
	const both = "---\ntitle: Front\n---\n\n# Body H1\n\ntext";
	...
	// Homepage shape: a title-less frontmatter block is stripped and the title
	// falls back to the body H1, so the block never renders as plain text.
...
test("parseFrontmatter reads a flat block and ignores non-frontmatter", () => {
	// A make gen block: the comment line is skipped and endLine is the first body line.
	assert.deepEqual(
		parseFrontmatter(
			"---\n# Code generated by make gen. DO NOT EDIT.\ntitle: General\n---\nbody",
		),
	...
	// A nested YAML value is still a valid mapping, so endLine covers the whole block.
	assert.deepEqual(
		parseFrontmatter(
			"---\n# Code generated by make gen. DO NOT EDIT.\ntitle: Chats\ndescription: X\nstate:\n  - early access\n---\nbody",
		),
	...
	// A --- block that is not a mapping is a thematic break, not frontmatter.
	assert.deepEqual(parseFrontmatter("---\njust prose\n---\n"), {
		title: null,
		description: null,
		endLine: 0,
...
test("parseFrontmatter decodes YAML escapes and only treats mappings as frontmatter", () => {
	// A quoted description with YAML escapes decodes to real inner quotes.
	assert.deepEqual(
		parseFrontmatter(
	...
	// Extra keys are fine: the block is still a mapping, so it is frontmatter.
	assert.deepEqual(parseFrontmatter("---\nfoo: bar\ntitle: T\n---\nbody"), {
		title: "T",
		description: null,
		endLine: 4,
	});
	// A title-less mapping is still frontmatter (the docs homepage shape), so the
	// whole block is stripped and the key never renders as plain text.
	assert.deepEqual(
		parseFrontmatter("---\nmarkdown_url: /docs.md\n---\n\nbody"),
		{
			title: null,
	...
	// A non-string title/description scalar is treated as absent.
	assert.deepEqual(parseFrontmatter("---\ntitle: 123\n---\nbody"), {
		title: null,
		description: null,
		endLine: 3,
...
test("parseFrontmatter throws a labeled FrontmatterError on invalid YAML", () => {
	// An opened/closed block whose body is invalid YAML throws a labeled
	// FrontmatterError so the sync can name the offending file.
	assert.throws(
		() => parseFrontmatter('---\ntitle: "unterminated\n---\nbody'),
		(err) =>
...
test("rewriteTarget points source-tree links at GitHub via ctx.sourceLink", () => {
	...
	// A non-image target that escapes docs/ is handed to sourceLink.
	assert.equal(
