...
test("a </details><br> close is left as native disclosure (not converted)", () => {
	...
	// All three nodes must survive unchanged - no accordion conversion.
...
test("a nested <details> falls back to native disclosure (outer not converted)", () => {
	// Outer opener, inner opener, inner body, inner close, outer body, outer close.
	const tree = run([
	...
	// Outer must not be converted - its opener stays as raw html.
	assert.equal(kids(tree)[0].type, "html");
	...
	// Outer body and stray outer close remain as-is.
