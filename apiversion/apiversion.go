...
func (v *APIVersion) Validate(version string) error {
	...
	if major > v.supportedMajor {
		return fmt.Errorf("server is at version %d.%d, behind requested major version %s",
			v.supportedMajor, v.supportedMinor, version)
	}
	if major == v.supportedMajor {
		if minor > v.supportedMinor {
			return fmt.Errorf("server is at version %d.%d, behind requested minor version %s",
				v.supportedMajor, v.supportedMinor, version)
		}
		return nil
	}
	...
	return fmt.Errorf("version %s is no longer supported", version)
}
...
func Parse(version string) (major int, minor int, err error) {
	...
	if err != nil {
		return 0, 0, fmt.Errorf("invalid minor version: %s", version)
	}
