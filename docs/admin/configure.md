...
### Migrating from internal database to external database

To migrate from the internal database to an external database, follow these steps:
...
4. Run `pg_dump <internal-connection-string> > coder.sql` to dump the internal database to a file.
