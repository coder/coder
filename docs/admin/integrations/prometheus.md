...
|---------------------------------------------------|-----------|------------------------------------------------------------------------------------------------------|-----------------------------------------|
| `coderd_authz_scope_enforcement_total`            | counter   | Requests grouped by scope enforcement decision (allowed, denied, allow-list miss) and resource type. | `decision` `scope` `resource` `outcome` |
| `coderd_authz_scope_enforcement_duration_seconds` | histogram | Latency distribution for scope evaluation.                                                           | `decision` `scope` `resource` `outcome` |
| `coderd_authz_scope_allowlist_miss_total`         | counter   | Requests rejected because the scope allow-list excluded the resource.                                | `scope` `resource`                      |

#### Example PromQL queries

- Scope denials by resource type

  ```promql
  sum by (resource) (
    rate(coderd_authz_scope_enforcement_total{decision="scope_deny"}[5m])
  )
  ```

- Scope permitted but overall denied (roles or ACLs stopped it)

  ```promql
  sum by (resource, scope) (
    rate(coderd_authz_scope_enforcement_total{decision="scope_allow", outcome="deny"}[5m])
  )
  ```

- Allow-list misses (either view — both read from the same data)

  ```promql
  sum by (scope, resource) (
    rate(coderd_authz_scope_enforcement_total{decision="allow_list_deny"}[5m])
  )
  ```

  ```promql
  sum by (scope, resource) (
    rate(coderd_authz_scope_allowlist_miss_total[5m])
  )
  ```

- Success rate for scope evaluation

  ```promql
  sum(rate(coderd_authz_scope_enforcement_total{decision="scope_allow", outcome="allow"}[5m]))
  /
  sum(rate(coderd_authz_scope_enforcement_total[5m]))
  ```

- 95th percentile latency for scope enforcement decisions

  ```promql
  histogram_quantile(
    0.95,
    sum by (le, decision) (
      rate(coderd_authz_scope_enforcement_duration_seconds_bucket[5m])
    )
  )
  ```
