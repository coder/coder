...
> such as coder1.domain.com, coder2.domain.com, to be able to authenticate with the
...
> domain, and become the username (`example@coder.com` becomes `example`).
...
> (for instance, the example above maps to 2 roles in Coder.)
...
1. Configure
   ...
   - `email`: You can use the LDAP attribute "E-Mail-Addresses" as
     required.
