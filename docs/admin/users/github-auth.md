...
> [!TIP]
> Consider [using a GitHub App instead](#use-a-github-app-recommended). GitHub
> Apps use fine-grained permissions instead of the coarse OAuth scopes below
> and can issue expiring user tokens.

At sign-in, Coder requests the following OAuth scopes:

| Scope        | Purpose                                                                                                                 |
| ------------ | ----------------------------------------------------------------------------------------------------------------------- |
| `read:user`  | Read the user's profile.                                                                                                |
| `user:email` | Read the user's email addresses.                                                                                        |
| `read:org`   | Verify organization and team membership for `CODER_OAUTH2_GITHUB_ALLOWED_ORGS` and `CODER_OAUTH2_GITHUB_ALLOWED_TEAMS`. |

With a classic OAuth app these scopes are coarse: `read:org` grants read
access to the membership of **every** organization the authorizing user
belongs to, not only the organizations you allow, and the issued tokens do
not expire.
...
### Use a GitHub App (recommended)

You can use a GitHub App instead of an OAuth app for authentication.
GitHub Apps are limited to the fine-grained permissions you configure below,
rather than broad OAuth scopes, and can optionally issue expiring user
tokens. Coder's [default GitHub app](#default-configuration) works this way.

1. [Register a GitHub App](https://docs.github.com/en/apps/creating-github-apps/registering-a-github-app/registering-a-github-app) with:

   - **Homepage URL**: your deployment's access URL, such as `https://coder.domain.com`.
   - **Callback URL**: `https://coder.domain.com/api/v2/users/oauth2/github/callback`.
   - **Webhook**: not required for authentication; you can deselect **Active**.
   - **Account permissions**: **Email addresses: Read-only**, so Coder can read
     user email addresses.
   - **Organization permissions**: **Members: Read-only**, so Coder can verify
     membership for `CODER_OAUTH2_GITHUB_ALLOWED_ORGS` and
     `CODER_OAUTH2_GITHUB_ALLOWED_TEAMS`.
   - (Optional) **Expire user authorization tokens**, so user tokens held by
     the Coder server expire and are refreshed instead of living forever.

1. Install the app in every organization listed in
   `CODER_OAUTH2_GITHUB_ALLOWED_ORGS`. User tokens issued by a GitHub App can
   only read organization membership where the app is installed.

1. Generate a client secret, then configure Coder with the app's **Client ID**
   and the secret using the same environment variables described in
   [Step 2](#step-2-configure-coder-with-the-oauth-credentials).
