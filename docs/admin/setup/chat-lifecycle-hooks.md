...
| `CODER_CHAT_HOOK_ALLOW_INSECURE` | `--chat-hook-allow-insecure` | `false` | Set to `true` to allow a plain `http` hook URL. Hook traffic carries sensitive chat data and signed tokens, so only enable this on a trusted network. |
