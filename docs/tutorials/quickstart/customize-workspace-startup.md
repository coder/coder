...
- It applies no personal dotfiles, so each workspace starts with default shell and editor settings.
...
or pick the one that solves your problem:
...
It consists of one or more Terraform files (`*.tf`),
and it can include templated scripts (`*.tftpl`), a `README`, and any other supporting file.
...
- A [provider](https://developer.hashicorp.com/terraform/language/providers) is a plugin that connects Terraform to a platform or service. The `coder` provider connects Terraform to Coder.
...
- A [data source](https://developer.hashicorp.com/terraform/language/data-sources) reads data from a provider without creating or changing infrastructure. Parameters and external auth are data sources.
- A [module](https://developer.hashicorp.com/terraform/language/modules) is a reusable bundle of resources, data sources, providers, and other Terraform blocks you pull in by reference. Modules reduce the need to define the same Terraform code multiple times. The dotfiles guide uses a module from the [Coder Registry](https://registry.coder.com).

To go deeper on any of these,
...
Open it for editing now,
either in the Coder web editor or in your local editor with the `coder` CLI.
...
With the server running in your existing terminal,
open a second terminal and log in:
...
The guides use the CLI command `coder templates push`,
which reads from the `~/coder-quickstart` folder you pulled.
If you opened the template in the web editor instead,
publish the new version from the editor each time a guide tells you to push.
