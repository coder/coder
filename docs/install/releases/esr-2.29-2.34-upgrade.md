...
| AI Gateway dashboards and alerts use `coder_aibridged_*` or `coder_aibridgeproxyd_*` Prometheus metrics.        | AI Gateway emits new `coder_ai_gateway_*` and `coder_ai_gateway_proxy_*` metrics alongside legacy names during a deprecation window.                     | Migrate dashboards and alerts to the new names. Do not relabel new names back to legacy names until the legacy names are removed because that can create duplicate legacy series. See [AI Gateway monitoring](../../ai-coder/ai-gateway/monitoring.md). |
