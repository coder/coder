...
| ACL-based workspace sharing and chat sharing do not exist (Shared Workspaces arrived in 2.30, chat sharing later). | ACL-based workspace and chat sharing are available, with deployment-wide disable flags.                                                                  | These are new in this window. Review `CODER_DISABLE_WORKSPACE_SHARING` and `CODER_DISABLE_CHAT_SHARING` if you want to disable ACL-based sharing globally.                                                                                             |
