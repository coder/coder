...
| SFTP and SCP connections always landed in `$HOME`.                                                              | SFTP and SCP now respect the workspace agent `dir` setting. The `coder_agent` `dir` attribute still works but is deprecated and emits a warning. Setting `dir` to anything other than `$HOME` or `~` also breaks [Coder Desktop file sync](../../user-guides/desktop/desktop-connect-sync.md). | Set `dir` to `$HOME` or omit it on `coder_agent` resources. Update scripts that relied on implicit `$HOME` paths and prefer explicit absolute paths for file transfers.                                                                                |
