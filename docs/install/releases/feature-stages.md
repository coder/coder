...
- Enable the opt-in set, which is only the experiments Coder marks as ready for opt-in in this version (the note under the table lists them; it does not enable every experiment):

  ```sh
  coder server --experiments=*
  ```

- Enable specific experiments by flag, which is the only way to enable an experiment outside the opt-in set:
...
| Auto-fill Template Parameters   | `auto-fill-parameters`        | Pre-fills a new workspace's parameter form with the values from the user's most recent build of that template. |
...
| OAuth2 Provider Functionality   | `oauth2`                      | Enables OAuth2 provider functionality.                                                                         |
...
In this version, `--experiments=*` enables no experiments; enable each experiment by name.
