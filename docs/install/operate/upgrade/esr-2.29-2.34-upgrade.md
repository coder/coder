...
| GitLab external auth required only the `write_repository` scope.                                                | GitLab external auth now requires `write_repository` and `read_api` scopes by default.                                                                      | Add the `read_api` scope to your GitLab OAuth application. Existing users must re-authenticate to obtain the new scope. If you set scopes explicitly via `CODER_EXTERNAL_AUTH_<N>_SCOPES`, add `read_api` there as well. See [external authentication](../../../admin/external-auth/index.md). |
...
| Pre-2.28 Tasks templates might still exist in older deployments.                                                | The pre-2.28 Tasks template format is no longer supported as of 2.30.                                                                                       | Update Tasks templates to use `app_id` instead of the deprecated `sidebar_app` flow.                                                                                                                                                                                                           |
