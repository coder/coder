...
- 2 CPU cores and 4&nbsp;GB of memory to evaluate Coder.
  For production sizing, refer to the [Coder Validated Architecture](../admin/infrastructure/validated-architectures/index.md).
- `root` or `sudo` access.
...
For every server option, refer to the [`coder server`](../reference/cli/server.md) command reference.
