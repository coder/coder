...
| `» enabled`     | boolean           | false    |              | Enabled records whether the user intends the secret to be injected into workspaces. Deployment policy can block a stored target, so Enabled alone does not guarantee effective delivery. Disabled secrets remain visible and editable but are omitted from agent manifests. |
