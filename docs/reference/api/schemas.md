...
| `last_model_config_id` | string                                                          | false    |              | Last model config ID records the most recent explicit model selection. On external runtimes it is nil until a model is picked and serves only as a client restore hint: the runtime default applies whenever a message carries no selection.                               |
