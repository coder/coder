...
| `» snapshot_version`      | integer                                                                            | false    |              | Snapshot version is a monotonic per-chat version of the chat's durable execution state, incremented under the chat row lock on every ChatMachine.Update, so version order equals commit order. Clients use it to order chat status payloads received from REST, the per-chat stream, and the global watch stream. Metadata mutations (title, archive, summary, context) may not advance it. |
