...
[
  {
    ...
    "pin_order": 0,
    "plan_mode": "plan",
    "queued_for_capacity_at": "2019-08-24T14:15:22Z",
...
| `» queued_for_capacity_at` | string(date-time)                                                                  | false    |              | Queued for capacity at is when the chat began waiting for a concurrent-agent capacity slot. Nil unless the chat is queued. Status may be running or interrupting while queued, so clients must check this field.                                                           |
