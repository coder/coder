...
| Environment | <code>$CODER_SUPPORT_BUNDLE_WORKSPACE_EXTRA_PATH</code> |

File path or glob to collect from inside the remote workspace. Paths must be absolute or start with $HOME/, ${HOME}/, or ~/, which resolve against the agent user's home directory. Files local to the machine running this command are not collected. Can be specified multiple times.
