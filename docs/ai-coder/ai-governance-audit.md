...
> As of Coder v2.32, deployments without the add-on will not be able to access
> AI Gateway or Agent Firewall.
...
The following resource types are written to the `audit_logs` table when AI
Governance features are used. They are accessible through the standard
[Audit Logs UI and API](../admin/security/audit-logs.md).
...
Some fields are recorded as "changed but value withheld" rather than storing
the actual value. This prevents sensitive material from appearing in audit
diffs while still creating a record that a change occurred:

- **`AIProvider.settings`**: Encrypted JSON blob that may contain provider
  secrets (such as AWS Bedrock access keys). The audit log records that
  settings changed, but not the new value.
- **`AIProviderKey.api_key`**: The provider API key. The audit log records
  creation or deletion, but never exposes the key material.
- **`Chat.title`**: May contain sensitive content. The audit log records
  that a chat was created or modified, but not its title.
- **`UserSecret.value`**: The secret value. The audit log records
  create, write, and delete events but not the actual secret material.

## Querying audit log events

### UI

In the Coder dashboard, navigate to **Deployment** > **Audit Logs**. Use the
**Resource type** filter to narrow results:
...
AI Gateway session data, including prompts, token usage, tool calls, and model
reasoning, is stored separately from the `audit_logs` table in the `aibridge`
session store. It is not accessible through the Audit Logs UI or API.

See [Auditing AI Sessions](./ai-gateway/audit.md) for details on navigating
session history, tracing agentic loops, and conducting forensic audits of
AI activity.

## Agent Firewall events

Agent Firewall enforcement decisions (allow or deny) are emitted to workspace
logs and the connection log. These are not written to the `audit_logs` table.
