...
Claude Code child process per session, and streams results back to
Anthropic. Sessions originate from Claude Code on the web, the mobile
apps, scheduled routines, agents, or other Anthropic surfaces. The
runner is surface-agnostic.

Because the runner is just a Linux or macOS process with outbound
HTTPS to `api.anthropic.com`, **a Coder workspace is a natural host for
it**. Coder already gives you the things runners need: a reproducible
base image (template), credential delivery (parameters, env vars,
secrets), networking policy (workspace egress to your internal Git and
registries), workspace observability, and a place to attach an IDE if
you want to debug what the runner is doing.
