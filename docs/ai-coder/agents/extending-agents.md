...
Open the **+** menu in the chat composer to turn a Coder-managed or workspace MCP server on or off for the current chat.
The setting persists for that chat and applies from the next turn.
A server that is off contributes no tools to the model.
Force On servers cannot be turned off.
The toggle does not edit `.mcp.json`, and other chats keep their own selection.
