...
| `message_agent`                             | Sends a direct message that is promoted ahead of queued follow-up work. Queueing and promotion are separate operations, so queue processing can start first or the tool can report a promotion error after the message starts.                                       |
| `followup_agent`                            | Queues follow-up work after the sub-agent's current and already queued work.                                                                                                                                                                                         |
| `interrupt_agent`                           | Halts a sub-agent's current turn without adding new instructions; it transitions to waiting or running if there are queued messages.                                                                                                                                 |
