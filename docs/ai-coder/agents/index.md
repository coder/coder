...
Each upload can be up to 10&nbsp;MiB, and a single conversation can reference at most 50 attachments by default. Administrators can change the attachment count with [`CODER_CHAT_MAX_ATTACHMENTS_PER_CHAT`](../../admin/setup/configuration-reference.md#max-attachments-per-chat).
