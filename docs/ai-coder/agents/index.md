...
| `spawn_subagent` (`subagent_type=general` or `explore`) | Delegate a task to a sub-agent running in parallel                       |
...
| `spawn_subagent` (`subagent_type=computer_use`)         | Spawn a sub-agent with desktop interaction (screenshot, mouse, keyboard) |
...
`start_workspace`, `propose_plan`, `ask_user_question`) and orchestration tools (`spawn_subagent`,
...
`spawn_subagent` with `subagent_type=computer_use` additionally requires an
