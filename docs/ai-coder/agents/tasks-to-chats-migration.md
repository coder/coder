...
| n/a              | `paused`          | A turn finished while the queue head was under edit; the agent does not run again until the edit ends. New messages queue behind the head. Ending the edit via `PATCH /chats/{chat}/queue/{queuedMessage}` with `{"editing": false}` sends the head. |
