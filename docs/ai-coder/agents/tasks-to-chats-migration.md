...
| `paused`         | n/a               | The Tasks API pause stops the workspace. The Chats API has no workspace-stopping status; use `interrupt` plus separate workspace lifecycle. The Chats `paused` status below means something else. |
