...
You can also interact with Coder Tasks from your IDE. The [Coder extension for VS Code](https://marketplace.visualstudio.com/items?itemName=coder.coder-remote) (and compatible forks like Cursor) enables you to create, monitor, and manage Tasks directly from the IDE — eliminating the need to context-switch to a browser. Once installed, a dedicated Tasks view in the sidebar lets you select a template, configure parameters, prompt an agent, and track task status or download logs. This is all while your tasks run in fully governed Coder workspaces with access to your repos, credentials, and internal network.
