...
## How agent workspace usage is measured
...
The usage metric used to measure agent workspace consumption is called **Agent
Workspace Builds** (prev. "managed agents").

An Agent Workspace Build is counted each time a workspace is started
specifically for a coding agent to independently work on a request. Most of
the work in this workspace is performed by the agent, not a human developer.
Each agent-driven request starts its own workspace, and the usage meter counts
one Agent Workspace Build.
...
|-----------------------------------------------------------------------------------|--------------------------------|
| Developer asks a coding agent to write end-to-end tests in its own workspace      | Yes                            |
| Automated pipeline starts a workspace for a coding agent to review a pull request | Yes                            |
| Developer resumes an earlier agent workspace to continue prototyping              | Yes                            |
...
In the future, additional capabilities for managing agents may also consume
...
agent-driven workspaces to a large audience in the enterprise. Both Community
and Premium deployments include 1,000 Agent Workspace Builds, primarily for
...
Coder Agents. The column helps administrators identify which users contribute to
