...
This guide is itself exempt from the Coder rules: it demonstrates the violations those rules ban, so the repo-root `.vale.ini` clears `BasedOnStyles` for `docs/.style/`.
Zero baseline is measured over `docs/` excluding `docs/.style/`.
