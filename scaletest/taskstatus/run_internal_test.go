...
func (m *fakeClient) PatchAppStatus(ctx context.Context, req agentsdk.PatchAppStatus) error {
	m.logger.Debug(ctx, "called fake PatchAppStatus", slog.F("req", req))
	// Send the request to the channel so tests can verify it
	select {
	case m.patchStatusCalls <- req:
	case <-ctx.Done():
		return ctx.Err()
	}
...
func TestRunner_Run(t *testing.T) {
	...
	for i := 0; i < 4; i++ {
		...
		patchCall := testutil.RequireReceive(ctx, t, fClient.patchStatusCalls)
...
func TestRunner_RunMissedUpdate(t *testing.T) {
	...
	for i := 0; i < 4; i++ {
		tickWaiter := mClock.Advance((10 * time.Second) - updateDelay)
		patchCall := testutil.RequireReceive(testCtx, t, fClient.patchStatusCalls)
		require.Equal(t, appSlug, patchCall.AppSlug)
		require.Equal(t, fmt.Sprintf("scaletest status update:%d", i), patchCall.Message)
		require.Equal(t, codersdk.WorkspaceAppStatusStateWorking, patchCall.State)
...
func TestRunner_Run_WithErrors(t *testing.T) {
	...
	for i := 0; i < 4; i++ {
		...
		testutil.RequireSend(testCtx, t, fClient.patchStatusErrors, xerrors.New("a bad thing happened"))
		_ = testutil.RequireReceive(testCtx, t, fClient.patchStatusCalls)
		tickWaiter.MustWait(testCtx)
	}
