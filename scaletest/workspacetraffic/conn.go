...
func (c *rptyConn) writeNoLock(p []byte) (n int, err error) {
	...
	for len(p) > 0 {
		...
		if err := c.wenc.Encode(req); err != nil {
			return n, fmt.Errorf("encode pty request: %w", err)
		}
		n += len(pp)
	}
	return n, nil
...
func (c *rptyConn) Close() (err error) {
	...
	if err != nil {
		return fmt.Errorf("write ctrl+c: %w", err)
	}
	select {
	case <-time.After(connCloseTimeout):
		return fmt.Errorf("timeout waiting for read to finish")
	case err = <-c.readErr:
...
func connectSSH(ctx context.Context, client *codersdk.Client, agentID uuid.UUID, cmd string, requestPTY bool) (rwc *countReadWriteCloser, err error) {
	...
	if err != nil {
		return nil, fmt.Errorf("dial workspace agent: %w", err)
	}
	...
	closeFn := func() error {
		...
		select {
		case <-time.After(connCloseTimeout):
			merr = errors.Join(merr, fmt.Errorf("timeout waiting for ssh session to close"))
		case err := <-waitErr:
			if err != nil {
				...
				if errors.As(err, &exitErr) {
					...
					if exitErr.ExitStatus() != 255 {
						merr = errors.Join(merr, fmt.Errorf("ssh session exited with unexpected status: %d", int32(exitErr.ExitStatus())))
					}
				} else {
					merr = errors.Join(merr, err)
				}
