...
func connectRPTY(ctx context.Context, client *codersdk.Client, agentID, reconnect uuid.UUID, cmd string) (*countReadWriteCloser, error) {
	width, height := 80, 25
	conn, err := workspacesdk.NewWorkspaceClient(client).WorkspaceAgentReconnectingPTY(ctx, workspacesdk.WorkspaceAgentReconnectingPTYOpts{
		AgentID:   agentID,
		Reconnect: reconnect,
		Width:     uint16(width),
...
func connectSSH(ctx context.Context, client *codersdk.Client, agentID uuid.UUID, cmd string, requestPTY bool) (rwc *countReadWriteCloser, err error) {
	...
	agentConn, err := workspacesdk.NewWorkspaceClient(client).DialWorkspaceAgent(ctx, agentID, &workspacesdk.DialWorkspaceAgentOptions{})
	if err != nil {
