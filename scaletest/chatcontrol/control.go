...
func DeriveChatSeed(seed int64, values ...string) int64 {
	...
	upper := int64(binary.LittleEndian.Uint32(sum[4:8]) & 0x7fffffff)
	lower := int64(binary.LittleEndian.Uint32(sum[:4]))
	return (upper << 32) | lower
}

func newDeterministicRand(seed int64) *rand.Rand {
	sum := sha256.Sum256([]byte(strconv.FormatInt(seed, 10)))
	//nolint:gosec // Deterministic RNG is required for repeatable scaletest runs.
