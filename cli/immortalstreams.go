...
func DialImmortalOrFallback(
	...
	if err != nil {
		...
		tooManyErr := strings.Contains(err.Error(), "Too many immortal streams")
	...
	reconnector := &clientStreamReconnector{agentConn: agentConn, client: client, agentID: agentID, streamID: stream.ID, logger: logger}
