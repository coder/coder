...
// credentialValue generates the credential value (a string of JSON) to be stored
// in the operating system keyring.
func credentialValue(u *url.URL, token string) (string, error) {
	if u == nil {
...
func (o Keyring) Write(serverURL *url.URL, token string) error {
	cred, err := credentialValue(serverURL, token)
	if err != nil {
		return err
	}
