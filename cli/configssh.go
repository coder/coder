...
func sshConfigSplitOnCoderSection(data []byte) (before, section []byte, after []byte, err error) {
	...
	if startCount > 1 || endCount > 1 {
		return nil, nil, nil, xerrors.New("ssh config has multiple coder sections, please remove all but one")
	}

	startIndex := bytes.Index(data, []byte(sshStartToken))
	endIndex := bytes.Index(data, []byte(sshEndToken))
	if startIndex == -1 && endIndex != -1 {
		return nil, nil, nil, xerrors.New("ssh config has start header, but missing end header")
	}
	if startIndex != -1 && endIndex == -1 {
		return nil, nil, nil, xerrors.New("ssh config has end header, but missing start header")
	}
	if startIndex != -1 && endIndex != -1 {
		if startIndex > endIndex {
			return nil, nil, nil, xerrors.New("ssh config has coder section, but it is malformed and the END header is before the START header")
		}
