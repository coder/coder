...
func (pr *ParameterResolver) resolveWithInput(resolved []codersdk.WorkspaceBuildParameter, inv *serpent.Invocation, action WorkspaceCLIAction, templateVersionParameters []codersdk.TemplateVersionParameter) ([]codersdk.WorkspaceBuildParameter, error) {
	for _, tvp := range templateVersionParameters {
		...
		if (tvp.Ephemeral && pr.promptEphemeralParameters) ||
			...
			v, hasCliDefault := pr.richParametersDefaults[tvp.Name]
			if hasCliDefault {
				parameterValue = v
			}

			// Auto-accept the default value. We check !tvp.Required
			// instead of parameterValue != "" because a parameter
			// with default="" in Terraform is not required and has
			// an explicit (empty) default that should be accepted.
			// A CLI-provided default (--parameter-default) also counts.
			if pr.useParameterDefaults && (hasCliDefault || !tvp.Required) {
