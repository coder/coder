...
func TestScaleTestWorkspaceTraffic_ShardValidation(t *testing.T) {
	t.Parallel()
	...
	cases := []struct {
		...
		{
			name:    "TargetAndShardMutuallyExclusive",
			args:    []string{"--target-workspaces", "0:10", "--shard-count", "2"},
			errText: "--target-workspaces cannot be used with --shard-index/--shard-count",
		},
		{
			name:    "ShardIndexRequiresShardCount",
		...
		{
			name:    "ShardCountRequiresShardIndex",
			args:    []string{"--shard-count", "2"},
		...
		{
			name:    "ShardIndexOutOfRange",
			args:    []string{"--shard-index", "3", "--shard-count", "3"},
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			args := append([]string{"exp", "scaletest", "workspace-traffic"}, tc.args...)
