	...
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
				...
				workspaceName, err = cliui.Prompt(inv, cliui.PromptOptions{
					...
					Validate: func(workspaceName string) error {
						...
						if err != nil {
							...
							return fmt.Errorf("workspace name %q is invalid: %w", workspaceName, err)
						}
			...
			if templateName == "" {
				...
				if len(templates) == 0 {
					return fmt.Errorf("no template found with the name %q", templateName)
				}
				if len(templates) > 1 {
					...
					if err != nil {
						return fmt.Errorf("multiple templates found with the name %q, use `--org=<organization_name>` to specify which template by that name to use. Organizations available: %s", templateName, strings.Join(templateOrgs, ", "))
					}
					index := slices.IndexFunc(templates, func(i codersdk.Template) bool {
						return i.OrganizationID == selectedOrg.ID
					})
					if index == -1 {
						return fmt.Errorf("no templates found with the name %q in the organization %q. Templates by that name exist in organizations: %s. Use --org=<organization_name> to select one.", templateName, selectedOrg.Name, strings.Join(templateOrgs, ", "))
					}
			...
			if orgValue != "" && !(orgValueSource == serpent.ValueSourceDefault || orgValueSource == serpent.ValueSourceNone) {
				...
				if template.OrganizationID != selectedOrg.ID {
					...
					return fmt.Errorf("template is in organization %q, but %s was specified. Use %s to use this template",
			...
			if err != nil {
				return fmt.Errorf("can't parse given parameter values: %w", err)
			}
			cliBuildParameterDefaults, err := asWorkspaceBuildParameters(parameterFlags.richParameterDefaults)
			if err != nil {
				return fmt.Errorf("can't parse given parameter defaults: %w", err)
			...
			if copyParametersFrom != "" {
				...
				if err != nil {
					return fmt.Errorf("get source workspace build parameters: %w", err)
			...
			if err != nil {
				return fmt.Errorf("watch build: %w", err)
...
func prepWorkspaceBuild(inv *serpent.Invocation, client *codersdk.Client, args prepWorkspaceBuildArgs) ([]codersdk.WorkspaceBuildParameter, error) {
	...
	if args.RichParameterFile != "" {
		...
		if err != nil {
			return nil, fmt.Errorf("can't parse parameter map file: %w", err)
		}
	}
	...
	if err != nil {
		return nil, fmt.Errorf("template version git auth: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("get matched provisioners: %w", err)
