...
func (r *RootCmd) Server(newAPI func(context.Context, *coderd.Options) (*coderd.API, io.Closer, error)) *serpent.Command {
	...
	serverCmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			purger := dbpurge.New(ctx, logger.Named("dbpurge"), options.Database, options.DeploymentValues, options.PrometheusRegistry, &coderAPI.Auditor, options.NotificationsEnqueuer)
			defer purger.Close()
