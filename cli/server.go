...
func markEnterprise(an bigcli.Annotations) bigcli.Annotations {
	return an.Mark("enterprise", "true")
}

func markSecret(an bigcli.Annotations) bigcli.Annotations {
...
func serverOptions(c *codersdk.DeploymentConfig) *bigcli.OptionSet {
	...
	return &bigcli.OptionSet{
		...
		},
		{
			...
			Annotations: markEnterprise(nil),
		...
		},
		{
			...
			Annotations: markEnterprise(markSecret(nil)),
			Value:       &c.SCIMAPIKey,
		},
...
func Server(vip *viper.Viper, newAPI func(context.Context, *coderd.Options) (*coderd.API, io.Closer, error)) *cobra.Command {
	root := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			cacheDir := filepath.Join(cfg.CacheDir.Value, uuid.NewString())
