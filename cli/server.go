...
func createOIDCConfig(ctx context.Context, logger slog.Logger, vals *codersdk.DeploymentValues) (*coderd.OIDCConfig, error) {
	if vals.OIDC.ClientID == "" {
		return nil, fmt.Errorf("OIDC client ID must be set!")
	}
	...
	if len(vals.OIDC.GroupAllowList) > 0 && vals.OIDC.GroupField == "" {
		return nil, fmt.Errorf("'oidc-group-field' must be set if 'oidc-allowed-groups' is set. Either unset 'oidc-allowed-groups' or set 'oidc-group-field'")
	}
	...
	if !vals.OIDC.IgnoreUserInfo && vals.OIDC.UserInfoFromAccessToken {
		return nil, fmt.Errorf("to use 'oidc-access-token-claims', 'oidc-ignore-userinfo' must be set to 'false'")
	}
...
func enablePrometheus(
	...
	if err != nil {
		return nil, fmt.Errorf("register workspaces prometheus metric: %w", err)
	}
	...
	serverCmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			if vals.AccessURL.String() != "" &&
				...
				return fmt.Errorf("access-url must include a scheme (e.g. 'http://' or 'https://)")
			}
			...
			if vals.AccessURL.String() == "" {
				...
				if vals.WildcardAccessURL.String() == "" {
					...
					if err != nil {
						return fmt.Errorf("set wildcard access url %q: %w", wu, err)
					}
				}
			}
			...
			if err != nil {
				return fmt.Errorf("read external auth providers from env: %w", err)
			}
			...
			if err != nil {
				return fmt.Errorf("parse ssh config options %q: %w", vals.SSHConfig.SSHConfigOptions.String(), err)
			}
			options := &coderd.Options{
			...
			if vals.StrictTransportSecurity > 0 {
				...
				if err != nil {
					return fmt.Errorf("coderd: setting hsts header failed (options: %v): %w", vals.StrictTransportSecurityOptions, err)
				}
			}
			if vals.UpdateCheck {
			...
			if err != nil {
				return fmt.Errorf("get github oauth2 config params: %w", err)
			}
			...
			if err != nil {
				return fmt.Errorf("remove secrets from deployment values: %w", err)
			}
			...
			if err != nil {
				return fmt.Errorf("failed to create agent stats batcher: %w", err)
			}
			...
			if vals.Prometheus.Enable {
				...
				if err != nil {
					return fmt.Errorf("register agents prometheus metric: %w", err)
				}
				...
				if err = prometheusmetrics.Experiments(options.PrometheusRegistry, active); err != nil {
					return fmt.Errorf("register experiments metric: %w", err)
				}
			}
			...
			if err != nil && flag.Lookup("test.v") != nil {
				return fmt.Errorf("write config url: %w", err)
			}
			...
			if notificationsCfg.Enabled() {
				...
				if err != nil {
					return fmt.Errorf("failed to instantiate notification store enqueuer: %w", err)
				}
				...
				if err != nil {
					return fmt.Errorf("failed to instantiate notification manager: %w", err)
				}
			...
			case <-tunnelDone:
				exitErr = errors.New("dev tunnel closed unexpectedly")
			case <-pubsubWatchdogTimeout:
				exitErr = errors.New("pubsub Watchdog timed out")
			case exitErr = <-errCh:
			}
...
func configureGithubOAuth2(instrument *promoauth.Factory, params *githubOAuth2ConfigParams) (*coderd.GithubOAuth2Config, error) {
	...
	if !params.allowEveryone && len(params.allowOrgs) == 0 {
		return nil, errors.New("allowed orgs is empty: must specify at least one org or allow everyone")
	}
...
func ConnectToPostgres(ctx context.Context, logger slog.Logger, driver string, dbURL string, migrate func(db *sql.DB) error) (*sql.DB, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("get postgres version: %w", err)
	}
	if !version.Next() {
		return nil, fmt.Errorf("no rows returned for version select")
	}
	var versionNum int
	...
	if versionNum < 130000 {
		return nil, fmt.Errorf("PostgreSQL version must be v13.0.0 or higher! Got: %d", versionNum)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("migrations in database: %w", err)
	}
...
func ConfigureHTTPServers(logger slog.Logger, inv *serpent.Invocation, cfg *codersdk.DeploymentValues) (_ *HTTPServers, err error) {
	...
	if cfg.TLS.Enable {
		...
		if !tcpAddrValid {
			return nil, fmt.Errorf("invalid TCP address type %T", httpServers.TLSListener.Addr())
		}
		if tcpAddr.IP.IsUnspecified() {
			tcpAddr.IP = net.IPv4(127, 0, 0, 1)
		}
...
func parseExternalAuthProvidersFromEnv(prefix string, environ []string) ([]codersdk.ExternalAuthConfig, error) {
	...
	for _, v := range serpent.ParseEnviron(environ, prefix) {
		...
		case "DEVICE_FLOW":
			...
			if err != nil {
				return nil, fmt.Errorf("parse bool: %s", v.Value)
			}
...
func getAndMigratePostgresDB(ctx context.Context, logger slog.Logger, postgresURL string, auth codersdk.PostgresAuth, sqlDriver string) (*sql.DB, string, error) {
	...
	if auth == codersdk.PostgresAuthAWSIAMRDS {
		...
		if err != nil {
			return nil, "", fmt.Errorf("register aws rds iam auth: %w", err)
		}
	}
