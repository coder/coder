	...
	serverCmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			if experiments.Enabled(codersdk.ExperimentNotifications) {
				...
				notificationsManager, err = notifications.NewManager(cfg, coderd.NewRuntimeConfigStore(options.Database), options.Database, helpers, metrics, logger.Named("notifications.manager"))
				if err != nil {
