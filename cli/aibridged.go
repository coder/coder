...
// configuration; env-derived rows reach this loader via
// [coderd.SeedAIProvidersFromEnv], which must run before this is
// called.
//
...
func BuildProviders(ctx context.Context, db database.Store, cfg codersdk.AIBridgeConfig, logger slog.Logger) ([]aibridge.Provider, error) {
	...
	dumpBase := cfg.APIDumpDir.String()

	// AsAIBridged is the dedicated, narrowly-scoped subject the aibridge
	// daemon uses for runtime provider discovery. We grant
	// ResourceAIProvider:Read on that subject precisely for this loader,
	// so callers (in-process boot) get only what they need.
	//nolint:gocritic // AsAIBridged is the correct subject for this loader; see comment above.
	...
	for _, row := range rows {
		var dumpDir string
		if dumpBase != "" {
			dumpDir = path.Join(dumpBase, row.Name)
		}
		prov, err := buildAIProviderFromRow(row, keysByProvider[row.ID], cbCfg, sendActorHeaders, dumpDir)
		if err != nil {
			logger.Warn(ctx, "skip ai provider, failed to build",
...
// bedrockConfigFromRow extracts AWS Bedrock settings from a provider
// row, returning nil when the row is not configured for Bedrock. The
// detection rule matches [codersdk.IsBedrockConfigured]: a non-empty
// base URL or any IsConfigured signal on the settings blob.
func bedrockConfigFromRow(row database.AIProvider, settings codersdk.AIProviderSettings) *aibridge.AWSBedrockConfig {
	var bedrockSettings codersdk.AIProviderBedrockSettings
