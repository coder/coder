...
func (r *RootCmd) scaletestWorkspaceUpdates() *serpent.Command {
	...
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			var reuse []loadtestutil.ReuseUser
			if reuseUsers {
				...
				count := int(powerUserCount + regularUserCount)
				// The workspace-updates test does not select users by role; power and
				// regular users differ only in how many workspaces they own, so any
				// scaletest user in the pool works (nil filter).
				users, err := loadtestutil.SelectReuseUsers(ctx, client, usernameInfix, count, nil)
				if err != nil {
					return annotateInsufficientUsersError(err, createUsersCommandHint(count, 0, usernameInfix))
				}
				reuse, err = loadtestutil.MintReuseTokens(ctx, client, users, tokenLifetime)
				if err != nil {
