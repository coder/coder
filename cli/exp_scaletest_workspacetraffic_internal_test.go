...
func TestCreateWorkspaceAppConfig(t *testing.T) {
	...
	t.Run("PathAppTrailingSlashBaseURL", func(t *testing.T) {
		t.Parallel()
		agent := codersdk.WorkspaceAgent{
			Name: "main",
			Apps: []codersdk.WorkspaceApp{{Slug: "wsec", Subdomain: false}},
		}
		// A deployment URL with a trailing slash must not produce a double
	...
	t.Run("SubdomainAppRequiresAppHost", func(t *testing.T) {
		...
		require.ErrorContains(t, err, "no app host is configured")
	})
}
