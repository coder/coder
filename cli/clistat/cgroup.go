...
func (s *Statter) ContainerCPU() (*Result, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("get total cpu: %w", err)
	}
	used1, err := s.cGroupCPUUsed()

	if err != nil {
		return nil, fmt.Errorf("get cgroup CPU usage: %w", err)
	}
...
func (s *Statter) cGroupV2CPUUsed() (used float64, err error) {
	...
	if err != nil {
		return 0, fmt.Errorf("get cgroupv2 cpu used: %w", err)
	}
	periodUs, err := readInt64SepIdx(s.fs, cgroupV2CPUMax, " ", 1)
	if err != nil {

		return 0, fmt.Errorf("get cpu period: %w", err)
	}
...
func (s *Statter) cGroupV2CPUTotal() (total float64, err error) {
	...
	if err != nil {
		...
		return 0, fmt.Errorf("get cpu quota: %w", err)
	}
...
func (s *Statter) cGroupV1CPUTotal() (float64, error) {
	...
	if err != nil {
		...
		var merr error
		merr = multierror.Append(merr, fmt.Errorf("get cpu period: %w", err))
	...
	if err != nil {
		// Try alternate path under /sys/fs/cpu
		var merr error
		merr = multierror.Append(merr, fmt.Errorf("get cpu quota: %w", err))
...
func (s *Statter) cGroupV1CPUUsed() (float64, error) {
	...
	if err != nil {
		...
		var merr error
		merr = multierror.Append(merr, fmt.Errorf("read cpu used: %w", err))
...
func (s *Statter) cGroupV2Memory(p Prefix) (*Result, error) {
	...
	if err != nil {
		if !errors.Is(err, strconv.ErrSyntax) {
			return nil, fmt.Errorf("read memory total: %w", err)
		}
		// If the value is not a valid integer, assume it is the string
		// 'max' and that there is no limit set.
	} else {
...
func readInt64SepIdx(fs afero.Fs, path, sep string, idx int) (int64, error) {
	...
	if len(parts) < idx {
		return 0, fmt.Errorf("expected line %q to have at least %d parts", string(data), idx+1)
	}
...
func readInt64Prefix(fs afero.Fs, path, prefix string) (int64, error) {
	...
	for scn.Scan() {
		...
		if len(parts) != 2 {
			return 0, fmt.Errorf("parse %s: expected two fields but got %s", path, line)
		}
	...
	return 0, fmt.Errorf("parse %s: did not find line with prefix %s", path, prefix)
}
