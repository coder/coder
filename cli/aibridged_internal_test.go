...
func TestBuildProviders(t *testing.T) {
	...
	t.Run("IndexedOnly", func(t *testing.T) {
		t.Parallel()
		// The dump directory is now a top-level config: each provider
		// writes under a subdirectory named after the provider.
		cfg := codersdk.AIBridgeConfig{
		...
		assert.Equal(t, "/tmp/aibridge-dump/anthropic-zdr", byName["anthropic-zdr"].APIDumpDir())
		assert.Equal(t, "/tmp/aibridge-dump/openai-azure", byName["openai-azure"].APIDumpDir())
	})
	...
	t.Run("CopilotVariants", func(t *testing.T) {
		...
		assert.Equal(t, "/tmp/aibridge-dump/"+aibridge.ProviderCopilot, byName[aibridge.ProviderCopilot].APIDumpDir())
