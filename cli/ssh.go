...
func setupImmortalSSHStream(ctx context.Context, client *codersdk.Client, workspace codersdk.Workspace, workspaceAgent codersdk.WorkspaceAgent, logger slog.Logger, immortalFallback bool, conn workspacesdk.AgentConn) (net.Conn, *immortalStreamClient, *uuid.UUID, error) {
	// Use shared helper to manage immortal creation/fallback
	connInfo, _ := workspacesdk.New(client).AgentConnectionInfoGeneric(ctx)
	coderConnectHost := ""
	...
	logger.Info(ctx, "immortal: coder connect host", slog.F("coder_connect_host", coderConnectHost))
...
func setupImmortalSSHClient(ctx context.Context, client *codersdk.Client, workspace codersdk.Workspace, workspaceAgent codersdk.WorkspaceAgent, logger slog.Logger, immortalFallback bool, conn workspacesdk.AgentConn) (*gossh.Client, *immortalStreamClient, *uuid.UUID, error) {
...
func (*RootCmd) handleStdioSSH(cfg stdioSSHConfig) error {
	...
	if cfg.immortal {
		...
		var err error
		rawSSH, immortalStreamClient, streamID, err = setupImmortalSSHStream(cfg.ctx, cfg.client, cfg.workspace, cfg.workspaceAgent, cfg.logger, cfg.immortalFallback, cfg.conn)
		if err != nil {
			return err
		}
	} else {
...
func (r *RootCmd) ssh() *serpent.Command {
	...
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) (retErr error) {
			...
			// If --immortal is set, prefer immortal streams and skip Coder Connect fast path.
			...
			if immortal {
				// Use immortal stream for SSH connection
				sshClient, immortalStreamClient, streamID, err = setupImmortalSSHClient(ctx, client, workspace, workspaceAgent, logger, immortalFallback, conn)
				if err != nil {
					return err
				}
			} else {
...
func (c *genericSSHCopier) copy(_ *sync.WaitGroup) {
	...
	go func() {
		...
		// Attempt half-close (write side) if supported; otherwise leave
		// the connection open so the read side can continue until remote EOF.
