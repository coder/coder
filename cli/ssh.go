...
func (r *RootCmd) ssh() *serpent.Command {
	...
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) (retErr error) {
			// Resolve the client session ID before initializing the client so it
			// is attached as client_session_id baggage to every request. It comes
			// from CODER_TRACE_SESSION_ID when set (so Toolbox and the VS Code
			// plugin can correlate the CLI they spawn), otherwise it is generated.
			sessionID, err := resolveClientSessionID(inv)
			if err != nil {
				return err
			}
			...
			logger := inv.Logger
			logger = logger.With(slog.F("client_session_id", sessionID))
			defer func() {
				if retErr != nil {
...
// clientSessionIDEnv is the environment variable the ssh sub-command reads to
// reuse a client session ID generated by a spawning client (Toolbox, the VS
// Code plugin), per the connection-log RFC.
const clientSessionIDEnv = "CODER_TRACE_SESSION_ID"

// resolveClientSessionID returns the client session ID for this ssh invocation.
