...
func Rotate(ctx context.Context, log slog.Logger, sqlDB *sql.DB, ciphers []Cipher) error {
	...
	if err != nil {
		return fmt.Errorf("create cryptdb: %w", err)
	}
	...
	for idx, uid := range userIDs {
		err := cryptDB.InTx(func(cryptTx database.Store) error {
			...
			if err != nil {
				return fmt.Errorf("get user links for user: %w", err)
			}
			for _, userLink := range userLinks {
				...
				if _, err := cryptTx.UpdateUserLink(ctx, database.UpdateUserLinkParams{
					...
					return fmt.Errorf("update user link user_id=%s linked_id=%s: %w", userLink.UserID, userLink.LinkedID, err)
				}
			}
			externalAuthLinks, err := cryptTx.GetExternalAuthLinksByUserID(ctx, uid)
			if err != nil {
				return fmt.Errorf("get git auth links for user: %w", err)
			}
			for _, externalAuthLink := range externalAuthLinks {
				...
				if _, err := cryptTx.UpdateExternalAuthLink(ctx, database.UpdateExternalAuthLinkParams{
					...
					return fmt.Errorf("update external auth link user_id=%s provider_id=%s: %w", externalAuthLink.UserID, externalAuthLink.ProviderID, err)
				}
			}
			return nil
		}, &database.TxOptions{
		...
		if err != nil {
			return fmt.Errorf("update user links: %w", err)
		}
...
func Decrypt(ctx context.Context, log slog.Logger, sqlDB *sql.DB, ciphers []Cipher) error {
	...
	if !ok {
		return fmt.Errorf("developer error: dbcrypt.New did not return *dbCrypt")
	}
...
func Delete(ctx context.Context, log slog.Logger, sqlDB *sql.DB) error {
	...
	if err != nil {
		return fmt.Errorf("get db crypt keys: %w", err)
	}
	for _, k := range keys {
