...
func TestPGCoordinatorSingle_PeriodicRefreshRecoversLostUpdate(t *testing.T) {
	t.Parallel()
	...
	// Trap the periodic refresh TickerFunc so we can synchronize with its
	// creation during coordinator startup.
	refreshTrap := mClock.Trap().TickerFunc("querier", "periodicRefresh")
	defer refreshTrap.Close()

	coordinator, err := tailnet.NewTestPGCoord(ctx, logger, ps, store, mClock)
	require.NoError(t, err)
	defer coordinator.Close()

	// Release the periodic refresh ticker creation.
	refreshTrap.MustWait(ctx).MustRelease(ctx)
	...
	// Look up the agent's peer row so we can update it directly in the DB.
	peers, err := store.GetTailnetPeers(ctx, agent.ID)
	require.NoError(t, err)
	require.Len(t, peers, 1)
	coordID := peers[0].CoordinatorID

	// Write the updated node directly to the DB WITHOUT publishing a
	// notification. This simulates a lost pubsub message.
	pNode, err := agpl.NodeToProto(&agpl.Node{PreferredDERP: 12})
	require.NoError(t, err)
	...
	// The client should NOT have the update yet since no notification was
	// published. Advance the clock past the 30s mapper refresh interval to
	// trigger the periodic refresh, which re-queries the DB. The mock
	// clock requires stepping through each pending event, so we loop
	// AdvanceNext until we have passed 30s of mock time.
	start := mClock.Now()
	for mClock.Now().Sub(start) < 31*time.Second {
		_, w := mClock.AdvanceNext()
		w.MustWait(ctx)
	}

	// The periodic refresh should cause the coordinator to pick up the
	// DB-only update and send it to the client.
	client.AssertEventuallyHasDERP(agent.ID, 12)
