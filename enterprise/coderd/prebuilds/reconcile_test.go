...
//
//	go test -bench='^BenchmarkReconcileAll_NoOps$' -benchmem -benchtime=10x ./enterprise/coderd/prebuilds/
...
//
// This validates the performance benefit of SetLimit(5), which prevents connection pool
// exhaustion when many presets need reconciliation actions (creates/deletes).
//
//	go test -bench='^BenchmarkReconcileAll_ConnectionContention$' -run=^$ -benchtime=5s -count=5 ./enterprise/coderd/prebuilds/
func BenchmarkReconcileAll_ConnectionContention(b *testing.B) {
	benchCases := []struct {
		...
		// 10 total database operations
		//{"5_presets_2_instances", 5, 2},
		//
		//// 20 total database operations
		//{"10_presets_2_instances", 10, 2},
		//
		//// 200 total database operations
		//{"20_presets_10_instances", 20, 10},
		//
		//// 500 total database operations
		//{"50_presets_10_instances", 50, 10},
		//
		//// 1000 total database operations
		//{"100_presets_10_instances", 100, 10},
		{"1000_presets_10_instances", 1000, 10},
	}
			...
			for i := 0; i < b.N; i++ {
				...
				for p := 0; p < bc.presetsForReconciliation; p++ {
					...
					dbgen.Preset(b, db, database.InsertPresetParams{
						TemplateVersionID: tv.ID,
						Name:              "default",
						DesiredInstances:  sql.NullInt32{Int32: int32(bc.desiredInstances), Valid: true},
					})
				...
				if i == 0 {
					// On first iteration, check what actually happened
					workspaces, _ := db.GetWorkspaces(ctx, database.GetWorkspacesParams{
						OwnerID: database.PrebuildsSystemUserID,
					})
					jobs, _ := db.GetProvisionerJobsCreatedAfter(ctx, clock.Now().Add(-1*time.Hour))
					b.Logf("Iteration 0: Created %d workspaces, %d jobs, elapsed=%v",
						len(workspaces), len(jobs), stats.Elapsed)
					break
				}
			}
		})
...
//
// Scenario: 5000 total presets (4950 from inactive versions, 50 active that need work)
//
//	go test -bench='^BenchmarkReconcileAll_Mix$' -run=^$ -benchtime=5s -count=5 ./enterprise/coderd/prebuilds/
func BenchmarkReconcileAll_Mix(b *testing.B) {
	benchCases := []struct {
		...
		desiredInstances     int // Desired prebuilds per preset
	}{
		{
			name:                 "5000_total_50_active",
			inactivePresetsCount: 4950,
			activePresetsCount:   50,
			desiredInstances:     2,
		},
		// Smaller scenario for faster testing
		//{
		//	name:                 "500_total_20_active",
		//	inactivePresetsCount: 480,
		//	activePresetsCount:   20,
		//	desiredInstances:     2,
		//},
	}
	...
	for _, bc := range benchCases {
		b.Run(bc.name, func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				...
				fmt.Println("############### elapsed: ", stats.Elapsed)

				if i == 0 {
					// On first iteration, verify what happened
					workspaces, _ := db.GetWorkspaces(ctx, database.GetWorkspacesParams{
						OwnerID: database.PrebuildsSystemUserID,
					})
					jobs, _ := db.GetProvisionerJobsCreatedAfter(ctx, clock.Now().Add(-1*time.Hour))
					b.Logf("Iteration 0: Total presets=%d, Active=%d, Created workspaces=%d, jobs=%d, elapsed=%v",
						totalCount, bc.activePresetsCount, len(workspaces), len(jobs), stats.Elapsed)
				}
			}
		})
	}
