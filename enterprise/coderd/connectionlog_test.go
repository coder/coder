...
func TestConnectionLogs(t *testing.T) {
	...
	t.Run("SSHInfo", func(t *testing.T) {
		...
		require.Empty(t, logs.ConnectionLogs[0].SSHInfo.CloseTime)
		require.Empty(t, logs.ConnectionLogs[0].SSHInfo.CloseReason)
		...
		require.True(t, logs.ConnectionLogs[0].SSHInfo.CloseTime.Equal(now))
		require.Equal(t, updatedClog.CloseReason.String, logs.ConnectionLogs[0].SSHInfo.CloseReason)
	})
}
