...
func TestManagedAgentLimit(t *testing.T) {
	...
	// Create one AI workspace, which should succeed.
	task, err := codersdk.NewExperimentalClient(cli).CreateTask(ctx, owner.UserID.String(), codersdk.CreateTaskRequest{
		Name:                    "test-task-1",
	...
	// workspace creation.
	task2, err := codersdk.NewExperimentalClient(cli).CreateTask(ctx, owner.UserID.String(), codersdk.CreateTaskRequest{
		Name:              "test-task-2",
		TemplateVersionID: aiTemplate.ActiveVersionID,
	})
