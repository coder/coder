...
func (api *API) patchChatSharingSettings(rw http.ResponseWriter, r *http.Request) {
	...
	if allowedOwners == "" {
		// No field provided - default to 'everyone'.
	...
	err := api.Database.InTx(func(tx database.Store) error {
		...
		// Serialize organization-wide system-role reconciliation across
		// coderd instances. Mirrors the workspace-sharing handler.
		...
		// If sharing is tightened, clear chat ACLs so existing shares
		// stop granting access. In "service_accounts" mode we preserve
