...
func TestTemplateACLBlocksWorkspaceAppAccess(t *testing.T) {
	// Verify that revoking a user's template ACL access also blocks access
	// to workspace apps served via the app proxy.
	t.Parallel()
	...
	// A non-404 status means the auth check passed.
	appURL := fmt.Sprintf("/@%s/%s/apps/test-app/", me.Username, workspace.Name)
	resp, err := memberClient.Request(ctx, http.MethodGet, appURL, nil)
	require.NoError(t, err)
	_ = resp.Body.Close()
	...
	// Verify the member can no longer access the workspace app. The app
	// proxy should check template ACL and block access.
	resp, err = memberClient.Request(ctx, http.MethodGet, appURL, nil)
	require.NoError(t, err)
	_ = resp.Body.Close()
