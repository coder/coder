...
func TestExternalAgentTokensByWorkspaceIDs(t *testing.T) {
	t.Parallel()
	...
	t.Run("Returns tokens for all external-agent workspaces", func(t *testing.T) {
		t.Parallel()
		ctx := testutil.Context(t, testutil.WaitShort)
		...
		resp, err := client.ExternalAgentTokensByWorkspaceIDs(ctx, []uuid.UUID{r.Workspace.ID})
		require.NoError(t, err)
		require.Len(t, resp.Agents, 1)
		require.Equal(t, r.Workspace.ID, resp.Agents[0].WorkspaceID)
		require.Equal(t, "test-agent", resp.Agents[0].AgentName)
		require.Equal(t, r.AgentToken, resp.Agents[0].AgentToken)
	})

	t.Run("Non-external-agent workspace returns no rows", func(t *testing.T) {
		t.Parallel()
		ctx := testutil.Context(t, testutil.WaitShort)
	...
	t.Run("Empty workspace_ids returns 400", func(t *testing.T) {
		t.Parallel()
		ctx := testutil.Context(t, testutil.WaitShort)

		_, err := client.ExternalAgentTokensByWorkspaceIDs(ctx, []uuid.UUID{})
		require.Error(t, err)
		var apiErr *codersdk.Error
		require.ErrorAs(t, err, &apiErr)
