...
// TestPoolWithCircuitBreakerProviders validates that pools work correctly
// with providers that have circuit breaker configuration.
func TestPoolWithCircuitBreakerProviders(t *testing.T) {
	t.Parallel()
	...
	t.Run("DefaultPoolOptions", func(t *testing.T) {
		...
		// DefaultPoolOptions works with no providers (circuit breaker is per-provider).
		pool, err := aibridged.NewCachedBridgePool(aibridged.DefaultPoolOptions, nil, logger, nil, testTracer)
		require.NoError(t, err)
		t.Cleanup(func() { pool.Shutdown(context.Background()) })

		// Pool should be created successfully with default options.
		require.NotNil(t, pool)
	})

	t.Run("WithCircuitBreakerProvider", func(t *testing.T) {
		t.Parallel()
		...
		// Create providers with circuit breaker config using DefaultCircuitBreaker.
		...
		// Acquiring a pool instance should work with circuit breaker provider.
		inst, err := pool.Acquire(t.Context(), aibridged.Request{
			SessionKey:  "key",
			InitiatorID: id,
			APIKeyID:    apiKeyID.String(),
		}, clientFn, newMockMCPFactory(mcpProxy))
		require.NoError(t, err, "acquire pool instance with circuit breaker provider")
		require.NotNil(t, inst)
	})

	t.Run("WithoutCircuitBreakerProvider", func(t *testing.T) {
		t.Parallel()
		...
		// Create providers without circuit breaker config (nil CircuitBreaker).
		providers := []aibridge.Provider{
			aibridge.NewOpenAIProvider(aibridge.OpenAIConfig{
				BaseURL:        "https://api.openai.com",
				Key:            "test-key",
		...
		// Pool should work normally without circuit breaker.
		inst, err := pool.Acquire(t.Context(), aibridged.Request{
			SessionKey:  "key",
			InitiatorID: id,
			APIKeyID:    apiKeyID.String(),
