...
func TestUnitGraph(t *testing.T) {
	...
	t.Run("ForwardAndReverseEdges", func(t *testing.T) {
		...
		require.Contains(t, vertices, unit.UnitEdge{
			From: unit1,
			To:   unit2,
		...
		require.Contains(t, unit2ReverseEdges, unit.UnitEdge{
			From: unit1,
			To:   unit2,
			Edge: unit.StatusCompleted,
		})
		...
		require.Contains(t, unit3ReverseEdges, unit.UnitEdge{
			From: unit1,
			To:   unit3,
			Edge: unit.StatusStarted,
		})
