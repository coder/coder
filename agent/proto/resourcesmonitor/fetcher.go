...
func (f *fetcher) FetchMemory() (total int64, used int64, err error) {
	...
	if err != nil {
		return 0, 0, fmt.Errorf("failed to fetch memory: %w", err)

	}
	if mem.Total == nil {
		return 0, 0, errors.New("memory total is nil - can not fetch memory")
	}
...
func (f *fetcher) FetchVolume(volume string) (total int64, used int64, err error) {
	...
	if vol.Total == nil {
		return 0, 0, errors.New("volume total is nil - can not fetch volume")
