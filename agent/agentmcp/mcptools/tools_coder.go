...
func RegisterCoderReportTask(ta ToolAdder, log slog.Logger) {
	ta.AddTool(toolCoderReportTask, handleCoderReportTask(log))
}
...
func handleCoderReportTask(log slog.Logger) mcpserver.ToolHandlerFunc {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		log.Info(ctx, "Reporting task", slog.F("summary", summary), slog.F("link", link), slog.F("done", done), slog.F("emoji", emoji))
		/*
...
func handleCoderWhoami(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	if !ok {
		return nil, xerrors.New("coder_url is required, is CODER_URL set?")
	}

	coderSessionToken, ok := args["coder_session_token"].(string)
	if !ok {
		return nil, xerrors.New("coder_session_token is required, is CODER_SESSION_TOKEN set?")
	}

	coderURLParsed, err := url.Parse(coderURL)
	if err != nil {
		return nil, xerrors.Errorf("invalid coder_url: %s", err.Error())
	}
...
func handleCoderWorkspaceExec(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	if !ok {
		return nil, xerrors.New("workspaceis required")
	}
