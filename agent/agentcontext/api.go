...
// in each requested directory, the on-demand counterpart of snapshot
// discovery for directories outside the scan roots.
	...
	for _, dir := range req.Directories {
		...
		for _, res := range a.manager.resolver.ResolveInstructionFiles(dir) {
			...
			if res.Status == StatusOK {
				// JSON cannot carry invalid UTF-8; replacing it before the
				// bytes are counted keeps every cap on what is shipped.
