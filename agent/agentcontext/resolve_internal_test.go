...
// TestCompactResourcesDropsReplacedOccurrence checks that the tombstone
// appendResource leaves behind when a valid occurrence replaces an invalid
// one with the same ID is removed before a result is shipped, since an
// empty resource would otherwise be pinned under an empty source.
