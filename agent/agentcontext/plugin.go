...
// pluginOrigin says whether a plugin candidate directory is a scan root
// itself or an entry of a plugin container under a scan root.
...
// or its $schema is absent or names a different format; err then
// explains why the file was not taken as an Agent Plugins manifest so
// the caller can decide whether to report it. When recognized is true,
// a non-nil err rejects the plugin.
func parsePluginManifest(data []byte) (m pluginManifest, recognized bool, err error) {
	...
	if !ok {
		return pluginManifest{}, false, xerrors.Errorf("plugin.json has no $schema; Agent Plugins manifests declare %q", PluginSchemaV1)
	}
	if json.Unmarshal(raw, &schema) != nil || !strings.HasPrefix(schema, pluginSchemaPrefix) {
		return pluginManifest{}, false, xerrors.Errorf("plugin.json $schema %s is not an Agent Plugins schema", string(raw))
	}
...
// path.
//
// origin says where dir was found. A plugin.json inside a plugins/ or
// .agents/plugins/ container is meant to be an Agent Plugins manifest,
// so one that is malformed or lacks the $schema is reported as an
// invalid plugin. At a scan root the same file may belong to another
// tool and is ignored after a debug log.
func (r *Resolver) discoverPlugin(dir string, root ScanRoot, out *[]Resource, seenID map[string]int, pluginNames map[string]int, origin pluginOrigin) (recognized, ok bool) {
	...
	if !recognized {
		if origin == pluginAtScanRoot {
			r.Logger.Debug(context.Background(), "ignoring plugin.json at scan root",
				slog.F("path", manifestPath), slog.Error(err))
			return false, false
		}
		r.Logger.Warn(context.Background(), "invalid plugin manifest",
			slog.F("path", manifestPath), slog.Error(err))
	...
	if _, taken := pluginNames[manifest.Name]; taken {
		r.Logger.Warn(context.Background(), "duplicate plugin name",
			slog.F("path", manifestPath), slog.F("plugin", manifest.Name))
		res.Status = StatusInvalid
