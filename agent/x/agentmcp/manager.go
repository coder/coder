...
// same enriched environment a .mcp.json stdio server receives, then
// PLUGIN_ROOT and PLUGIN_DATA, then the entry's own env on top. Entry
// env cannot name PLUGIN_ROOT or PLUGIN_DATA, so those two always hold
// the plugin's values.
func (m *Manager) buildPluginEnv(ctx context.Context, scope PluginScope, explicit map[string]string) []string {
	env := m.buildEnv(ctx, map[string]string{
