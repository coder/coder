...
// rejected. Missing files are silently skipped; whole-file parse errors
// are logged and skipped. Entry-level errors are logged and reported.
//
...
// entry with a taken name is reported so the plugin's server does not
...
func (m *Manager) parseAndDedup(ctx context.Context, sources []ConfigSource) ([]ServerConfig, []ServerStatus, map[string]fileSnapshot) {
	...
	for _, src := range sources {
		...
		for _, cfg := range configs {
			if _, taken := seen[cfg.Name]; taken {
				if cfg.Plugin == nil {
					continue
				}
				...
				rejected = append(rejected, ServerStatus{
					...
					Err:        "server name already in use by an earlier config",
				})
				continue
			}
