...
// inherits the enriched agent environment, receives the plugin variables
// and entry env with only plugin placeholders expanded, and runs in the
// configured cwd.
func TestReloadSources_PluginStdioLaunch(t *testing.T) {
	...
	assert.True(t, updateEnvCalled, "plugin servers receive the enriched agent environment")
	...
	assert.Equal(t, "secret", env["CODER_AGENT_TOKEN"])
	assert.Equal(t, "1", env["UNRELATED"])
	assert.Equal(t, "1", env["FROM_UPDATE_ENV"])
