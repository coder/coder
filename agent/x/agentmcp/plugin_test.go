...
func TestParseSource_PluginEntries(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			want: func(scope agentmcp.PluginScope) agentmcp.ServerConfig {
				return agentmcp.ServerConfig{
					Name: "srv", Transport: "sse", URL: "https://example.com/sse", Plugin: &scope,
				}
			},
		},
		{
		...
		},
		{
			...
			entry:       map[string]any{"type": "sse", "url": "https://example.com/mcp#frag"},
			errContains: "fragment",
		},
		{
			name:        "URLEmptyFragmentRejected",
			entry:       map[string]any{"type": "sse", "url": "https://example.com/mcp#"},
			errContains: "fragment",
		},
		{
