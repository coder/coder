...
type agent struct {
	...
	boundaryAuditListener *BoundaryAuditListener
}
...
func (a *agent) run() (retErr error) {
	...
	// Boundary audit logs are part of auditing, we should keep sending them via
	// gracefulShutdownBehaviorRemain.
	connMan.startAgentAPI("boundary audit", gracefulShutdownBehaviorRemain, a.runBoundaryAuditListener)
...
func (a *agent) Close() error {
	...
	if a.boundaryAuditListener != nil {
		if err := a.boundaryAuditListener.Close(); err != nil {
			a.logger.Error(a.hardCtx, "boundary audit listener close", slog.Error(err))
		}
	}
...
func (a *agent) runBoundaryAuditListener(ctx context.Context, aAPI proto.DRPCAgentClient27) error {
	...
	if err := listener.Start(ctx); err != nil {
		...
		a.logger.Warn(ctx, "failed to start boundary audit listener", slog.Error(err))
		return nil
	}

	a.closeMutex.Lock()
	a.boundaryAuditListener = listener
	a.closeMutex.Unlock()

	// Wait for context to be done, then close the listener.
	<-ctx.Done()

	if err := listener.Close(); err != nil {
		a.logger.Warn(ctx, "failed to close boundary audit listener", slog.Error(err))
	}

	return nil
}

// boundaryAuditReporterAdapter adapts proto.DRPCAgentClient27 to BoundaryAuditReporter.
type boundaryAuditReporterAdapter struct {
	aAPI proto.DRPCAgentClient27
}
