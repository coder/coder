...
// deprecatedConnectionType maps an app name onto the family enum, still set
// for coderd predating API v2.13.
func deprecatedConnectionType(appName string) proto.Connection_Type {
	...
	case codersdk.AppFamilyReconnectingPTY:
		return proto.Connection_RECONNECTING_PTY
	default:
		return proto.Connection_TYPE_UNSPECIFIED
	}
}

func (a *agent) reportConnection(id uuid.UUID, appName string, ip string) (disconnected func(code int, reason string)) {
	connectionType := deprecatedConnectionType(appName)
