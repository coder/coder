...
//   - Detect coderd availability issues: "forward_failed" drops increase when
...
//     "forward_failed").
//   - Boundary-reported: boundary self-reports drops via proto Metadata
//     (reasons "boundary_channel_full", "boundary_batch_full"). These
//     arrive on the next successful flush from boundary.
//
// What these metrics do NOT capture: drops that boundary accumulates but
// never reports. For example, if the boundary process exits or is killed
// before its next flush. Those drops are lost silently.
type Metrics struct {
...
func newMetrics(registerer prometheus.Registerer) *Metrics {
	batchesDropped := prometheus.NewCounterVec(prometheus.CounterOpts{
		...
		Help:      "Total number of boundary log batches dropped. The reason label indicates where: buffer_full (agent channel), forward_failed (RPC to coderd).",
	}, []string{"reason"})
	...
	logsDropped := prometheus.NewCounterVec(prometheus.CounterOpts{
		...
		Help:      "Total number of individual boundary log entries dropped. The reason label indicates where: buffer_full (agent channel), forward_failed (RPC to coderd), boundary_channel_full or boundary_batch_full (self-reported by boundary).",
	}, []string{"reason"})
	...
	batchesForwarded := prometheus.NewCounter(prometheus.CounterOpts{
		...
		Help:      "Total number of boundary log batches successfully forwarded.",
	})
