...
func EnvInfo(ctx context.Context, execer agentexec.Execer, container, containerUser string) (*DockerEnvInfoer, error) {
	...
	stdout, stderr, err := run(ctx, execer, cmd, args...)
	if err != nil {
		return nil, fmt.Errorf("get container user: read /etc/passwd: %w: %q", err, stderr)
	}
	...
	if foundLine == "" {
		return nil, fmt.Errorf("get container user: no matching entry for %q found in /etc/passwd", containerUser)
	}
	// Parse the output of /etc/passwd. It looks like this:
	// postgres:x:999:999::/var/lib/postgresql:/bin/bash
	passwdFields := strings.Split(foundLine, ":")
	if len(passwdFields) != 7 {
		return nil, fmt.Errorf("get container user: invalid line in /etc/passwd: %q", foundLine)
	}
...
func devcontainerEnv(ctx context.Context, execer agentexec.Execer, container string) ([]string, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("inspect container: %w: %q", err, stderr)
	}
	if len(ins) != 1 {
...
func (dcl *DockerCLILister) List(ctx context.Context) (codersdk.WorkspaceAgentListContainersResponse, error) {
	...
	if err := scanner.Err(); err != nil {
		return codersdk.WorkspaceAgentListContainersResponse{}, fmt.Errorf("scan docker ps output: %w", err)
	}
	...
	if err != nil {
		return codersdk.WorkspaceAgentListContainersResponse{}, fmt.Errorf("run docker inspect: %w", err)
	}
	for _, in := range ins {
...
func runDockerInspect(ctx context.Context, execer agentexec.Execer, ids ...string) ([]dockerInspect, string, error) {
	...
	if err := json.NewDecoder(&stdoutBuf).Decode(&ins); err != nil {
		return nil, stderr, fmt.Errorf("decode docker inspect output: %w", err)
	}
