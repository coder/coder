...
func TestEmbeddedMicroVMConfigWiresEvaluatorRecorderAndAgent(t *testing.T) {
	...
	stagedDir := config.hostOptions.Mounts[0].Source
	require.NotEqual(t, filepath.Dir(binaryPath), stagedDir,
		"binary must be staged out of its origin directory, whose permissions the sandbox does not control")
	stagedDirInfo, err := os.Stat(stagedDir)
	require.NoError(t, err)
	require.Equal(t, os.FileMode(0o755), stagedDirInfo.Mode().Perm(),
		"staged directory must be world-traversable for capability-dropped guest processes")
	stagedBinary := filepath.Join(stagedDir, filepath.Base(binaryPath))
	stagedInfo, err := os.Stat(stagedBinary)
	require.NoError(t, err)
	require.Equal(t, os.FileMode(0o755), stagedInfo.Mode().Perm())
	originalContent, err := os.ReadFile(binaryPath)
	require.NoError(t, err)
	stagedContent, err := os.ReadFile(stagedBinary)
	require.NoError(t, err)
	require.Equal(t, originalContent, stagedContent)
